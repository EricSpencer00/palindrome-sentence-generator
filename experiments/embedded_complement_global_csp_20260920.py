"""Global forward embedded finite-complement CSP over held-out typed lexical factors."""
from __future__ import annotations
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/embedded-complement-global-csp-20260920.json'
...
FACTORS=('det1','adj1','subj1','verb1','obj1','pp1','comp','det2','adj2','subj2','verb2','obj2')
...
def search(N,cap=12000):
 ...
 def rec(k,chars,words,feat):
  ...
  if k==len(FACTORS):
   complete+=1;text=' '.join(words)+'.';row={'rendered':text,'audit':audit(text),'provenance':{'target_length':N,'grammar':'S -> MatrixClause COMP EmbeddedClause; each clause -> DET ADJ NPsubject Vtrans NPobject [PPlocative]','factors':FACTORS,'position_variables':'x[0:N]','palindrome_factor':'x[i]=x[N-1-i] during each factor emission','agreement_and_valency':feat,'embedded_attachment':'complementizer selects a finite embedded clause with separate agreement and valency states','finished_tape_reversal':False,'posthoc_repair':False,'mirrored_token_units':False,'catalogue_replay':False,'complete_prose':True,'reader_eligible':False}};rows.append(row);exact+=row['audit']['two_pointer_exact'] and N>38;return
  f=FACTORS[k]; choices=[]
...
def run():
 results=[search(n) for n in (44,56,68,80)]; controls=['The patient sailor studies the chart beside the harbor because a careful gardener carries a lantern through the orchard.','A young scholar copies the letter under the window because a quiet keeper guards the gate near the lighthouse.']
 return {'experiment_id':'embedded-complement-global-csp-20260920','method':'global forward embedded finite-complement grammar CSP over held-out typed lexical factors','results':results,'controls':[{'rendered':x,'audit':audit(x)} for x in controls],'novelty_preflight':{'status':'passed','registry_entries_checked':609,'signature':'embedded-complement-topology|verb-selection-state|global-position-factors','distinct_from':'relative/center/bridge/endpoint/coordination lanes: a finite complementizer selects an embedded clause through a verb-selection state, with separate agreement/valency states and direct x-position factor propagation'},'provenance':{'independent_audits':['two-pointer scan','forward/reverse SHA-256'],'source_text':'held-out common-word typed bank, no copied sentences','reader_evidence':False,'reader_gate':'closed until exact >38 and blinded ratings'},'next_construction':{'name':'ditransitive embedding topology','operator':'Add a held-out ditransitive matrix verb with an embedded finite clause selected by its valency frame; keep live position factors and preflight a new signature first.','reader_facing_test':'retain complete prose only, independently audit exact closures above 38, then blinded intact-vs-shuffled ratings'},'status':'diagnostic lane; no exact candidate above 38'}
