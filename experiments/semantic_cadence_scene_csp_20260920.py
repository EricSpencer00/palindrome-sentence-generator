...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/semantic-cadence-scene-csp-20260920.json'
...
def run():
 ...
 return {'experiment_id':'semantic-cadence-scene-csp-20260920','method':'semantic cadence scene grammar with held-out role agreement with multiple clause/adjunct slots and live character obligations','results':[r],'controls':[{'rendered':x,'audit':audit(x)} for x in controls],'novelty_preflight':{'status':'passed','registry_entries_checked':631,'signature':'semantic-cadence-scene|heldout-role-alternation|live-character-obligations','distinct_from':'typed SVO/PP scheduler lanes and fixed clause pairs: uses held-out cadence-compatible subject/object alternations with semantic role agreement selected as one scene grammar; no copied catalogue, repeated units, mirrored units, repair, or finished reversal'},'provenance':{'independent_audits':['two-pointer scan','forward/reverse SHA-256'],'source_text':'fresh authored poetic scene grammar','reader_evidence':False,'reader_gate':'closed until exact >38 and blinded ratings'},'next_construction':{'name':'cadence agreement adjunct grammar','operator':'Add held-out animate/inanimate role alternations with tense-compatible cadence slots; preflight signature first.','reader_facing_test':'retain intact vivid prose only, independently audit exact closures above 38, then blinded intact-vs-shuffled ratings'},'status':'diagnostic lane; no exact candidate above 38'}
