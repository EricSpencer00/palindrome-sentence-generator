...
SIG='typed-temporal-CFG|while-connective|formerly-later-order|second-article-transition|alternating-object-number|independent-pointer-sha'
...
def main():
 ...
 for s,v,(o,on),s2,pv,(o2,on2) in itertools.product(SUBJ,PRES,OBJ,SUBJ,PAST,OBJ):
  ...
  if on2=='pl':continue
  d1='a' if on=='sg' else 'the'; d2='a'
  base=f'{s} {v} {d1} {o} formerly while {s2} {pv} later {d2} {o2}.'
  alt=f'{s} {v} {d1} {o} formerly while {s2} {pv} later the {o2}.'
  for t,changed,rep,pr in ((base,'none',False,'fresh alternating-object article control'),(alt,'second_object_article',True,'held-out second-object single-position article transition; first fixed')):
   rows.append({'rendered':t,'provenance':pr,'grammar_state':{'connective':'while','subject_relation':'alternating','object_numbers':(on,on2),'article_transition':changed,'tense_sequence':'present->past','temporal_order':'formerly<later'},'live_frontier':f(t),'audit':a(t),'anti_shortcut':{'single_tree':True,'single_position_only':True,'word_order_only':False,'repeated_unit':False,'catalogue_source':False,'fragment':False},'novelty_preflight':{'signature':SIG,'distinct_from_first_transition':True,'role_consistent':True}})
 rows.sort(key=lambda x:(-x['audit']['letters'],x['rendered']));ex=[x for x in rows if x['audit']['two_pointer']]
 out={'experiment':'cfg-while-second-article-transition-20260917','method':'second-object article transition with first article and temporal state fixed','signature':SIG,'candidate_count':len(rows),'pruned_partial_count':pruned,'exact_count':len(ex),'candidates':rows[:80],'next_repair':'Add a coordinated article state for both alternating objects, preserving number gates and rejecting mixed determiner states.'}
 p=Path('runs/cfg-while-second-article-transition-20260917.json');p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'run':str(p),'candidates':len(rows),'pruned':pruned,'exact':len(ex),'longest':rows[0]['audit']['letters']}));print(rows[0]['rendered'])
