...
def test_dependency_frame_lane_has_complete_controls_and_independent_audits():
    ...
    assert result["stats"]["held_out_passive_states"] == 3
    assert result["stats"]["held_out_modal_states"] == 3
    ...
    assert sum(row.get("held_out_passive", False) for row in result["rendered_candidates"]) == 3
    assert sum(row.get("held_out_modal", False) for row in result["rendered_candidates"]) == 3
