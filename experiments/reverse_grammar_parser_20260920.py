...
def main():
            ...
            for shape in shapes:
                ...
                for right in parse_reverse(tape[::-1], shape):
                    ...
                    if a["exact"] and text not in seen and len(set(clause+right)) == len(clause+right):
                        seen.add(text); candidates.append({"rendered":text,"audit":a,"provenance":{"construction":"authored left-clause enumeration with reverse-stream grammar parser","left_clause":rendered,"right_shape":name,"reverse_stream_constraint":True,"finished_tape_reversal":True,"post_hoc_repair":False,"catalogue_text":False,"repeated_word":False,"promotion_status":"diagnostic_only; reverse-stream parse is not the project's admissible generation path"},"reader_status":"not reader-eligible; exactness does not certify readability"})
    ...
    payload={"experiment_id":EXPERIMENT_ID,"method":"reverse-stream grammar parser","stats":{"left_clauses":len(LEFT),"states":states,"reverse_parses":parsed,"exact":len(candidates),"fresh_exact_gt38":sum(x["audit"]["letters"]>38 for x in candidates)},"candidates":candidates[:50],"diagnostic_parses":examples,"independent_audit":"two-pointer mismatch scan plus independent SHA-256 forward/reverse equality","novelty":"diagnostic parser-intersection lane; reverse-stream parsing is explicitly not promoted as the admissible no-shortcut generator","promotion_status":"rejected_for_core_generation_even_if_a_parse_closes","next_constructive_method":"stream two independently generated clause grammars through a shared character obligation without materializing or reversing a completed clause"}
