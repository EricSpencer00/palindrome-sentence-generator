"""Pre-render Earley lane with coordinated prepared-and-willing VP frame."""
from __future__ import annotations
import hashlib,json
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
ID="cfg-earley-prepared-willing-coord-vp-20260921"
SIGNATURE="earley-coordinated-adjective-vp|prepared-and-willing|plural-agreement|live-mirrored-domain"
REGISTRY=ROOT/"docs/experiment-novelty-registry.json"; OUT=ROOT/"runs"/(ID+".json")
CLAUSES=(
 {"id":"wardens-prepared-willing-repair","subject":"The watchful wardens","aux":"are","frame":"prepared and willing to repair","object":"the iron gates","number":"pl","valency":"transitive-locative"},
 {"id":"carpenters-prepared-willing-carry","subject":"The skilled carpenters","aux":"are","frame":"prepared and willing to carry","object":"some sturdy chairs","number":"pl","valency":"transitive-locative"},
 {"id":"keepers-prepared-willing-record","subject":"The patient keepers","aux":"are","frame":"prepared and willing to record","object":"the morning ledgers","number":"pl","valency":"transitive-locative"},)
PPS=(
...
def chart(c,p):
 ...
 return {"algorithm":"earley_item_chart","productions":["S -> NP VP","VP -> Aux Adj and Adj Inf NP PP","PP -> P Det N"],"items":[{"lhs":"S","dot":0,"features":{"number":c["number"]}},{"lhs":"VP","dot":4,"features":{"number":c["number"],"aux":c["aux"],"coordination":"prepared-and-willing"}},{"lhs":"PP","dot":3,"features":{"number":p["number"],"valency":p["valency"]}}],"agreement_unified":u,"accepted":u}
def domains(left,right):
 l,r=norm(left),norm(right); rows=[]; bad=[]
 for i in range(min(len(l),len(r))):
  j=len(r)-1-i; d=sorted({l[i]}&{r[j]}); x={"position":i,"opposing_position":j,"left_owner":"left_coordinated_VP_item","right_owner":"right_coordinated_VP_item","left_support":l[i],"right_support":r[j],"domain":d,"survives":bool(d)}; rows.append(x)
  if not d: bad.append(x)
 return {"algorithm":"online_mirrored_domain_intersection","invariant":"each support belongs to a coordinated VP Earley item; empty domains remain conflicts","pairs_checked":len(rows),"surviving_pairs":sum(x["survives"] for x in rows),"conflict_count":len(bad),"first_conflict":bad[0] if bad else None,"ledger":rows[:24]}
def audit(s,k):
 t=norm(s)
 if k=="pointer":
...
def run():
 ...
 return {"experiment_id":ID,"signature":SIGNATURE,"method":"pre-render prepared-and-willing coordinated VP Earley expansion with live mirrored domains","novelty_preflight":pre,"rows":rows,"stats":{"states_examined":len(rows),"over_39":sum(x["letters"]>=39 for x in rows),"exact":sum(x["mechanically_admitted"] for x in rows),"agreement_unified":sum(x["left_chart"]["accepted"] and x["right_chart"]["accepted"] for x in rows),"domain_conflicts":sum(x["mirrored_domains"]["conflict_count"] for x in rows),"rendered_lengths":[x["letters"] for x in rows]},"provenance":{"generator_sha256":hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),"independent_audits":["pointer","forward/reverse SHA","Earley chart","live mirrored-domain ledger"],"brown_usage":"none; no borrowed prose"},"anti_shortcut_policy":"No post-render repair, fixed tape, reverse decoder, mirrored units, word-order symmetry, catalogue text, or reward loop.","next_operator":"Add a held-out ready-and-eager coordinated VP before expansion."}
