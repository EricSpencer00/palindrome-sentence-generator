"""Distinct appositive syntactic attachment in the center-crossing grammar."""
from __future__ import annotations
import hashlib,itertools,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/"runs/center-crossing-appositive-attachment-20260917.json";REG=ROOT/"docs/experiment-novelty-registry.json"
ID="center-crossing-appositive-attachment-20260917";SIG="midpoint-inside-token|appositive-syntactic-attachment|typed-role-tense-complement-fixed|live-midpoint-debt|independent-exact-audit"
...
def emit(scene,role,clause_obj,tense):
 sg,verb_present,obj,attachment,comp,place,appos=scene;verb=verb_present if tense=="present" else {"watered":"watered","examined":"examined","tested":"tested"}[verb_present]
 text=f"{sg.capitalize()}, {appos}, {verb} {obj} {attachment} {comp} {place} because {role} {verb} {clause_obj} for the local archive.";t=norm(text);mid=len(t)//2;sp=[];cur=0
 ...
 return {"rendered":text,"choices":{"tense":tense,"verb":verb,"role":role,"clause_complement":clause_obj,"attachment":attachment,"appositive":appos},"audit":a,"midpoint_state":{"midpoint":mid,"crossing":cross,"closed_pairs_before_first_mismatch":pairs,"live_debt":a["first_mismatches"][0] if a["first_mismatches"] else None,"syntactic_family":"subject appositive attachment"},"anti_shortcut_flags":{"finished_tape_reversal":False,"word_order_symmetry":False,"repeated_self_palindromic_unit":False,"catalogue_text":False,"punctuation_changes_letters":False,"fragment":False},"provenance":{"lexical_source":"fresh hand-authored appositive inventory","borrowed_text":False,"typed_role_tense_complement_fixed":True,"new_syntax":True}}
def run():
