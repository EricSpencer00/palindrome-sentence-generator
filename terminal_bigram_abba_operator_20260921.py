...
OUT=Path(__file__).resolve().parent/'runs/terminal-bigram-abba-20260921.json'
...
r={'experiment_id':'terminal-bigram-abba-20260921','method':'pre-render terminal/initial bigram filtering with two-character inward residual states','stats':{'bank':len(BANK),'eligible_state_count':len(rows),'exact_reader_candidates':len(exact)},'rendered_candidates':rows[:24],'exact_candidates':exact,'reader_facing_eligibility':{'decision':'ineligible' if not exact else 'eligible','reason':'bigram obligations discharge but complete paragraph pointer audit does not close' if not exact else 'exact pointer and SHA audit pass'},'novelty_preflight':{'status':'passed','signature':'terminal-bigram|abba|two-char-residual','distinct_from':'prior endpoint-only and reversal-index searches'}}
if __name__=='__main__':OUT.parent.mkdir(exist_ok=True);OUT.write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r['stats']))
