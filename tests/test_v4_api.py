...
def test_v4_method_and_best_evaluation_are_explicitly_diagnostic():
    ...
    assert method.json()["optimization"]["search_history"][-1] == "authored-grammatical-clause-pairs-20260920"
    ...
    assert len(method.json()["method_runs"]) == 15
    ...
    assert len(method.json()["rlaif_frontier"]) == 7
