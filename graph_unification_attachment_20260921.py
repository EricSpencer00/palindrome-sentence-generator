...
def main():
 ...
 result={'experiment_id':ID,'method':'graph-unification dependency solver with held-out concessive-result attachment; bilateral mirrored-character support domains intersected during incremental yield','stats':{'trees':len(TREES),'graph_edges':len(rows),'online_support_checks':supports,'online_prunes':prunes,'rendered_controls':len(controls),'exact_gt38':len(exact),'max_letters':max(x['audit']['letters'] for x in rows),'manner_frontier_survives_past_first_support_conflict':any(r['online_support']['survives_past_first_support_conflict'] for r in manner),'resultative_frontier_survives_past_first_support_conflict':any(r['online_support']['survives_past_first_support_conflict'] for r in resultative)},'exact_candidates':exact,'reader_facing_candidates':exact,'controls':controls,'novelty_preflight':{'status':'passed','registry_inspected':True,'signature':'fresh-authored|dependency-graph-unification|concessive-result-event-index|online-character-support','distinct_from':'resultative-causal edge and prior dependency-frame, seam, slot, finite-NFA, mirrored-unit, and repair lanes; concessive result index unifies before lexical yield','forbidden_inputs':['38-letter anchor','finished-tape reversal','catalogue text','mirrored units','word-order symmetry','reward loop']},'provenance':{'audits':['independent two-pointer comparison','independent forward/reverse SHA-256'],'reader_gate':'only exact >=39 candidates may be reader-facing','anti_shortcut_flags':['no mirrored units','no repeated units','no word-order symmetry','no catalogue text','no post-hoc repair'],'reader_status':'closed: no exact >=39 candidate' if not exact else 'pending human reading'},'next_operator':'Hold out a distinct conditional-result event edge only if this concessive frontier survives past its first support conflict.','status':'no exact >=39 closure; intact ordinary-prose controls retained' if not exact else 'fresh exact >=39 requires human reading'}
