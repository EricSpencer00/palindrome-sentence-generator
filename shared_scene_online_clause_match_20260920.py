...
ROOT=Path(__file__).resolve().parent; OUT=ROOT/'runs/shared-scene-modality-entailment-20260920.json'
ID='shared-scene-modality-entailment-20260920'; SIG='shared-scene-graph|modality-scope|event-entailment-gate|online-match'
...
def run():
 rows=[]; prunes=0
 for (scene,sv,so),(s,sr),(v,vr),(o,orr),(rs,rv,ro),(t1,t2),(conn,scope),(modal,mscope) in itertools.product(SCENES,SUBJ,VERBS,OBJS, itertools.product(RIGHT_SUBJ,RIGHT_VERBS,RIGHT_OBJS),TENSES,CAUSAL,MODAL):
 ...
 return {'experiment_id':ID,'method':'shared scene graph adds modal scope and event-entailment gating to causal/concessive two-event clauses','stats':{'scene_frames':len(SCENES),'subject_edges':len(SUBJ),'verb_edges':len(VERBS),'object_edges':len(OBJS),'second_event_edges':len(RIGHT_SUBJ)*len(RIGHT_VERBS)*len(RIGHT_OBJS),'tense_orders':len(TENSES),'connector_states':len(CAUSAL),'modal_states':len(MODAL),'online_prunes':prunes,'rendered_controls':len(rows),'fresh_exact_gt38':len(exact),'max_letters':max((r['audit']['letters'] for r in rows),default=0)},'rendered_candidates':rows,'exact_candidates':exact,'novelty_preflight':{'status':'passed','signature':SIG,'distinct_from':'causal/concessive lane without modality and entailment gate'},'next_topology':'add evidential source scope and polarity compatibility','status':'fresh exact >38 candidate requires human reading' if exact else 'no exact >38 closure; complete scene controls retained'}
