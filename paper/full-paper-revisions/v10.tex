...
\title{The Mirror Cost of English: Exact Palindromic Generation as Bidirectional Re-segmentation}
...
\begin{document}
% Full-paper snapshot 10; critique and successor changes are in FULL-PAPER-REVISION-LEDGER.md.
\maketitle

\begin{abstract}
An exact palindrome requires one letter stream to support two readings. Matched forward/reverse re-segmentation yields a 2.16--3.60-bit-per-letter reverse penalty and lower reverse dictionary coverage across 57 retained WikiText-2 configurations. A sound POS-shape gate provides a bounded candidate-supply case study without establishing grammar or meaning.
\end{abstract}
...
Our empirical question is deliberately narrow: holding a finite lexicon, a
segmentation objective, and a scoring model fixed, how much does reversal
change lexical coverage and causal language-model cost?
...
The final manuscript makes a narrow empirical NLP claim about lexicalization under reversal; exact decoding is its case study rather than its novelty claim.

\section*{Limitations}

The evidence is pilot-scale: one corpus, one lexicon, related GPT-2 models, aggregate saved cells, and no human quality study.
