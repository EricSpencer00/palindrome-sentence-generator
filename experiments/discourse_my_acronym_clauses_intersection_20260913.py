"""Full connected-tree scheduler for the explicit my/acronym discourse."""
...
spec = importlib.util.spec_from_file_location("my_acronym_engine_20260913", ENGINE_PATH)
...
BASE = ENGINE.BASE; ENGINE.FIXED_LABELS.update({"fixed_my", "fixed_notes", "fixed_describe", "fixed_record", "fixed_mark", "fixed_copy", "fixed_form", "fixed_and", "fixed_the", "fixed_final", "fixed_clean", "fixed_narrow", "fixed_bright", "fixed_small", "fixed_map", "fixed_route", "fixed_sign", "fixed_chart", "fixed_acronym"})
from llm_palindrome.admission import mechanical_admission_checks

MIN_LETTERS, MAX_LETTERS = 100, 180
FRAMES = (("map", "clean"), ("route", "narrow"), ("sign", "bright"), ("chart", "small"))


class MyAcronymGrammar:
    ...
    def productions(self, lhs):
        if lhs.name == "D":
            rhs = [BASE.sym("T", label="fixed_my", form="my"), BASE.sym("T", label="fixed_notes", form="notes"), BASE.sym("T", label="fixed_describe", form="describe"), BASE.sym("NP", role="object_1", frame="map", type="object", number="sing", depth="0")]
            for i, (verb, frame) in enumerate((("record", "route"), ("mark", "sign"), ("copy", "chart")), 2): rhs.extend((BASE.sym("T", label=f"fixed_and_{i}", form="and"), BASE.sym("T", label=f"fixed_{verb}", form=verb), BASE.sym("NP", role=f"object_{i}", frame=frame, type="object", number="sing", depth="0")))
            rhs.extend((BASE.sym("T", label="fixed_and_5", form="and"), BASE.sym("T", label="fixed_form", form="form"), BASE.sym("T", label="fixed_the", form="the"), BASE.sym("T", label="fixed_final", form="final"), BASE.sym("T", label="fixed_acronym", form="acronym")))
            return (BASE.Production("D:my-notes-describe-map-record-route-mark-sign-copy-chart-form-acronym", lhs, tuple(rhs)),)
        if lhs.name == "NP":
...
def semantic_witness(tree):
    ...
    if tree: visit(tree)
    return {"semantic_roles": roles, "event_frames": frames, "relative_count": 0, "agreement_ok": bool(tree and tree.children[0].terminal == "my" and tree.children[1].terminal == "notes"), "valency_ok": frames == [x[0] for x in FRAMES], "complete_tree": bool(tree and tree.symbol.name == "D")}
...
def explicit_control(grammar):
    ...
    while True:
        ...
        if symbol.name == "D": ident = "D:my-notes-describe-map-record-route-mark-sign-copy-chart-form-acronym"
        elif symbol.name == "NP":
...
def boundary_preflight():
    grammar = MyAcronymGrammar(); state = explicit_control(grammar); events = []; failed = None
...
def run(max_states=100000):
    grammar = MyAcronymGrammar(); result = solver(grammar, max_states); control_state = explicit_control(grammar); control = audit(grammar, BASE.render(control_state), "complete_explicit_discourse_control", control_state.trace); control.update({"diagnostic_only": True, "grammar_tree_fully_expanded": True, "provenance": {"construction": "five explicit ordinary note-taking action/object clauses", "shared_tree": True}, "reader_status": "grammar control only; not a palindrome candidate or readability evidence"})
    return {"status": "discourse_my_acronym_clauses_intersection", "config": {"min_letters": MIN_LETTERS, "max_letters": MAX_LETTERS, "max_depth": 0, "max_states": max_states, "single_shared_derivation_tree": True, "character_residual_during_derivation": True, "outer_boundary_crossing": "my is matched inside longer acronym and into notes", "explicit_clause_count": 5, "recursive_padding": False, "closure_requires_all_leaves_consumed": True, "independent_complete_reparse": True, "corpus_generation": False}, "provenance": {"generator_sha256": sha256(Path(__file__).read_bytes()).hexdigest(), "grammar_sha256": grammar.digest(), "material": "authored ordinary note-taking clauses and typed map/route/sign/chart frames; no catalogue or known palindrome material"}, "complete_recursive_control": control, **result, "joint_boundary_preflight": boundary_preflight(), "reader_facing_next_operator": "Use the replayed deepest contradiction to alter the exact exposed clause boundary, then rerun the full scheduler.", "scope": "Zero-closure bounded run is diagnostic; no reader-facing palindrome is claimed."}
