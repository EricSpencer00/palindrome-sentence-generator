...
spec=importlib.util.spec_from_file_location('lane',ROOT/'experiments/typed_reversible_clause_composer_20260916.py')
lane=importlib.util.module_from_spec(spec); spec.loader.exec_module(lane)
def test_typed_growth_complete_semantics():
 ...
 assert all(x['complete_clause_parse'] and x['audit']['two_pointer_exact'] is False for x in d['candidates'])
