...
def test_punctuation_center_bridge_has_prose_and_independent_audits():
    ...
    verb_repaired = [r for r in repaired if r["repair_stage"] == "held_out_right_verb_inflection"]
    agreement_repaired = [r for r in repaired if r["repair_stage"] == "held_out_right_agreement_subject"]
    object_det_repaired = [r for r in repaired if r["repair_stage"] == "held_out_right_object_determiner"]
    ...
    locative_det_repaired = [r for r in repaired if r["repair_stage"] == "held_out_right_locative_determiner"]
    assert len(subject_repaired) == 12
    assert len(object_repaired) == 12
    assert len(locative_repaired) == 12
    assert len(verb_repaired) == 12
    assert len(agreement_repaired) == 12
    assert len(object_det_repaired) == 12
    ...
    assert len(locative_det_repaired) == 12
    ...
    assert all(r["held_out_slot"] == "right_verb_inflection" for r in verb_repaired)
    assert all(r["held_out_slot"] == "right_determiner_agreement_subject" for r in agreement_repaired)
    assert all(r["held_out_slot"] == "right_object_determiner" for r in object_det_repaired)
    ...
    assert all(r["held_out_slot"] == "right_locative_determiner" for r in locative_det_repaired)
    assert result["repair_summary"]["preserved_center_event"]
    assert result["repair_summary"]["preserved_punctuation"]
    assert result["repair_summary"]["held_out_slots"] == ["right_subject_np", "right_object_np", "right_locative", "right_verb_inflection", "right_determiner_agreement_subject", "right_object_determiner", "right_object_adjective", "right_object_head_noun", "right_locative_head_noun", "right_locative_modifier", "right_locative_determiner"]
