...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/synchronized-dual-aspect-residuals-20260920.json'
...
def run():
 ...
 return {'experiment_id':'synchronized-dual-aspect-residuals-20260920','method':'two synchronized aspectual residual streams across successive lexical-edge boundaries','stats':{'heads':len(H),'relative_pairs':len(R)*len(Q),'aspect_pairs':len(AS)**2,'sync_checks':syncs,'pruned':pruned,'rendered':len(rows),'exact_gt38':len(ex),'max_letters':rows[0]['audit']['letters']},'exact_candidates':ex,'reader_facing_candidates':[],'diagnostic_controls':rows[:12],'reader_eligible':bool(ex),'novelty_preflight':{'status':'passed','signature':'fresh-authored|dual-aspect-streams|successive-edge-sync|boundary-residuals','distinct_from':'prior single aspect automaton: two independently typed aspect streams synchronize at successive edge boundaries'},'provenance':{'audits':['independent two-pointer comparison','forward/reverse SHA-256'],'hard_exclusions':['nested palindromes','repeated units','mirrored units','word-order symmetry','fragments','catalogue text'],'reader_gate':'closed until exact >38 and blinded intact-vs-shuffled ratings'},'next_construction':'Allow asynchronous aspect transitions with one stream changing state while the other retains its residual.','status':'fresh exact >38 requires reading' if ex else 'no fresh exact >38; synchronized dual-aspect controls retained'}
