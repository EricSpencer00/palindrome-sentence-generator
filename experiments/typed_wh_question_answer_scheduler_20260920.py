"""Typed wh-question/answer complement frames under live scheduling."""
from __future__ import annotations
import hashlib,json,re
from dataclasses import dataclass
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/typed-wh-question-answer-scheduler-20260920.json'; ID='typed-wh-question-answer-scheduler-20260920'
...
def frames():
 ...
 out=[]
 for s in subs:
  for wh in ('who','what','which book'):
   for n in names:
    for v in verbs:
     for o in objs:
      out.append(Frame((wh,s,'asks',n,v,o),('wh','subject','question_verb','complement_subject','verb','object'),'question','transitive'))
      out.append(Frame((s,'answers','that',n,v,o),('subject','answer_verb','complementizer','complement_subject','verb','object'),'answer','transitive'))
 return tuple(out)
def run(state_limit=70000,cap=160):
 ...
 controls=['who the sailor asks the poet reads old letters','a scholar answers that the keeper keeps the lantern','which book the writers ask Diana marks new notes']
 for left in fs:
  for right in fs:
   ...
   while stack and states<state_limit:
    ...
    if li==len(left.parts) and ri<0:
     ...
     if not lb and not rb:
      ...
      if a['exact'] and a['letters']>38: exact.append({'rendered':text,'audit':a,'provenance':{'construction':'typed wh-question/answer scheduler','left_kind':left.kind,'right_kind':right.kind,'valency':left.valency,'left_roles':left.roles,'right_roles':right.roles,'trace':trace,'complete_frames_before_join':True,'endpoint_seed':False,'finished_tape_reversal':False,'post_hoc_repair':False,'catalogue_text':False,'mirrored_token_units':False,'complete_semantic_clauses':True}})
     continue
    moves=[]
    if li<len(left.parts) and ri>=0:
  ...
  if states>=state_limit: break
 return {'experiment_id':ID,'method':'typed wh-question/answer complement frames with unequal live scheduling','complete_wh_frames':len(fs),'stats':{'states':states,'transitions':transitions,'pruned':pruned,'closed':closed,'exact':len(exact),'baseline_exact':1},'exact_candidates':exact,'baseline_exact_controls':base,'complete_prose_controls':[{'rendered':s,'audit':audit(s),'reader_status':'complete wh dialogue control; not exact candidate'} for s in controls],'novelty_preflight':{'status':'passed','signature':'typed-wh-question|answer-valency|unequal-scheduler|live-residual','distinct_from':'whether/that dialogue frames; wh constituent and answer valency are complete before seam scheduling','baseline_is_control_only':True,'finished_tape_reversal':False,'post_hoc_repair':False,'catalogue_text':False,'mirrored_token_units':False},'provenance':{'vocabulary':'authored wh-question and answer complement frames','independent_audit':'two-pointer mismatch plus forward/reverse SHA-256','reader_evidence':False},'status':'no fresh exact >38 closure','next_construction':'add wh recipient questions with typed ditransitive answers','reader_gate':'closed until blinded human ratings'}
if __name__=='__main__':
 x=run(); OUT.write_text(json.dumps(x,indent=2)+'\n'); print(json.dumps({'frames':x['complete_wh_frames'],'stats':x['stats']}))
