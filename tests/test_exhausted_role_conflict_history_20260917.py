...
RUN=Path(__file__).parents[1]/'runs/exhausted-role-conflict-history-20260917.json'
def test_history_and_audits():
 d=json.loads(RUN.read_text());assert d['candidate_count']==2
 for r in d['candidates']:
  assert len(r['conflict_history'])==3 and r['exhausted_roles']
