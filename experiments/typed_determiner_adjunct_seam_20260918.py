...
EXPERIMENT = "typed-determiner-adjunct-seam-20260918"
        ...
        for verb, vn in VERBS:
            ...
            for obj, ot, on in OBJECTS:
                ...
                for prep, required in PREPS:
                    ...
                    for name in NAMES:
                        for det, adjunct in (("the", "at dawn"), ("a", "near noon"), ("those", "by the gate")):
                            if obj == "the lantern" and det == "those": continue
                            yield (subj, verb, prep, obj, name, st, ot, sn, prep, det, adjunct)
...
def discover(policy: str, budget: int = 700) -> dict:
    bank = list(_states()); bank.sort(key=lambda x: (len(letters(" ".join(x[:7]))), x))
    nodes, dead, closures = [], [], []
    for left in bank:
        lt = letters(" ".join(left[:7]))
        for right in bank:
            if len(nodes) >= budget: break
            rt = letters(" ".join(right[:7]))[::-1]
            k = min(len(lt), len(rt))
            if lt[:k] == rt[:k]:
...
def run() -> dict:
    ...
    return {"experiment": EXPERIMENT,
            "method": "Dream-RSI typed determiner/adjunct attachment transducer with reciprocal valency",
            ...
            "novelty_preflight": {"new_geometry": "typed determiner and adjunct attachment changes preserving reciprocal valency and animate/inanimate types",
            ...
            "next_repair": {"operator": "cross-attachment boundary carry with determiner/adjunct seam resegmentation",
                "reason": "fixed attachment geometry prunes all current mirrored prefixes before a complete closure"},
