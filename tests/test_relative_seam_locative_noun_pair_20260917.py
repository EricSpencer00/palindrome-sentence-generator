...
from experiments.relative_seam_locative_noun_pair_20260917 import run,letters
def test_locative_noun_pair_is_bounded_and_audited():
 p=run(); assert p['stats']['rendered']==64; assert p['stats']['exact']==0
 assert {x['pair_type'] for x in p['rendered_candidates']}=={'short_pair','expanded_pair'}
