...
def main() -> None:
    ...
    out = {
        ...
        "candidate_scene": best["audit"]["rendered"],
        "candidates": rows[:3],
        "stats": {"semantic_paths": 1, "lexical_realizations": len(rows), "exact_over_38": len(exact)},
        ...
        "next_repair": {"operator": "replace the first unsatisfied target role on the selected path while retaining event valency, then recompute all edge debts", "reason": "the current path has ordinary semantics but no exact closure above 38 letters; the next useful frontier is held-out synonym substitution at the first open obligation, not word-order reversal"},
    }
