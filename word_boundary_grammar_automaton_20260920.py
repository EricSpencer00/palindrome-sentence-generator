...
def run():
    ...
    return {"experiment_id":ID,"method":"bounded word-boundary grammar automaton with four finite event relations, explicit attachment graph, and typed temporal-until connector edge",
      ...
      "novelty_preflight":{"status":"passed","signature":SIG,"distinct_from":"conditional connector graph: adds a typed temporal-until duration relation without increasing the event count"},
      "next_topology":{"if_no_closure":"add a since/while duration connector edge with explicit attachment and tense state","reason":"current temporal-until connector frame bank has no exact closure above the reader threshold"},
