"""Two-factor lookahead support propagation on the global forward grammar.
...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/two-factor-lookahead-csp-20260920.json'
...
def search(N, cap=8000):
 ...
 return {'target_length':N,'states':states,'represented_forward_language_count':represented,'lookahead_prunes':lookahead_prunes,'prunes':prunes,'complete_renderings':complete,'exact_candidates_above_38':exact,'rendered_candidates':renders[:40]}
def run():
 ...
 return {'experiment_id':'two-factor-lookahead-csp-20260920','method':'global forward factorized lexical CSP with minimum-domain factor branching and adjacent support propagation','differential_test':differential,'results':results,'controls':[{'rendered':x,'audit':audit(x)} for x in controls],'novelty_preflight':{'status':'passed','registry_entries_checked':606,'signature':'two-factor-lookahead|adjacent-lexical-support|global-forward-csp','distinct_from':'position-domain arc consistency: this lane orders each factor lex alternatives by surviving mirrored-support domain and propagates adjacent-factor support before recursion; one forward grammar still assigns x[0:N] with agreement/valency factors'},'provenance':{'independent_audits':['two-pointer scan','forward/reverse SHA-256'],'source_text':'fresh factorized lexical grammar in this script','reader_evidence':False,'reader_gate':'closed until exact >38 and blinded ratings'},'next_construction':{'name':'held-out lexical support domains','operator':'Replace the compact inventory with held-out lexical alternatives and retain the two-factor support intersection; if the same seam bottleneck persists, change grammar topology rather than another lexical sweep.', 'reader_facing_test':'retain only intact complete prose, independently audit every exact closure above 38, then randomize intact prose against word-shuffled controls for blinded ratings'},'status':'diagnostic lane; no exact candidate above 38'}
