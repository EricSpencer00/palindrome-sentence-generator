"""Adjective-boundary seam shift using the same two fixed frames and words."""
from __future__ import annotations
import hashlib,itertools,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/"runs/two-frame-adjective-boundary-seam-20260917.json";REG=ROOT/"docs/experiment-novelty-registry.json"
ID="two-frame-adjective-boundary-seam-20260917";SIG="two-fixed-typed-frames|adjective-boundary-seam-shift|same-lexical-bank|ordinary-grammar|independent-exact-audit"
...
def emit(frame,owner):
 s,v,o,r,rv,ro=frame;first_adj=FIXED['adjective'] if owner=="first" else "";second_adj=FIXED['adjective'] if owner=="second" else "";lo=f"{FIXED['determiner']} {first_adj} {o}".replace("  "," ");ro_text=f"{FIXED['determiner']} {second_adj} {ro}".replace("  "," ");text=f"{s.capitalize()} {v} {lo} {FIXED['adverb']} {FIXED['preposition']} {FIXED['complement']} {FIXED['seam']} {r} {rv} {ro_text} {FIXED['adverb']}.";t=norm(text);mid=len(t)//2;sp=[];cur=0
 ...
 return {"rendered":text,"choices":{"frame":frame,"adjective_owner":owner,"fixed_states":FIXED},"audit":a,"center_state":{"midpoint":mid,"crossing":cross,"closed_pairs_before_first_mismatch":pairs,"live_debt":a["first_mismatches"][0] if a["first_mismatches"] else None,"operator":"adjective-boundary seam shift"},"anti_shortcut_flags":{"finished_tape_reversal":False,"word_order_symmetry":False,"repeated_self_palindromic_unit":False,"catalogue_text":False,"punctuation_changes_letters":False,"fragment":False},"provenance":{"lexical_source":"same two fixed hand-authored frames","borrowed_text":False,"new_lexical_bank":False,"no_clause_or_attachment":True,"ordinary_grammar":True}}
def run():
