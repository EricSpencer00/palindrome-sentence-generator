...
TREES=[
 ...
 {"kind":"causal_result","subject":"the steady nurse","verb":"kept","theme":"the patient","result":"safe inside","cause":"because the alarm worked","attachment_index":"event.kept.result->cause"},
 {"kind":"evidential_result","subject":"the wise judge","verb":"called","theme":"the claim","result":"true enough","cause":"as the records showed","attachment_index":"event.called.result->evidence"},
 {"kind":"inferential_result","subject":"the careful analyst","verb":"marked","theme":"the signal","result":"likely false","cause":"since the data conflicted","attachment_index":"event.marked.result->inference"},
 {"kind":"epistemic_result","subject":"the senior doctor","verb":"judged","theme":"the remedy","result":"safe to use","cause":"after the tests agreed","attachment_index":"event.judged.result->epistemic"},
]
...
def main():
 ...
 temporal=[r for r in rows if r['trees']['left']['kind']=='temporal_result' or r['trees']['right']['kind']=='temporal_result']
 causal=[r for r in rows if r['trees']['left']['kind']=='causal_result' or r['trees']['right']['kind']=='causal_result']
 evidential=[r for r in rows if r['trees']['left']['kind']=='evidential_result' or r['trees']['right']['kind']=='evidential_result']
 inferential=[r for r in rows if r['trees']['left']['kind']=='inferential_result' or r['trees']['right']['kind']=='inferential_result']
 result={'experiment_id':ID,'method':'graph-unification dependency solver with held-out epistemic-result attachment; bilateral mirrored-character support domains intersected during incremental yield','stats':{'trees':len(TREES),'graph_edges':len(rows),'online_support_checks':supports,'online_prunes':prunes,'rendered_controls':len(controls),'exact_gt38':len(exact),'max_letters':max(x['audit']['letters'] for x in rows),'inferential_frontier_survives_past_first_support_conflict':any(r['online_support']['survives_past_first_support_conflict'] for r in inferential),'evidential_frontier_survives_past_first_support_conflict':any(r['online_support']['survives_past_first_support_conflict'] for r in evidential)},'exact_candidates':exact,'reader_facing_candidates':exact,'controls':controls,'novelty_preflight':{'status':'passed','registry_inspected':True,'signature':'fresh-authored|dependency-graph-unification|epistemic-result-event-index|online-character-support','distinct_from':'inferential-result edge and prior dependency-frame, seam, slot, finite-NFA, mirrored-unit, and repair lanes; epistemic result index unifies before lexical yield','forbidden_inputs':['38-letter anchor','finished-tape reversal','catalogue text','mirrored units','word-order symmetry','reward loop']},'provenance':{'audits':['independent two-pointer comparison','independent forward/reverse SHA-256'],'reader_gate':'only exact >=39 candidates may be reader-facing','anti_shortcut_flags':['no mirrored units','no repeated units','no word-order symmetry','no catalogue text','no post-hoc repair'],'reader_status':'closed: no exact >=39 candidate' if not exact else 'pending human reading'},'next_operator':'Pivot to a genuinely different topology: multi-event graph with simultaneous attachment and character constraints; do not add another single event edge.','status':'no exact >=39 closure; intact ordinary-prose controls retained' if not exact else 'fresh exact >=39 requires human reading'}
