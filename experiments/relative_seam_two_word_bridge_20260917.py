"""Bounded two-word bridge across a relative-clause seam."""
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; EXPERIMENT='relative-seam-two-word-bridge-20260917'
...
BRIDGES=[('and then','coordinating'),('so now','causal_temporal')]
 ...
 for si,setup in enumerate(SETUPS):
  for ri,(noun,anaphor,typed_verb) in enumerate(REFERENTS):
   for ci,relative in enumerate(RELATIVES):
    for bi,(bridge,bridge_type) in enumerate(BRIDGES):
     for pi,response in enumerate(RESPONSES):
      text=f'{setup}, {noun}, {relative}; {bridge}, {anaphor} {response}.'
      rows.append({'candidate_id':f's{si}-r{ri}-c{ci}-b{bi}-p{pi}','rendered':text,'regions':['setup','relative_clause','two_word_seam_bridge','response'],'mutable_spans':['referent_number','seam_bridge','anaphoric_response'],'agreement':{'referent':noun,'anaphor':anaphor,'typed_verb':typed_verb},'bridge_type':bridge_type,'audit':audit(text),'provenance':{'construction':'typed_two_word_relative_seam_bridge','catalogue_used':False,'wrapped_seed':False,'finished_tape_reversal':False,'repeated_self_palindromic_unit':False}})
 best=min(rows,key=lambda x:x['audit']['mismatches'])
 return {'experiment':EXPERIMENT,'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'rendered_candidates':rows,'stats':{'rendered':len(rows),'exact':sum(x['audit']['two_pointer_exact'] for x in rows),'longest_letters':max(x['audit']['letters'] for x in rows),'best_mismatches':best['audit']['mismatches']},'novelty_preflight':{'new_geometry':'two-word typed bridge crossing relative-clause boundary','prior_lane_reused':False,'duplicate_sweep':False},'next_repair':{'operator':'co-design bridge polarity and a short complement at the seam','reason':'two-word connectors preserve complete prose but do not close character debt'},'provenance':{'bounded_states':len(rows),'human_readability_certified':False}}
