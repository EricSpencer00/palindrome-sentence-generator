...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/semantic-relation-half-grammar-20260920.json'
 ...
 for subj,verb,obj,adj in RELATIONS:
  for d1,d2,a,p in itertools.product(LEFT,RIGHT,ADJ,PLACE):
   ...
   for w in words:
    ...
    tape=b
   if not ok:continue
   text=' '.join(words)+'.';row={'rendered':text,'audit':audit(text),'provenance':{'semantic_relation':f'{subj}-{verb}-{obj}','relation_chosen_before_lexicalization':True,'word_boundaries_cross_clause_boundary':True,'position_variables':f'x[0:{N}]','finished_tape_reversal_for_generation':False,'posthoc_repair':False,'mirrored_token_units':False,'catalogue_replay':False,'complete_prose':True,'reader_eligible':False}};rows.append(row);complete+=1;exact+=row['audit']['two_pointer_exact'] and row['audit']['letters']>38
