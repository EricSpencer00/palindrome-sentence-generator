...
def main():
 ...
 for i,a in enumerate(CLAUSES):
  for j,b in enumerate(CLAUSES):
   if i==j or set(a.split())&set(b.split()): continue
   left=a+'; '+b; lt=tape(left); right=seg(lt[::-1]); rs=' '.join(right) if right else None
   rows.append({'left':left,'right':rs,'letters':len(lt),'exact':bool(rs and lt==tape(rs)[::-1]),'pointer_check':bool(rs and lt==tape(rs)[::-1]),'left_sha256':hashlib.sha256(lt.encode()).hexdigest(),'right_sha256':hashlib.sha256(tape(rs).encode()).hexdigest() if rs else None,'excluded_catalogue_or_self_unit':False,'repeated_content':False})
 eligible=[r for r in rows if r['letters']>38 and r['right'] and not r['repeated_content']]
