...
def test_v4_method_and_best_evaluation_are_explicitly_diagnostic():
    ...
    assert len(method.json()["method_runs"]) == 31
    ...
    assert method.json()["method_runs"][29]["visited_transitions"] == 159
    assert method.json()["method_runs"][29]["exact_candidates_over_38"] == 0
    assert method.json()["method_runs"][30]["calibration_letters"] == 38
    assert method.json()["method_runs"][30]["long_form_exact_candidates"] == 0
