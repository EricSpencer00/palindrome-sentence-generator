...
from experiments.relative_seam_auxiliary_phrase_20260917 import run,letters
def test_auxiliary_phrase_is_bounded_and_audited():
 p=run(); assert p['stats']['rendered']==64; assert p['stats']['exact']==0
 assert {x['phrase_type'] for x in p['rendered_candidates']}=={'continuative_modal','future_temporal'}
