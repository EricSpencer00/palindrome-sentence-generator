...
\title{Mirror Cost across Segmenters and Language-Model Scales}
...
\begin{document}
% Full-paper snapshot 5; critique and successor changes are in FULL-PAPER-REVISION-LEDGER.md.
\maketitle

\begin{abstract}
Across three segmentation objectives, six span lengths, and several GPT-2 sizes, mirror cost ranges from 2.16 to 3.60 extra bits per letter in 57 retained cells. The reverse penalty is not exclusive to one boundary objective.
\end{abstract}
...
The result now has a segmentation sensitivity check, but GPT-2 checkpoints remain one model family rather than independent architecture replication.

\section*{Limitations}

The study is restricted to WikiText-2, one lexicon, and related GPT-2 models. Coverage and quality need separate treatment.
