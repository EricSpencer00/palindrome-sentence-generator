...
OUT=Path(__file__).parent/'runs/semordnilap-phrase-graph-constructor-20260920.json'
...
def run():
 pairs=json.loads((Path(__file__).parent/'data/mirror_pairs.json').read_text())
 graph=[]
 for p in pairs:
  l=' '.join(p['left']);r=' '.join(p['right'])
  if len(p['left'])==2 and len(p['right'])==2 and len(set(p['left']+p['right']))>2:
   graph.append((l,r))
