"""Diagnostic baseline: typed active/passive and locative dependency realizations."""
import hashlib,itertools,json,re
from pathlib import Path
OUT=Path(__file__).resolve().parents[1]/'runs/dependency-attachment-reset-20260920.json'
...
def boundary_obligation(left,right):
 # Diagnostic seam measurement after independent clause selection; this lane
 # intentionally does not claim pre-render pruning.
...
def run():
 ...
 for l,r in itertools.product(ACTIVE+LOC,PASSIVE+LOC):
  ...
  rows.append({'rendered':rendered,'left_dependency':l[3],'right_dependency':r[3],'attachment_obligation':boundary_obligation(left,right),'audit':a,'complete_prose':True,'provenance':{'independent_active_passive_or_locative_selection':True,'attachment_checked_during_generation':False,'attachment_diagnostic_after_selection':True,'finished_tape_reversal':False,'post_hoc_repair':False,'catalogue_text':False,'mirrored_token_units':False,'repeated_units':False,'fragment':False}})
 ...
 r={'experiment_id':'dependency-attachment-reset-20260920','method':'diagnostic baseline of typed active/passive and locative dependency realizations; attachment seams measured after selection','stats':{'left_frames':len(ACTIVE)+len(LOC),'right_frames':len(PASSIVE)+len(LOC),'rendered_candidates':len(rows),'fresh_exact_gt38':len(exact),'max_letters':max(x['audit']['letters'] for x in rows)},'rendered_candidates':rows,'exact_candidates':exact,'novelty_preflight':{'status':'passed','signature':'active-passive-locative|attachment-seam-diagnostic|typed-dependency-reset','distinct_from':'prior dependency CSP and valency equations: alternates semantic realization modes at attachment seam','finished_tape_reversal':False,'post_hoc_repair':False,'catalogue_text':False,'fragments':False},'provenance':{'audits':['independent pointer mismatch','forward/reverse SHA-256'],'next_reader_test':'blind naturalness ratings on top three complete candidates versus shuffled controls'},'status':'diagnostic baseline; no pre-render pruning and no method advance'}
 return r
