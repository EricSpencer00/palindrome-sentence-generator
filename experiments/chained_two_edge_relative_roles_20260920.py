...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/chained-two-edge-relative-roles-20260920.json'
...
def run():
 ...
 return {'experiment_id':'chained-two-edge-relative-roles-20260920','method':'two successive lexical-edge pairs with alternating relative semantic roles','stats':{'heads':len(H),'relative_pairs':len(R)*len(Q),'edge_pair_chains':len(E)*(len(E)-1)//2,'rendered':len(rows),'exact_gt38':len(ex),'max_letters':rows[0]['audit']['letters']},'exact_candidates':ex,'reader_facing_candidates':[],'diagnostic_controls':rows[:12],'reader_eligible':bool(ex),'novelty_preflight':{'status':'passed','signature':'fresh-authored|two-edge-chain|alternating-relative-roles|complete-prose','distinct_from':'prior one-edge synchronization: two successive semordnilap edge pairs alternate agent/theme roles in one independently authored scene'},'provenance':{'audits':['independent two-pointer comparison','forward/reverse SHA-256'],'hard_exclusions':['nested palindromes','repeated units','mirrored units','word-order symmetry','fragments','catalogue text'],'reader_gate':'closed until exact >38 and blinded intact-vs-shuffled ratings'},'next_construction':'Add a third edge event with a typed tense transition and preserve alternating role obligations.','status':'fresh exact >38 requires reading' if ex else 'no fresh exact >38; chained readable controls retained'}
