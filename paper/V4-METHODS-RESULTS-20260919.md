...
The inchoative modal-passive lane introduced a typed `begin-to-be-given`
inchoative state before the modal passive beneficiary/agent/theme frame. Across
target lengths 44, 56, 68, and 80 it visited 1,550 states and made 496 live
factor prunes, with zero complete parses and zero exact candidates above 38.
Its two controls are complete ordinary-English sentences and were independently
pointer/SHA audited; the next construction is resultative modal passive.
Artifact: `runs/inchoative-modal-passive-csp-20260920.json`.

The typed agreement scene author generated 320 complete scenes with
singular/plural subject-verb agreement enforced before rendering. The longest
intact diagnostic is “The reader holds a quiet harbor near the quay.” (37
letters); zero exact candidates above 38 survived. The next construction adds
typed transitive and locative alternations while preserving number and tense
unification. Artifact: `runs/agreement-scene-author-20260920.json`.

The four-event attachment-graph lane extended the word-boundary grammar to a
fourth finite event with a distinct attachment index and tense state. Four
frames incurred 294,912 live boundary prunes and retained 20 grammatical
near-miss diagnostics to 163 letters, including “The patient gardener who
records the note records the patient diaper that the scribe records and that
the gardener guards and the courier notes the date and the witness checks the
seal before dusk.”; zero exact candidates above 38 survived. The next
construction is a resultative or causal event relation. Artifact:
`runs/word-boundary-fourth-event-graph-20260920.json`.

The resultative modal-passive lane introduced a typed `remain-being` state
before the modal passive beneficiary/agent/theme frame. Across target lengths
44, 56, 68, and 80 it visited 1,234 states and made 436 live factor prunes,
with zero complete parses and zero exact candidates above 38. Its complete
controls are ordinary English and were independently pointer/SHA audited; the
next construction is stative modal passive. Artifact:
`runs/resultative-modal-passive-csp-20260920.json`.

The typed transitive/locative agreement author generated 80 complete scenes
with subject number, tense, and valency unified before rendering. The longest
intact diagnostic is “Several young cartographers carried the narrow stone
landing.” (53 letters); zero exact candidates above 38 survived. The next
construction adds typed relative-clause attachment. Artifact:
`runs/typed-transitive-locative-author-20260920.json`.

The causal/resultative connector graph added a typed connector edge (“because
the record matters” plus a resultative alternative) to the four-event
attachment graph. Four frames incurred 884,736 live boundary prunes and
retained 20 ordinary-prose near-miss diagnostics to 186 letters, including
“The patient gardener who records the note records the patient diaper that the
scribe records and that the gardener guards and the courier notes the date and
the witness checks the seal before dusk because the record matters.”; zero
...
The iterative modal-passive lane introduced a typed `again-being` iterative
state before the modal passive beneficiary/agent/theme frame. Across target
lengths 44, 56, 68, and 80 it visited 1,278 states and made 460 live factor
prunes, with zero complete parses and zero exact candidates above 38. The next
construction is frequentative modal passive. Artifact:
`runs/iterative-modal-passive-csp-20260920.json`.

The indexed relative residual trie carried endpoint residual classes through
two indexed relative attachments, filtering subject-number agreement before
emission. It tested 81 transitions, pruned 72 endpoints, and retained 81
scenes to 92 letters; zero exact candidates above 38 survived. The best scene
remains attachment-awkward, so the next construction carries multi-character
residual buffers. Artifact:
`runs/indexed-relative-residual-trie-20260920.json`.

The temporal-until connector graph added a typed duration relation to the
conditional attachment graph. Four frames incurred 1,769,472 live boundary
prunes and retained 20 ordinary-prose near-miss diagnostics to 186 letters;
zero exact candidates above 38 survived. The next construction adds a
...
The asynchronous three-way typed join advanced separate agent, theme, and
attachment residual streams through 108 character steps. It retained 36
broad-English controls to 97 letters, but no exact candidates above 38. The
next construction adds cross-stream synchronization events. Artifact:
`runs/async-threeway-typed-join-20260920.json`.

The possibility modal-passive lane introduced a distinct `perhaps-being` scope
state before the typed beneficiary/agent/theme frame. Across target lengths
44, 56, 68, and 80 it visited 1,192 states and made 420 live factor prunes,
with zero complete parses and zero exact candidates above 38. The next
construction is probabilistic modal passive. Artifact:
`runs/possibility-modal-passive-csp-20260920.json`.

The possibility modal-scope connector graph added a typed `might if` relation
with explicit tense/agreement state. Four frames incurred 4,128,768 live
boundary prunes and produced zero exact candidates above 38; the next
construction adds probabilistic modal scope. Artifact:
`runs/word-boundary-modal-scope-graph-20260920.json`.

The cross-stream boundary synchronization lane added explicit leader events
while two other residual streams retained their obligations. It retained 36
complete prose controls through 108 synchronization events, to 97 letters,
with zero exact candidates above 38. The next construction chains alternating
leaders with typed tense transitions. Artifact:
`runs/crossstream-boundary-sync-20260920.json`.

The probabilistic modal-passive lane introduced a distinct `likely-being`
scope state before the typed beneficiary/agent/theme frame. Across target
lengths 44, 56, 68, and 80 it visited 1,234 states and made 436 live factor
prunes, with zero complete parses and zero exact candidates above 38. The next
construction is stochastic modal passive. Artifact:
`runs/probabilistic-modal-passive-csp-20260920.json`.

The probabilistic modal-scope connector graph added a typed `probably when`
relation with probabilistic scope and explicit attachment/tense state. Four
...
The cadence-compatible tense-adjunct branch carried held-out subject/object
role agreement into temporal adjunct slots. Its bounded depth-three run
retained four vivid diagnostics up to 121 letters after eight seam prunes, but
zero exact candidates above 38; the next construction adds aspect-compatible
adjunct states. Artifact:
`runs/cadence-tense-adjunct-growth-20260920.json`.

The incremental valency edge trie carried boundary residuals before complete
clause rendering and retained 15,360 intact ordinary-clause controls up to 53
letters. It produced zero exact candidates above 38; its next construction
synchronizes two tries through a shared relative-attachment state. Artifact:
`runs/valency-edge-trie-boundary-shift-20260920.json`.

The dual-relative antecedent graph bound two independent relative attachments
to explicit antecedent indices across 72 complete scene renderings up to 108
letters, with zero exact candidates above 38. Because exactness is a
prerequisite, the artifact's reader-facing list is empty and its surfaces are
...
The cadence-compatible quotation branch added held-out quotation-source frames
while preserving discourse, evidential, polarity, modal, and tense
compatibility, role agreement, and PP attachment. It retained four vivid
diagnostics up to 121 letters after eight seam prunes, but zero exact
candidates above 38; the next construction adds reported-speech frames.
Artifact: `runs/cadence-quotation-adjunct-growth-20260920.json`.

The two-adjunct scope branch added typed adjuncts with attachment agreement and
scope states under the strict reader gate. It retained 36 paired controls up to
161 letters, with 216 synchronous buffer pairs and zero exact candidates above
38. The next construction adds scope nesting. Artifact:
`runs/synchronized-valency-tries-two-adjunct-scope-20260920.json`.

The synchronized dual-aspect branch carried two independently typed aspect
streams across 288 lexical-edge checks and boundary prunes. It retained
...
The number-aware width-two semantic lattice added singular/plural agreement
states and carried a two-character residual across subject, verb, object, and
setting slots. It explored 36 scene states and retained diagnostics up to 58
letters, with zero exact candidates above 38. The next construction carries
asynchronous width-two buffers through typed adjunct choices. Artifact:
`runs/semantic-lattice-width2-scene-author-20260920.json`.

The clause-valency alternation CSP added held-out ditransitive/intransitive
valency and agreement states to independent full-clause factorization. Its
...
above 38 was zero. The next construction adds benefactive and locative valency
frames. Artifact: `runs/clause-valency-alternation-csp-20260920.json`.

The two-event shared-scene matcher added a second independently typed event to
the semantic graph and matched both SVO clauses online. It checked 243 online
prunes and retained 20 complete non-repeated controls up to 53 letters, with
...
The semantic-frame clause topology added held-out frame alternations with
agreement state to independent full-clause factorization. Its bounded run
visited 5,000 states, pruned 4,499, and reached no complete two-sided parse;
the longest intact control was 108 letters and the exact count above 38 was
zero. The next construction adds frame-conditioned tense and voice. Artifact:
`runs/semantic-frame-clause-csp-20260920.json`.

The intransitive locative cross-role lane added explicit adjacent, sheltered,
and path relation metadata to 27 transitions. It retained complete scene
diagnostics up to 65 letters, with zero exact candidates above 38. The next
construction unifies transitive theme and intransitive location alternatives.
Artifact: `runs/typed-locative-crossrole-relations-20260920.json`.

The evidential/polarity-scope lane added two evidential and two polarity states
to the modal/entailment-scoped graph. It checked 4,860 online prunes and
retained 20 complete controls up to 91 letters, with zero exact candidates
...
The evidential-conditioned clause topology added held-out evidential/polarity
alternations with agreement state to independent full-clause factorization. Its
bounded run visited 5,000 states, pruned 4,499, and reached no complete
two-sided parse; the longest intact control was 108 letters and the exact count
above 38 was zero. The next construction adds discourse/evidential
alternations. Artifact:
`runs/evidential-conditioned-clause-csp-20260920.json`.

The dual relation-specific adjunct lane delayed closure until two adjunct
obligations were consumed. It checked 168 residual steps across 12 scenes,
retaining diagnostics up to 87 letters with zero exact candidates above 38. The
next construction mixes valency adjunct pairs with role-labeled residuals.
Artifact: `runs/dual-relation-adjunct-delayed-closure-20260920.json`.

The quoted-speaker/embedded-tense lane added speaker-shift and quoted-tense
states inside quotation islands. It checked 186,624 online prunes and retained
20 complete controls up to 147 letters, with zero exact candidates above 38.
The next construction adds quotation polarity and attitude predicates with
scope agreement. Artifact:
`runs/shared-scene-quoted-speaker-tense-20260920.json`.

The discourse-conditioned clause topology added held-out discourse/evidential
alternations with agreement state to independent full-clause factorization. Its
bounded run visited 5,000 states, pruned 4,499, and reached no complete
two-sided parse; the longest intact control was 108 letters and the exact count
above 38 was zero. The next construction adds quotation/discourse
alternations. Artifact: `runs/discourse-conditioned-clause-csp-20260920.json`.

The mixed-valency role-residual lane paired transitive-theme and
intransitive-location adjuncts while carrying agent, event, theme/location,
and adjunct labels through delayed residuals. It rendered 72 controls, checked
1,008 residual steps, closed zero, and found zero exact candidates above 38;
the longest diagnostic was 87 letters. Because several controls reuse lexical
units, the artifact marks them diagnostic-only and sends none to readers. The
next construction uses role-compatible residual transitions with explicit
valency edges. Artifact:
`runs/mixed-valency-role-residuals-20260920.json`.

The quotation-attitude lane added attitude predicates and hard polarity-scope
agreement to the quotation-island graph. It checked 233,280 online prunes and
retained 20 complete controls up to 151 letters, with zero exact candidates
