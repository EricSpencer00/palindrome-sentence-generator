...
def run():
    ...
    return {"experiment_id":ID,"method":"bounded word-boundary grammar automaton with four finite event relations, explicit attachment graph, and typed conditional connector edge",
      ...
      "novelty_preflight":{"status":"passed","signature":SIG,"distinct_from":"concessive connector graph: adds a typed conditional relation without increasing the event count"},
      "next_topology":{"if_no_closure":"add a temporal-until connector edge with explicit attachment and tense state","reason":"current conditional connector frame bank has no exact closure above the reader threshold"},
