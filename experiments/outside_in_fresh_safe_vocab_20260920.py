...
def main():
 ...
 result={"experiment_id":EXPERIMENT_ID,"method":"outside-in WordTries beam overhang with whole-sentence POS plan","config":{"seeds":4,"vocabulary_size":base["vocabulary_size"],"min_zipf":4.0,"beam":32,"min_letters":30,"max_steps":96},"stats":{"records":len(rows),"mechanically_admitted":sum(not x["rejection_codes"] for x in rows),"longest_letters":max((x["audit"]["letters"] for x in rows),default=0),"exact":sum(x["audit"]["two_pointer_exact"] for x in rows)},"best_rendered_tapes":rows[:10],"complete_prose_controls":["The careful writer reads a quiet book.","A kind teacher helps the young student."],"independent_audit":["two-pointer normalized tape","forward/reverse SHA-256"],"novelty_preflight":{"status":"passed","signature":EXPERIMENT_ID,"distinct_from":"char-cfg-broad-earley-20260920","fresh_safe_vocabulary":True,"catalogue_imported":False,"post_hoc_repair":False},"provenance":{"generator":"experiments/outside_in_sentence_decoder.py","vocabulary_sha256":base["vocabulary_sha256"],"reader_gate":"all outputs are diagnostics until blinded human evaluation"},"next_construction":"Use the best intact overhang tape as a seed for a semantic-valency paired chart, without mutating a finished tape."}
