...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/cadence-evidential-adjunct-growth-20260920.json'
...
def run():
 ...
 return {'experiment_id':'cadence-evidential-adjunct-growth-20260920','method':'cadence-compatible subject/object grammar with active/passive voice and aspect-compatible adjunct growth with held-out role agreement with multiple clause/adjunct slots and live character obligations','results':[r],'controls':[{'rendered':x,'audit':audit(x)} for x in controls],'novelty_preflight':{'status':'passed','registry_entries_checked':637,'signature':'cadence-evidential-adjunct-growth|certainty-source-state|live-character-obligations','distinct_from':'typed SVO/PP scheduler lanes and fixed clause pairs: uses cadence-compatible subject/object alternations with held-out evidential certainty frames and polarity/modal/tense agreement; no copied catalogue, repeated units, mirrored units, repair, or finished reversal'},'provenance':{'independent_audits':['two-pointer scan','forward/reverse SHA-256'],'source_text':'fresh authored poetic scene grammar','reader_evidence':False,'reader_gate':'closed until exact >38 and blinded ratings'},'next_construction':{'name':'cadence discourse adjunct topology','operator':'Add held-out discourse-source frames with evidential-compatible temporal adjuncts; preflight signature first.','reader_facing_test':'retain intact vivid prose only, independently audit exact closures above 38, then blinded intact-vs-shuffled ratings'},'status':'diagnostic lane; no exact candidate above 38'}
