...
def main():
 witness='Stressed desserts.'; near='The tired baker served stressed desserts, then repaired a drawer while a quiet traveler delivered bread to the river.'
 rows=[]
 for kind,s in [('witness',witness),('near_miss',near)]:
  t=tape(s); rows.append({'kind':kind,'text':s,'letters':len(t),'exact':audit(s)['exact'],'audit':audit(s),'forward_hash':hashlib.sha256(t.encode()).hexdigest(),'reverse_hash':hashlib.sha256(t[::-1].encode()).hexdigest(),'admitted':False})
