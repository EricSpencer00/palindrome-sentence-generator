...
def run():
 ...
 out={'experiment_id':'typed-slot-reverse-trie-repair-20260917','status':'quarantined_no_reader_candidate','candidates':sorted(rows,key=lambda x:x['trie_prefix_depth'],reverse=True)[:20],'stats':{'base_frames':len(VARIANTS),'typed_variants':len(items),'inflectional_verb_forms':sum(len(VERBS[x[4]]) for x in VARIANTS),'rendered_near_misses':len(rows),'exact':sum(x['audit']['exact'] for x in rows),'live_prefix_only':True,'full_two_sided_closure_checked':False},'provenance':{'source':'fresh typed phrase variants plus number-conditioned verb lattice','catalogue_imported':False,'seed_used_as_scaffold':False,'finished_tape_reversal':False,'independent_audits':['two-pointer','SHA-256 forward/reverse']},'failure_and_repair':{'next_repair':'replace prefix probes with a full two-sided chart closure, then add typed object/adjunct substitutions'}}
