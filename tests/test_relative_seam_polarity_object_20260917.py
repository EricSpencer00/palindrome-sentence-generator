...
from experiments.relative_seam_polarity_object_20260917 import run,letters
def test_polarity_object_lane_is_bounded_and_audited():
 p=run(); assert p['stats']['rendered']==32; assert p['stats']['exact']==0
 assert {x['pair_type'] for x in p['rendered_candidates']}=={'positive_map','negative_key'}
