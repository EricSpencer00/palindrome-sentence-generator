"""Bounded three-way agreement + locative preposition repair."""
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; EXPERIMENT='relative-seam-threeway-prep-agreement-20260918'
...
CHOICES=[('who marks the harbor map','she','records','a map','by','the inlet','singular_by'),('who carry the cedar key','they','carry','maps','near','our shores','plural_near')]
 ...
 for si,setup in enumerate(SETUPS):
  for ri,(noun,number) in enumerate(REFERENTS):
   for ci,(relative,anaphor,verb,obj,prep,place,kind) in enumerate(CHOICES):
    ...
    rows.append({'candidate_id':f's{si}-r{ri}-c{ci}','rendered':text,'regions':['setup','relative_subject_agreement','fixed_bridge','anaphor_agreement','locative_number_preposition'],'mutable_spans':['relative_subject_number','anaphor_number','locative_number_preposition'],'agreement':{'referent':noun,'referent_number':number,'relative_number':kind,'anaphor':anaphor,'locative_number':kind,'preposition':prep},'frame_type':kind,'bridge_fixed':BRIDGE,'audit':audit(text),'provenance':{'construction':'threeway_agreement_locative_preposition','catalogue_used':False,'wrapped_seed':False,'finished_tape_reversal':False,'repeated_self_palindromic_unit':False}})
 best=min(rows,key=lambda x:x['audit']['mismatches'])
 return {'experiment':EXPERIMENT,'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'rendered_candidates':rows,'stats':{'rendered':len(rows),'exact':sum(x['audit']['two_pointer_exact'] for x in rows),'longest_letters':max(x['audit']['letters'] for x in rows),'best_mismatches':best['audit']['mismatches']},'novelty_preflight':{'new_geometry':'three-way agreement jointly chooses locative preposition','prior_lane_reused':False,'duplicate_sweep':False},'next_repair':{'operator':'couple three-way agreement with locative determiner and noun while preserving preposition','reason':'preposition coupling preserves complete prose but leaves character debt'},'provenance':{'bounded_states':len(rows),'human_readability_certified':False}}
