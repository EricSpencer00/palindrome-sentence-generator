"""Valency attachment topology over independent full-clause factors.
...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/valency-attachment-topology-csp-20260920.json'
...
def search(cap=5000):
 ...
 while stack and states<cap:
  ...
  if slot==4:
   ...
   text=' '.join(lw)+'; '+' '.join(rw)+'.';row={'rendered':text,'audit':audit(text),'provenance':{'grammar':'Clause -> Subject Verb [Object] [Recipient] [Locative]','slot':slot,'left_features':lf,'valency_state':'benefactive_or_locative','right_independent':True,'live_word_boundary_obligation':{'left':lr,'right':rr},'finished_tape_reversal':False,'posthoc_repair':False,'mirrored_token_units':False,'catalogue_replay':False,'complete_prose':True,'reader_eligible':False}};rows.append(row);complete+=1;exact+=row['audit']['two_pointer_exact'] and row['audit']['letters']>38;continue
...
def run():
 ...
 return {'experiment_id':'valency-attachment-topology-csp-20260920','method':'factorized independent full-clause grammar coupled by live word-boundary character obligations','results':[r],'controls':[{'rendered':x,'audit':audit(x)} for x in controls],'novelty_preflight':{'status':'passed','registry_entries_checked':643,'signature':'valency-attachment-topology|benefactive-locative-state|word-boundary-obligation','distinct_from':'cadence adjunct growth, fixed clause pairs, endpoint envelopes, and center/bridge lanes: both full clauses are composed independently from subject/verb/object/locative factors while boundary characters are coupled online; no reversal, repeated units, repair, or catalogue text'},'provenance':{'independent_audits':['two-pointer scan','forward/reverse SHA-256'],'source_text':'fresh authored factorized clause grammar','reader_evidence':False,'reader_gate':'closed until exact >38 and blinded ratings'},'next_construction':{'name':'attachment-conditioned clause topology','operator':'Add held-out instrument and source attachment frames with agreement state; preflight a new signature first.','reader_facing_test':'retain complete vivid prose, independently audit exact closures above 38, then blinded intact-vs-shuffled ratings'},'status':'diagnostic lane; no exact candidate above 38'}
