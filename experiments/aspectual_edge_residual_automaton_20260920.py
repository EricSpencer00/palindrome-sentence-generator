...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/aspectual-edge-residual-automaton-20260920.json'
...
def run():
 ...
 return {'experiment_id':'aspectual-edge-residual-automaton-20260920','method':'live lexical-edge residual automaton carrying simple/progressive/perfect aspect states','stats':{'heads':len(H),'relative_pairs':len(R)*len(Q),'aspect_states':len(AS),'transitions':transitions,'pruned':pruned,'rendered':len(rows),'exact_gt38':len(ex),'max_letters':rows[0]['audit']['letters']},'exact_candidates':ex,'reader_facing_candidates':[],'diagnostic_controls':rows[:12],'reader_eligible':bool(ex),'novelty_preflight':{'status':'passed','signature':'fresh-authored|aspectual-automaton|live-edge-residual|boundary-compatibility','distinct_from':'prior aspect metadata: aspect is now a live automaton state controlling lexical-edge boundary acceptance'},'provenance':{'audits':['independent two-pointer comparison','forward/reverse SHA-256'],'hard_exclusions':['nested palindromes','repeated units','mirrored units','word-order symmetry','fragments','catalogue text'],'reader_gate':'closed until exact >38 and blinded intact-vs-shuffled ratings'},'next_construction':'Carry two residual streams simultaneously and synchronize their aspect transitions at successive edge boundaries.','status':'fresh exact >38 requires reading' if ex else 'no fresh exact >38; aspectual automaton prose diagnostics retained'}
