"""Bounded relative-attachment/locative-semantic complement repair."""
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; EXPERIMENT='relative-seam-attachment-locative-semantics-20260918'
...
CHOICES=[('who marks the harbor map','she','records','a map','by','the inlet','maritime_short'),('that the guide carried the cedar key','the keepers','carry','maps','near','the road','terrestrial_short')]
 ...
 for si,setup in enumerate(SETUPS):
  for ri,(noun,number) in enumerate(REFERENTS):
   for ci,(relative,anaphor,verb,obj,prep,place,kind) in enumerate(CHOICES):
    text=f'{setup}, {noun}, {relative}; {BRIDGE} {TENSE}, {anaphor} {verb} {obj} {prep} the {place}.'
    rows.append({'candidate_id':f's{si}-r{ri}-c{ci}','rendered':text,'regions':['setup','relative_attachment','fixed_bridge','anaphor_lexical_class','locative_semantic_complement'],'mutable_spans':['relative_attachment','locative_semantics'],'agreement':{'referent':noun,'referent_number':number,'relative_attachment':kind,'anaphor':anaphor,'preposition':prep,'determiner':'the'},'frame_type':kind,'bridge_fixed':BRIDGE,'audit':audit(text),'provenance':{'construction':'relative_attachment_locative_semantics_fixed_length','catalogue_used':False,'wrapped_seed':False,'finished_tape_reversal':False,'repeated_self_palindromic_unit':False}})
 best=min(rows,key=lambda x:x['audit']['mismatches'])
 return {'experiment':EXPERIMENT,'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'rendered_candidates':rows,'stats':{'rendered':len(rows),'exact':sum(x['audit']['two_pointer_exact'] for x in rows),'longest_letters':max(x['audit']['letters'] for x in rows),'best_mismatches':best['audit']['mismatches']},'novelty_preflight':{'new_geometry':'relative attachment jointly selects semantically distinct locative complement','prior_lane_reused':False,'duplicate_sweep':False},'next_repair':{'operator':'couple semantic complement with locative preposition while preserving phrase length','reason':'semantic coupling preserves complete prose but leaves character debt'},'provenance':{'bounded_states':len(rows),'human_readability_certified':False}}
