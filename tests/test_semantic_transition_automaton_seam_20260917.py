...
RUN=Path(__file__).parents[1]/'runs/semantic-transition-automaton-seam-20260917.json'
def test_automaton_rows_and_audits():
 d=json.loads(RUN.read_text());assert d['candidate_count']==len(d['candidates'])>0
 for r in d['candidates']:
  assert r['transition'] in {'cooperative','contrastive'} and r['precompletion_seam_matches']>=0
