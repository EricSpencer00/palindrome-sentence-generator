...
\title{Exact Palindromes as Double Lexicalization}
...
\begin{document}
% Full-paper snapshot 2; critique and successor changes are in FULL-PAPER-REVISION-LEDGER.md.
\maketitle

\begin{abstract}
Exact palindromes make a single normalized letter stream support a forward lexical reading and a reverse lexical reading with potentially different word boundaries. We recast the decoder as a case study of this double-lexicalization problem rather than treating length as the main result.
\end{abstract}
...
This version defines an NLP object of study, but the argument remains conceptual. The next version must measure reversal in ordinary English.

\section*{Limitations}

No controlled forward/reverse measurement exists yet, and output length remains a poor proxy for language quality.
