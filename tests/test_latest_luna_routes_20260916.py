...
def test_parallel_readability_report_is_diagnostic_and_keeps_provenance():
    ...
    assert report["candidate_count"] == 4297
    ...
    assert len(report["route_summary"]) == 104
...
def test_aggregate_includes_nested_rendered_candidate_formats():
    ...
    assert by_source["runs/live-slot-equation-cfg-resegmentation-20260916.json"]["rows"] == 4
    assert by_source["runs/clause-growth-frame-repair-20260916.json"]["rows"] == 5
