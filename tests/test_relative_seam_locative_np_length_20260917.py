...
from experiments.relative_seam_locative_np_length_20260917 import run,letters
def test_locative_np_length_is_bounded_and_audited():
 p=run(); assert p['stats']['rendered']==32; assert p['stats']['exact']==0
 assert {x['length_type'] for x in p['rendered_candidates']}=={'short_np','expanded_np'}
