...
OUT = ROOT / "runs/word-boundary-concessive-graph-20260920.json"
ID = "word-boundary-concessive-graph-20260920"
SIG = "word-boundary-aware|concessive-relation|connector-edge|live-equations"
...
RELATION_EDGES = (("", "temporal"), (" because the record matters", "causal"), (" so the garden opens", "resultative"), (" although the path narrows", "concessive"))
...
def run():
    ...
    return {"experiment_id":ID,"method":"bounded word-boundary grammar automaton with four finite event relations, explicit attachment graph, and causal/resultative connector edge",
      ...
      "novelty_preflight":{"status":"passed","signature":SIG,"distinct_from":"fourth-event attachment graph: adds a typed causal/resultative connector edge that changes event semantics without increasing the event count"},
      "next_topology":{"if_no_closure":"add a concessive connector edge with explicit attachment and tense state","reason":"current causal/resultative connector frame bank has no exact closure above the reader threshold"},
