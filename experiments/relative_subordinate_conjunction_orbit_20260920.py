"""Relative-clause-in-subject construction over a central conjunction."""
...
OUT = ROOT / "runs" / "relative-subordinate-conjunction-orbit-20260920.json"
EXPERIMENT_ID = "relative-subordinate-conjunction-orbit-20260920"
...
def fs(role: str, clause: str, *texts: str, number: str | None = None,
       agreement: str | None = None, tense: str | None = None) -> tuple[Frame, ...]:
    return tuple(Frame(role, text, clause, number, agreement, tense) for text in texts)


def build_lattice() -> tuple[tuple[Frame, ...], ...]:
    # Rendered order: main S-V-O while subordinate S-[relative]-V-O.
    # Pairing order lists right-side slots from the outside inward.
    return (
        fs("subject", "main", "the steward", "a raven", "the poet", number="singular"),
        fs("event", "main", "keeps the vow", "marks the page", "kept the vow", agreement="singular", tense="present"),
        fs("object", "main", "a silver cup", "the quiet bell", "one true word"),
        fs("subordinator", "bridge", "while", "though", "when"),
        fs("object", "subordinate", "the winter seal", "a hidden key", "one pale crown"),
        fs("event", "subordinate", "guard the gate", "name the heir", "guarded the gate", agreement="plural", tense="present"),
        fs("relative_event", "relative", "who waits", "that sings", "who remembers", agreement="plural", tense="present"),
...
def run(*, state_limit: int = 250_000) -> dict[str, object]:
        ...
        if lo > hi:
            ...
            if checked["exact"]:
                candidates.append({"rendered": rendered, "audit": checked,
                    "provenance": {"construction": "relative clause inside subordinate subject",
        ...
        for lframe in lattice[lo]:
            for rframe in lattice[hi]:
                ...
                if rframe.role == "event" and rframe.agreement != next_state.get("sub_number"):
                    feature_pruned += 1
                    continue
                if rframe.role == "relative_event" and rframe.agreement != next_state.get("sub_number"):
                    feature_pruned += 1
                    continue
    ...
    result = {"experiment": EXPERIMENT_ID,
              "method": "relative clause inside subordinate subject with central conjunction",
              ...
              "provenance": {"complete_main_clause": True, "complete_subordinate_clause": True,
                  "embedded_relative_clause": True, "central_conjunction": True,
                  "agreement_state_before_emission": True,
                  "held_out_subordinate_lexicalizations": True,
                  "finished_tape_reversal": False, "post_hoc_repair": False,
                  "catalogue_text": False, "aligned_token_mirror": False,
                  "next_construction": "add a second held-out relative attachment site and compare valency-conditioned seams"}}
