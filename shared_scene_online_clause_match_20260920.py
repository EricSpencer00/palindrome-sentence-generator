...
ROOT=Path(__file__).resolve().parent; OUT=ROOT/'runs/shared-scene-source-negation-20260920.json'
ID='shared-scene-source-negation-20260920'; SIG='shared-scene-graph|speaker-attribution|negation-scope|online-match'
...
def run():
 rows=[]; prunes=0
 for (scene,sv,so),(s,sr),(v,vr),(o,orr),(rs,rv,ro),(t1,t2),(conn,scope),(modal,mscope),(evidence,escope),(polarity,pscope),(source,sscope),(neg,nscope) in itertools.product(SCENES,SUBJ,VERBS,OBJS, itertools.product(RIGHT_SUBJ,RIGHT_VERBS,RIGHT_OBJS),TENSES,CAUSAL,MODAL,EVIDENCE,POLARITY,SOURCES,NEGATION):
 ...
 return {'experiment_id':ID,'method':'shared scene graph adds speaker attribution and negation scope over evidential/polarity-gated events','stats':{'scene_frames':len(SCENES),'subject_edges':len(SUBJ),'verb_edges':len(VERBS),'object_edges':len(OBJS),'second_event_edges':len(RIGHT_SUBJ)*len(RIGHT_VERBS)*len(RIGHT_OBJS),'tense_orders':len(TENSES),'connector_states':len(CAUSAL),'modal_states':len(MODAL),'evidential_states':len(EVIDENCE),'polarity_states':len(POLARITY),'source_states':len(SOURCES),'negation_states':len(NEGATION),'online_prunes':prunes,'rendered_controls':len(rows),'fresh_exact_gt38':len(exact),'max_letters':max((r['audit']['letters'] for r in rows),default=0)},'rendered_candidates':rows,'exact_candidates':exact,'novelty_preflight':{'status':'passed','signature':SIG,'distinct_from':'evidential/polarity lane: adds source attribution and negation attachment'},'next_topology':'add quotation/reporting scope and negative-polarity agreement','status':'fresh exact >38 candidate requires human reading' if exact else 'no exact >38 closure; complete scene controls retained'}
