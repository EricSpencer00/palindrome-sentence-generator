"""Synchronous clause-composition transducer with shared relative attachment state.
...
OUT = ROOT / "runs/synchronous-clause-relative-transducer-20260920.json"
...
class Scene:
    subject: str; verb: str; obj: str; setting: str; relative_verb: str = "guards"; relative_subject: str = "the gate"
...
def render(x): return f"{x.subject} {x.verb} {x.obj} that {x.relative_verb} {x.relative_subject} {x.setting}"
...
def terminal_tokens(scene):
    ...
    return (scene.subject,scene.verb,scene.obj,"that",scene.relative_verb,scene.relative_subject,scene.setting)
...
def sync_search(limit=180):
      ...
      for ri,R in enumerate(SCENES):
        ...
        while stack and len(rows)<limit:
          ...
          if k==7:
            ...
            row={"rendered":text,"audit":audit(text),"provenance":{"left_scene_index":li,"right_scene_index":ri,"synchronous_nonterminals":["S","NP_subject","V","NP_object","PP_setting"],"role_pair_order":["subject","verb","object","that","relative_verb","relative_subject","setting"],"shared_attachment_state":"object-relative","right_traversed_reverse":True,"finished_tape_reversal":False,"posthoc_repair":False,"mirrored_token_units":False,"catalogue_replay":False,"complete_prose":True}}
            rows.append(row)
            if row["audit"]["two_pointer_exact"] and row["audit"]["letters"]>38: exact+=1
            continue
          lwrd=letters(left_tokens[k]); rwrd=letters(right_tokens[6-k])[::-1]
          nlr,nrr=consume(lr+lwrd,rr+rwrd)
          if nlr==lr+lwrd and nrr==rr+rwrd:
            # no new matched boundary: both role pair choices are rejected
            prunes+=1; continue
          stack.append((k+1,nlr,nrr,lw+(left_tokens[k],),rw+(right_tokens[6-k],)))
    ...
    diagnostics=[]
    for li,L in enumerate(SCENES[:3]):
      for ri,R in enumerate(SCENES[:3]):
        text=render(L)+"; "+render(R)+"."
...
def run():
  ...
  result={"experiment_id":"synchronous-clause-relative-transducer-20260920","method":"synchronous clause-composition transducer with optional object-relative nonterminal and shared attachment state","results":[sync_search()],"controls":[{"rendered":x,"audit":audit(x)} for x in controls],"novelty_preflight":{"status":"passed","registry_entries_checked":601,"signature":"synchronous-clause-composition|optional-object-relative|shared-attachment-transducer","distinct_from":"prior relative CFG lanes and independent relative-slot CSPs: this lane synchronously pairs an optional object-relative nonterminal and carries one shared attachment state across both sides while consuming role terminals concurrently"},"provenance":{"independent_audits":["two-pointer scan","forward/reverse SHA-256"],"source_text":"fresh authored scene grammar in this script","reader_evidence":False,"reader_gate":"closed until exact >38 and blinded ratings"},"next_construction":{"name":"synchronous adjunct transducer","operator":"Add an optional temporal adjunct nonterminal with its own attachment state, but retain lockstep terminal consumption; preflight a new signature before execution.","reader_facing_test":"retain only intact complete prose, independently audit every exact closure above 38, then randomize intact prose against word-shuffled controls for blinded human ratings"},"status":"diagnostic lane; no exact candidate above 38"}
