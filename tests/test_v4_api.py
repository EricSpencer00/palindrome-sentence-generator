"""v4 serves one palindrome. These tests are about what "one palindrome" means.
...
def bank():
    v4.ensure_loaded()
    assert not v4._load_error, v4._load_error
    return v4._bank
...
def test_served_text_is_a_palindrome_across_many_draws(client):
    ...
    for seed in range(60):
        r = client.get(f"/api/v4/palindrome?seed={seed}")
...
def test_reported_letter_count_matches(client):
    body = client.get("/api/v4/palindrome?seed=7").json()
...
def test_no_junk_short_words_are_served(client):
    ...
    for seed in range(40):
        body = client.get(f"/api/v4/palindrome?seed={seed}").json()
...
def test_real_words_rejects_what_it_should():
    assert v4.real_words(["level", "madam"])
    assert not v4.real_words(["bn", "level"])      # junk two-letter
    assert not v4.real_words(["utc"])              # in frequency lists, not English
    assert not v4.real_words(["ab3"])              # not alphabetic
...
def test_seed_fixes_the_choice(client):
    a = client.get("/api/v4/palindrome?seed=99").json()
    b = client.get("/api/v4/palindrome?seed=99").json()
    assert a["plain"] == b["plain"]


def test_different_seeds_reach_different_palindromes(client):
    seen = {client.get(f"/api/v4/palindrome?seed={s}").json()["plain"]
            for s in range(30)}
...
def test_novel_is_the_default_and_excludes_the_catalogue(client):
    ...
    got = {client.get(f"/api/v4/palindrome?seed={s}&novel=false").json()["source"]
           for s in range(60)}
...
def test_growth_is_off_by_default(client):
    body = client.get("/api/v4/palindrome?seed=3").json()
    assert body["grown"] is False
    assert body["operations"] == []


def test_growth_lengthens_and_still_mirrors(client):
    plain = client.get("/api/v4/palindrome?seed=3").json()
    grown = client.get("/api/v4/palindrome?seed=3&grow=3").json()
...
def test_the_response_says_growth_reads_worse(client):
    ...
    body = client.get("/api/v4/palindrome?seed=1").json()
...
def test_health_reports_the_bank(client):
    body = client.get("/api/v4/health").json()
...
def test_bank_file_and_loaded_bank_agree(bank):
    raw = json.loads(Path(v4.BANK_PATH).read_text())
...
def test_composition_is_a_palindrome_at_every_length(client, letters):
    r = client.get(f"/api/v4/composition?seed=4&letters={letters}")
    assert r.status_code == 200, r.text
    body = r.json()
...
def test_no_chunk_ever_repeats(client, letters):
    ...
    """
    body = client.get(f"/api/v4/composition?seed=4&letters={letters}").json()
...
def test_no_repeats_across_many_seeds(client):
    for seed in range(12):
        body = client.get(
            f"/api/v4/composition?seed={seed}&letters=2000").json()
...
def test_left_and_right_chunks_mirror_pairwise(client):
    ...
    body = client.get("/api/v4/composition?seed=6&letters=1200").json()
...
def test_length_slider_tracks_the_request(client):
    ...
    for want in (200, 800, 2400, 6000):
        body = client.get(f"/api/v4/composition?seed=2&letters={want}").json()
...
def test_capacity_is_reported_and_is_the_ceiling(client):
    cap = client.get("/api/v4/health").json()["capacity"]["novel"]["max_letters"]
    body = client.get(f"/api/v4/composition?seed=2&letters={cap + 5000}").json()
...
def test_chops_caps_the_pair_count(client):
    body = client.get(
        "/api/v4/composition?seed=2&letters=4000&chops=15").json()
    assert body["pairs"] <= 15


def test_longest_first_uses_fewer_pairs_for_the_same_length(client):
    a = client.get("/api/v4/composition?seed=2&letters=1200").json()
    b = client.get(
        "/api/v4/composition?seed=2&letters=1200&longest_first=true").json()
...
def test_every_word_in_a_composition_is_a_real_word(client):
    from llm_palindrome.lexicon import is_real_word
    lex = v4.lexicon()
    body = client.get("/api/v4/composition?seed=8&letters=2000").json()
    for word in body["plain"].split():
...
def test_no_degenerate_pair_is_used(client):
    ...
    body = client.get("/api/v4/composition?seed=3&letters=14000").json()
...
def test_novel_default_excludes_the_catalogue(client):
    body = client.get("/api/v4/composition?seed=5&letters=1200").json()
