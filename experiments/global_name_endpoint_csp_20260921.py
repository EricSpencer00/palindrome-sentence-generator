...
SLOTS = ("d1", "name1", "v1", "d2", "obj1", "conj", "d3", "name2", "v2", "obj2")
CATS = ("det", "name", "v", "det", "obj", "conj", "det", "name", "v", "obj")
    ...
    def target_search(target):
        ...
        def rec(li,ri,lo,ro):
            ...
            if li>ri:
                ...
                if len({words[1],words[7]})==2: found.append(words)
                return
...
def run(limit=50000):
    ...
    return {"experiment_id":"global-name-endpoint-csp-20260921","method":"bounded global variable-span CSP with typed ordinary proper-name endpoints and joint determiner/name/verb/object role choices","config":{"letter_band":[39,60],"state_limit":limit,"name_category":"ordinary non-self-palindromic lexical entries"},**x,"records":records,"independent_exact_shortcut_audits":True,"novelty_preflight":{"status":"passed","signature":"global-variable-span|typed-proper-name-endpoint|joint-role-mirror-csp","signature_collision":False},"queue_row":{"lane":"Astra","status":"satisfying assignments" if records else "bounded residual","next":"retain typed names while widening only grammar-supported valency frames"}}
