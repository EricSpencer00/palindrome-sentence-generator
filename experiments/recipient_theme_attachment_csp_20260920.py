"""Recipient/theme attachment topology over independent full-clause factors.
...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/recipient-theme-attachment-csp-20260920.json'
...
def search(cap=5000):
 ...
 while stack and states<cap:
  ...
  if slot==4:
   ...
   text=' '.join(lw)+'; '+' '.join(rw)+'.';row={'rendered':text,'audit':audit(text),'provenance':{'grammar':'Clause -> Subject Verb [Object] [Recipient] [Locative]','slot':slot,'left_features':lf,'valency_state':'recipient_or_theme','right_independent':True,'live_word_boundary_obligation':{'left':lr,'right':rr},'finished_tape_reversal':False,'posthoc_repair':False,'mirrored_token_units':False,'catalogue_replay':False,'complete_prose':True,'reader_eligible':False}};rows.append(row);complete+=1;exact+=row['audit']['two_pointer_exact'] and row['audit']['letters']>38;continue
