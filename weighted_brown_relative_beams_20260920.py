...
LEX={'det':[('the',.8),('a',.6)],'subj':[('gardener',.7),('teacher',.8)],'verb':[('walks',.7),('opens',.6)],'relverb':[('works',.7),('lives',.6)],'place':[('garden',.8),('school',.7)]}
...
def run():
 beams=[('the gardener who works in the garden walks',.1),('a teacher that lives near the school opens',.1)]; right=[('the school stays quiet',.8),('a garden holds flowers',.7)]; transitions=0
 complete=[]; fragments=[]
 for text,w in beams:
