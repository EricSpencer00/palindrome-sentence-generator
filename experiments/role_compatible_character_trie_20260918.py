...
def run():
 ...
 return {'experiment':ID,'rendered_candidates':rows,'stats':{'rendered':len(rows),'sibling_branches':len({r['sibling_branch_id'] for r in rows}),'exact':sum(r['audit']['two_pointer_exact'] for r in rows),'two_pair_survivors':sum(all(r['two_character_obligations'].values()) for r in rows),'longest_letters':max((r['audit']['letters'] for r in rows),default=0),'best_mismatches':best['audit']['mismatches'] if best else None},'next_repair':'Replay sibling branches with a held-out role alternative and compare residual trajectories before deployment.','reader_gate':'closed; programmatic diagnostics do not certify readability','provenance':{'independent_audits':['two-pointer','forward/reverse SHA-256'],'novelty_preflight':'live role trie before rendering','replay_world_branching':True}}
