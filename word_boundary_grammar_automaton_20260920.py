...
def run():
    ...
    return {"experiment_id":ID,"method":"bounded word-boundary grammar automaton with explicit theme-headed attachment graph, coordinated object-relative indices, and tense state",
      ...
      "next_topology":{"if_no_closure":"add a third event relation with an explicit attachment graph and tense state","reason":"current attachment-graph frame bank has no exact closure above the reader threshold"},
