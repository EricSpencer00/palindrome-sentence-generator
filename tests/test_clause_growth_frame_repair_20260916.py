...
def test_clause_growth_has_real_prose_and_independent_rejection():
    ...
    assert x["stats"] == {"candidates": 6, "exact": 0, "admitted": 0}
    assert [len(c["frames"]) for c in x["candidates"]] == [1, 2, 3, 2, 4, 5]
    ...
    assert x["candidates"][-2]["provenance"]["repair_from"] == "growth-3-clause"
    assert x["candidates"][-1]["provenance"]["repair_from"] == "frontier-selected-fourth-clause"
