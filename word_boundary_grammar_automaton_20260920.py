...
OUT = ROOT / "runs/word-boundary-conditional-graph-20260920.json"
ID = "word-boundary-conditional-graph-20260920"
SIG = "word-boundary-aware|conditional-relation|connector-edge|live-equations"
...
RELATION_EDGES = (("", "temporal"), (" because the record matters", "causal"), (" so the garden opens", "resultative"), (" although the path narrows", "concessive"), (" if the seal remains", "conditional"))
...
def run():
    ...
    return {"experiment_id":ID,"method":"bounded word-boundary grammar automaton with four finite event relations, explicit attachment graph, and typed concessive connector edge",
      ...
      "novelty_preflight":{"status":"passed","signature":SIG,"distinct_from":"causal/resultative connector graph: adds a typed concessive contrast relation without increasing the event count"},
      "next_topology":{"if_no_closure":"add a conditional connector edge with explicit attachment and tense state","reason":"current concessive connector frame bank has no exact closure above the reader threshold"},
