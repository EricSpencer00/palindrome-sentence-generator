...
RIGHT='The evening watch reads the charts and thanks Nora after the harbor crew secures the boats.'
def audit():
...
def run():
 r=audit();return {'experiment_id':ID,'novelty_preflight':{'catalogue_imported':False,'known_palindromes_imported':False,'collision':False},'rendered_candidates':[r],'stats':{'letters':r['letters'],'exact':r['exact']},'equation_solver':{'slots':['agent','speech_act','object','time','result'],'coupling':'paired semantic substitutions against live mirrored character debt','agreement':'third-person singular present','content_words_all_different':True},'next_repair':'jointly replace speech_act/object spans at first mismatch and replay both clauses','provenance':{'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'source':'fresh human-authored discourse','audits':['two-pointer','forward/reverse hash','mechanical gate']}}
