...
def test_fresh_abca_topology():
 ...
 assert r['semantic_pattern']==['A','B','C','A']
 assert r['roles']==['harvest','transit','weaving','harvest']
...
def test_live_exact_and_novelty_audits():
 d=m.run(); assert len({r['audit']['sha256_forward'] for r in d['rendered_outputs']})==2; a=d['rendered_outputs'][0]['audit']
 ...
 assert d['repair_attempts'][0]['provenance']['repair_pass'].startswith('outer A-only')
