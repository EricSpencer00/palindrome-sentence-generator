...
def decode(avoid=None):
    avoid = set(avoid or ())
    text=''; trace=[]
    for state, choices in SLOTS:
        ranked=sorted(choices,key=lambda x:(score(text+' '+x),x),reverse=True)
        # Keep the second clause independently lexicalized; a repeated clause
        # is a diagnostic failure, not a construction shortcut.
        choice=next((item for item in ranked if item not in avoid), ranked[0])
        avoid.add(choice); text += (' ' if text else '') + choice
...
def main():
    left,trace=decode(); right,_=decode({item['chosen'] for item in trace}) ; rendered=left+' '+right
    lt,rt=tape(left),tape(right); p=pointer(lt,rt)
    normalized=tape(rendered)
    forward=hashlib.sha256(normalized.encode()).hexdigest(); reverse=hashlib.sha256(normalized[::-1].encode()).hexdigest()
    hashes={'left':hashlib.sha256(lt.encode()).hexdigest(),'right':hashlib.sha256(rt.encode()).hexdigest(),'rendered':forward,'reverse_rendered':reverse,'equal':bool(normalized) and forward==reverse}
    checks=mechanical_admission_checks(rendered, min_letters=39, max_letters=1000)
    out={'method':'character_lm_grammar_constrained_decode_v1','candidates':[{'text':rendered,'letters':len(normalized),'exact':p['equal'],'admitted':all(checks.values()),'pointer_audit':p,'hash_audit':hashes,'mechanical_checks':checks}],
