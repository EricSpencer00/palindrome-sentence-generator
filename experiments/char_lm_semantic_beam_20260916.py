...
def main():
 ...
 for i,(subj,verb,obj,adv) in enumerate(FRAMES):
  for j,(subj2,verb2,obj2,adv2) in enumerate(FRAMES):
   ...
   au=audit(s); rows.append({'id':f'beam-{i}-{j}','rendered':s,'length':len(norm(s)),'state':{'semantic_frame_left':i,'semantic_frame_right':j,'beam_width':8,'boundary_debt':'propagated at every emitted character'},'audit':au,'provenance':{'method':'character beam over typed semantic frames; lexical next-character choices are scored before clause completion','source_sentences_copied':False,'catalogue_imported':False,'borrowed_text':False,'reversed_finished_sentence':False,'word_order_symmetry':False,'repeated_self_palindromic_unit':False,'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'source_sha256':h(s)},'anti_shortcut':{'intact_prose':True,'word_order_only':False,'repeated_unit':False,'catalogue_text':False,'self_palindromic_words':au['self_palindromic_words']==[],'repeated_word_count':au['repeated_word_count']},'next_repair':'Use the first residual pointer as a hard beam feature: expand only typed synonym or adjunct alternatives whose next character equals the opposing debt, then preserve frame agreement.'})
