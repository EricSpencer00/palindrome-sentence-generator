...
ROOT=Path(__file__).resolve().parent;OUT=ROOT/'runs/role-typed-three-tile-composition-20260921.json'
...
def run():
 ...
 for path in itertools.permutations(TILES,3):
  ...
  gates={'role_compatible':compatible,'whole_output_exact':au['exact'],'readable_control':True,'content_disjoint':len(content)==len(set(content)),'no_self_pal_units':all(norm(w)!=norm(w)[::-1] for w in content)}
