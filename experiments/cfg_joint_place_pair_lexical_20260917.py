...
SIG='joint-locative-state|place-pair-lexical-alternation|dual-subject-pairs|complementizer-preposition-fixed|live-character-frontier|independent-pointer-sha'
...
def row(t,pair,rep,pr):return {'rendered':t,'attachment_state':'joint-locative','place_pair':pair,'repaired':rep,'provenance':pr,'novelty_preflight':{'signature':SIG,'subject_pairs_fixed':True,'complementizer_preposition_fixed':True,'not_catalogue_replay':True},'audit':a(t),'live_frontier':f(t),'anti_shortcut':{'single_tree':True,'place_lexical_only':True,'word_order_only':False,'repeated_unit':False,'catalogue_source':False,'fragment':False}}
def main():
 ...
 for comp in ('where','in which'):
  for m1,m2,v,n,r1,r2,lv,p,x,y in itertools.islice(itertools.product(S,S,V,O,S,S,LV,P,O,O),128):
   ...
   for t,pair,rep,pr in ((make(comp,m1,m2,v,n,r1,r2,lv,p,x,y),f'{x}/{y}',False,'fresh joint place pair'),(make(comp,m1,m2,v,n,r1,r2,lv,p,x,alt),f'{x}/{alt}',True,'held-out joint place-pair lexical alternation')):
 ...
 out={'experiment':'cfg-joint-place-pair-lexical-20260917','method':'joint place-pair lexical alternation over fixed subject and attachment state','signature':SIG,'control_count':controls,'repair_count':repairs,'candidate_count':len(rows),'exact_count':len(ex),'candidates':rows[:60],'next_repair':'Add a held-out joint preposition lexical alternation while preserving both place pairs and selected complementizer.'}
 p=Path('runs/cfg-joint-place-pair-lexical-20260917.json');p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'run':str(p),'controls':controls,'repairs':repairs,'candidates':len(rows),'exact':len(ex),'longest':rows[0]['audit']['letters']}));print(rows[0]['rendered'])
