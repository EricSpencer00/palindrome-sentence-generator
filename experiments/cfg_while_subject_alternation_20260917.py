...
SIG='typed-temporal-CFG|while-connective|formerly-later-order|subject-alternation|present-to-past|independent-pointer-sha'
...
def main():
 ...
 for s,rv,o,s2,pv in itertools.product(SUBJ,PRES,OBJ,SUBJ,PAST):
  ...
  rows.append({'rendered':t,'provenance':'held-out while subject alternation with distinct typed event subjects','grammar_state':{'connective':'while','subject_relation':'alternating','tense_sequence':'present->past','temporal_order':'formerly<later'},'live_frontier':f(t),'audit':a(t),'anti_shortcut':{'single_tree':True,'subject_alternation_explicit':True,'word_order_only':False,'repeated_unit':False,'catalogue_source':False,'fragment':False},'novelty_preflight':{'signature':SIG,'distinct_from_shared_subject_lane':True,'tense_consistent':True}})
 rows.sort(key=lambda x:(-x['audit']['letters'],x['rendered']));ex=[x for x in rows if x['audit']['two_pointer']]
 out={'experiment':'cfg-while-subject-alternation-20260917','method':'while formerly/later CFG with alternating typed subjects and fixed present-to-past order','signature':SIG,'candidate_count':len(rows),'pruned_partial_count':pruned,'exact_count':len(ex),'candidates':rows[:80],'next_repair':'Add a held-out alternating object frame, preserving subject alternation and tense order while rejecting role-inconsistent transitions.'}
 p=Path('runs/cfg-while-subject-alternation-20260917.json');p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'run':str(p),'candidates':len(rows),'pruned':pruned,'exact':len(ex),'longest':rows[0]['audit']['letters']}));print(rows[0]['rendered'])
