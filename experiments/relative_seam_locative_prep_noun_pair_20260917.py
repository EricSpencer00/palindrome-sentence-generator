"""Bounded locative preposition+noun pair repair at fixed phrase length."""
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; EXPERIMENT='relative-seam-locative-prep-noun-pair-20260917'
...
PAIRS=[('can still','a map','by the inlet','near an island','short_prep_pair'),('cannot yet','maps','near the old docks','along our wide shore','expanded_prep_pair')]
 ...
 for si,setup in enumerate(SETUPS):
  for ri,(noun,anaphor,typed_verb) in enumerate(REFERENTS):
   for ci,relative in enumerate(RELATIVES):
    for xi,(polarity,obj,loc_a,loc_b,kind) in enumerate(PAIRS):
     for li,loc in enumerate((loc_a,loc_b)):
      text=f'{setup}, {noun}, {relative}; {BRIDGE} {polarity}, {anaphor} record {obj} {loc}.'
      rows.append({'candidate_id':f's{si}-r{ri}-c{ci}-x{xi}-l{li}','rendered':text,'regions':['setup','relative_clause','fixed_bridge','polarity_object','fixed_length_prep_noun_pair'],'mutable_spans':['referent_number','locative_preposition_noun_pair'],'agreement':{'referent':noun,'anaphor':anaphor,'typed_verb':typed_verb},'pair_type':kind,'bridge_fixed':BRIDGE,'audit':audit(text),'provenance':{'construction':'fixed_length_locative_preposition_noun_pair','catalogue_used':False,'wrapped_seed':False,'finished_tape_reversal':False,'repeated_self_palindromic_unit':False}})
 best=min(rows,key=lambda x:x['audit']['mismatches'])
 return {'experiment':EXPERIMENT,'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'rendered_candidates':rows,'stats':{'rendered':len(rows),'exact':sum(x['audit']['two_pointer_exact'] for x in rows),'longest_letters':max(x['audit']['letters'] for x in rows),'best_mismatches':best['audit']['mismatches']},'novelty_preflight':{'new_geometry':'locative preposition+noun pair jointly varied at fixed phrase length','prior_lane_reused':False,'duplicate_sweep':False},'next_repair':{'operator':'couple object determiner with locative preposition+noun pair while preserving length','reason':'preposition+noun coupling preserves complete prose but leaves character debt'},'provenance':{'bounded_states':len(rows),'human_readability_certified':False}}
