...
DET=['a','an','the']; NOUN=['aide','artist','baker','pilot','clerk','guard']; VERB=['rips','marks','opens','reads','packs','seals','carries','carried']; NUM=['one','nine','two']; OBJ=['memo','map','note','gate','parcel','memos','maps','notes','gates','parcels']; ADJ=['near','by','at','in','under','over'];
LEX=set(DET+NOUN+VERB+NUM+OBJ+ADJ+['some','men','nora','ada'])
...
for d,n,v,num,obj in itertools.product(DET,NOUN,VERB,NUM,OBJ):
 ...
 right=' '.join(toks)
 if not (toks[0] in DET and toks[1] in NOUN and toks[2] in VERB and any(x in OBJ for x in toks[3:])): continue
