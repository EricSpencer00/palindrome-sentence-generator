...
def main():
 ...
 for s in SEEDS:
  ...
  rows.append({'left':s,'right_resegmented':rt,'lm_score':round(score(s),6),'exact':bool(rev and tape(s)==tape(rt)[::-1]),'left_tape':tape(s),'right_tape':tape(rt) if rt else None})
