...
HISTORICAL_RUNS = (
    ROOT / "runs" / "polar-boundary-slot-repair-20260918-v2.json",
    ROOT / "runs" / "dream-rsi-discourse-ellipsis-20260918.json",
)
EDGES=(
...
def prior_exact_tapes() -> set[str]:
    """Collect exact tapes already recorded by earlier experiment runs."""
    tapes: set[str] = set()
    for path in HISTORICAL_RUNS:
        if not path.exists():
            continue
        try:
            raw = path.read_text()
            ...
            continue
        # A regex scan avoids reparsing the many large historical run files;
        # both keys are emitted by the experiment ledger for exact candidates.
        for match in re.finditer(r'"(?:normalized|rendered)"\s*:\s*"((?:\\.|[^"\\])*)"', raw):
            try:
...
def run():
 rows=[]; nodes=0; prior=prior_exact_tapes()
  ...
  for s in EDGES[1][1]:
   for w in EDGES[2][1]:
    ...
    if a["two_pointer_exact"]:
     g=mechanical_admission_checks(text,local_catalogue=prior,min_letters=30,max_letters=260)
 ...
 exact_collisions=sum(x["provenance"]["prior_tape_collision"] for x in rows)
 return {"experiment_id":EXPERIMENT_ID,"method":"authored scene lattice with live character equations","stats":{"nodes":nodes,"exact":len(rows),"novel_exact":len(rows)-exact_collisions,"prior_exact_collisions":exact_collisions,"mechanically_admitted":sum(x["mechanically_admitted"] for x in rows),"longest_exact_letters":max((x["audit"]["letters"] for x in rows),default=0)},"candidates":sorted(rows,key=lambda x:-x["audit"]["letters"]),"independent_audit":["two-pointer character comparison","SHA-256 forward/reverse"],"novelty_preflight":{"status":"blocked" if exact_collisions else "passed","prior_run_tapes_scanned":len(prior),"exact_collisions":exact_collisions,"no_posthoc_reversal":True,"no_catalogue_import":True},"next_repair":"replace the witness edge with fresh authored grammatical reverse-compatible clauses, then require a blinded reader gate"}
