...
SIG='joint-locative-state|preposition-lexical-alternation|place-pairs-fixed|subject-pairs-fixed|complementizer-fixed|independent-pointer-sha'
...
def row(t,p,rep,pr):return {'rendered':t,'attachment_state':'joint-locative','preposition':p,'repaired':rep,'provenance':pr,'novelty_preflight':{'signature':SIG,'place_pairs_subject_pairs_fixed':True,'complementizer_fixed':True,'not_catalogue_replay':True},'audit':a(t),'live_frontier':f(t),'anti_shortcut':{'single_tree':True,'preposition_only':True,'word_order_only':False,'repeated_unit':False,'catalogue_source':False,'fragment':False}}
def main():
 ...
 for comp in ('where','in which'):
  for m1,m2,v,n,r1,r2,lv,p,x,y in itertools.islice(itertools.product(S,S,V,O,S,S,LV,P,O,O),128):
   ...
   for t,pp,rep,pr in ((make(comp,m1,m2,v,n,r1,r2,lv,p,x,y),p,False,'fresh joint preposition realization'),(make(comp,m1,m2,v,n,r1,r2,lv,alt,x,y),alt,True,'held-out joint preposition lexical alternation')):
 ...
 out={'experiment':'cfg-joint-preposition-lexical-20260917','method':'joint preposition lexical alternation over fixed subject/place pairs','signature':SIG,'control_count':controls,'repair_count':repairs,'candidate_count':len(rows),'exact_count':len(ex),'candidates':rows[:60],'next_repair':'Add a joint matrix/locative predicate pair substitution while preserving preposition, complementizer, and place roles.'}
 p=Path('runs/cfg-joint-preposition-lexical-20260917.json');p.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({'run':str(p),'controls':controls,'repairs':repairs,'candidates':len(rows),'exact':len(ex),'longest':rows[0]['audit']['letters']}));print(rows[0]['rendered'])
