...
def records(roles):
 ...
 for ws in itertools.product(*(INV[x] for x in roles)):
  if len(set(w for x,w in zip(roles,ws) if x not in {'det','prep'}))<3: continue
  out.append(' '.join(ws))
 return out
