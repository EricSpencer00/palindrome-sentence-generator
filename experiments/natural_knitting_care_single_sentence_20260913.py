"""Frame-first, no-repeated-content construction for a natural textile sentence.

The complete event frame is selected before search: fine knitting keeps hands
warm while makers quietly repair garments for winter travelers.  Only then is
the non-palindromic word-internal ``knit|ting`` pivot derived.  A single
expanded grammar owns every leaf, and the live frontier emits and replays one
character at a time before central admission.
"""
...
LEXICON = (
    Lexeme("fine", "adj_center", "knitting"), Lexeme("knitting", "noun_center", "knitting"), Lexeme("keeps", "verb_intro", "warmth"),
    Lexeme("patient", "adj_target", "hands"), Lexeme("weary", "adj_target", "hands"), Lexeme("hands", "noun_target", "hands"), Lexeme("warm", "adj_complement", "warmth"),
    Lexeme("while", "conj", "contrast"), Lexeme("careful", "adj_person", "maker"), Lexeme("skilled", "adj_person", "maker"), Lexeme("makers", "noun_person", "maker"), Lexeme("quietly", "adv_manner", "manner"),
    Lexeme("repair", "verb_record", "repair"), Lexeme("delicate", "adj_artifact", "garment"), Lexeme("garments", "noun_artifact", "garment"),
    Lexeme("for", "prep", "beneficiary"), Lexeme("winter", "adj_event", "traveler"), Lexeme("travelers", "noun_event", "traveler"), Lexeme("today", "adv", "temporal"),
)
...
class Grammar:
    def productions(self, s: Symbol) -> tuple[tuple[Symbol, ...], ...]:
        if s.name == "S":
            return ((T("adj_center", "center_adj"), T("noun_center", "center_word"), T("verb_intro", "predicate"), T("adj_target", "target_adj"), T("noun_target", "target_noun"), T("adj_complement", "complement"), T("conj", "subordinator"), T("adj_person", "second_adj"), T("noun_person", "second_noun"), T("adv_manner", "manner"), T("verb_record", "second_predicate"), T("adj_artifact", "object_adj"), T("noun_artifact", "object_noun"), T("prep", "beneficiary_prep"), T("adj_event", "beneficiary_adj"), T("noun_event", "beneficiary_noun"), T("adv", "today")),)
        return ()
...
FRAME = {"center": "knitting", "pivot": "knit|ting", "predicate": "keeps", "center_adjective": "fine", "target_adjectives": {"patient", "weary"}, "target_noun": "hands", "complement": "warm", "subordinator": "while", "second_subject": {"careful", "skilled"}, "second_noun": "makers", "manner": "quietly", "second_predicate": "repair", "object": ("delicate", "garments"), "beneficiary": ("for", "winter", "travelers"), "today": "today"}


def semantic_choice(slot: Slot, item: Lexeme) -> bool:
    allowed = {
        "center_adj": {FRAME["center_adjective"]}, "center_word": {FRAME["center"]}, "predicate": {FRAME["predicate"]},
        "target_adj": FRAME["target_adjectives"], "target_noun": {FRAME["target_noun"]}, "complement": {FRAME["complement"]},
        "subordinator": {FRAME["subordinator"]}, "second_adj": FRAME["second_subject"], "second_noun": {FRAME["second_noun"]},
        "manner": {FRAME["manner"]}, "second_predicate": {FRAME["second_predicate"]}, "object_adj": {FRAME["object"][0]}, "object_noun": {FRAME["object"][1]},
        "beneficiary_prep": {FRAME["beneficiary"][0]}, "beneficiary_adj": {FRAME["beneficiary"][1]}, "beneficiary_noun": {FRAME["beneficiary"][2]}, "today": {FRAME["today"]},
    }
...
def run(*, state_limit: int = 100_000, closure_limit: int = 100) -> dict[str, object]:
    ...
    frames = []
    for target_adj in ("patient", "weary"):
        for maker_adj in ("careful", "skilled"):
            frames.append({"rendered": f"Fine knitting keeps {target_adj} hands warm while {maker_adj} makers quietly repair delicate garments for winter travelers today.", "center": FRAME["center"], "pivot": FRAME["pivot"], "predicate": FRAME["predicate"], "semantic_status": "complete ordinary textile-care event fixed before center search"})
