...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/constructive-qa-polarity-debt-20260921.json'
...
ANSWERS=[
 ...
 {'subject':'the sailors','aux':'have','verb':'charted','object':'the path','tense':'present','agreement':'plural','aspect':'completed','polarity':'negative'},
 {'subject':'the keeper','aux':'had','verb':'secured','object':'one lantern','tense':'past','agreement':'singular','aspect':'completed','polarity':'negative'},
 {'subject':'the scouts','aux':'had','verb':'watched','object':'a star','tense':'past','agreement':'plural','aspect':'completed','polarity':'affirmative'},]
...
def run():
 ...
 for q,a in itertools.product(QUESTIONS,ANSWERS):
  ...
  compatible=q['tense']==a['tense'] and q['agreement']==a['agreement'] and q['aspect']==a['aspect'] and q.get('polarity','affirmative')==a.get('polarity','affirmative')
  gates={'online_debt_closed':closed,'whole_output_exact':au['exact'],'tense_compatible':compatible,'agreement_compatible':q['agreement']==a['agreement'],'aspect_compatible':q['aspect']==a['aspect'],'polarity_compatible':q.get('polarity','affirmative')==a.get('polarity','affirmative') and q.get('polarity','affirmative')==a.get('polarity','affirmative'),'distinct_clause_vocab':not(set(content_words(qt)) & set(content_words(at))),'no_self_palindromic_units':all(len(norm(w)) < 2 or norm(w)!=norm(w)[::-1] for w in words(qt+' '+at))}
  rows.append({'rendered':rendered,'question_frame':q,'answer_frame':a,'debt_equation':eq,'audit':au,'gates':gates,'accepted':all(gates.values()),'provenance':{'construction':'four hand-authored typed tense/agreement frames per side','selected_online_against_opposing_character_debt':True,'feature_state':{'question_tense':q['tense'],'answer_tense':a['tense'],'question_agreement':q['agreement'],'answer_agreement':a['agreement'],'question_aspect':q['aspect'],'answer_aspect':a['aspect'],'question_polarity':q.get('polarity','affirmative'),'answer_polarity':a.get('polarity','affirmative')},'lexical_cartesian_expansion':False,'finished_tape_reversal':False,'borrowed_catalogue_text':False,'posthoc_repair':False}})
 exact=[r for r in rows if r['accepted']]
 return {'experiment_id':'constructive-qa-polarity-debt-20260921','method':'typed Q/A debt with tense, agreement, and progressive/completed aspect and affirmative/negative polarity state','stats':{'question_frames':len(QUESTIONS),'answer_frames':len(ANSWERS),'pairs':len(rows),'feature_compatible':sum(r['gates']['tense_compatible'] and r['gates']['agreement_compatible'] and r['gates']['aspect_compatible'] and r['gates']['polarity_compatible'] for r in rows),'online_closed':sum(r['debt_equation']['closed'] for r in rows),'accepted_exact':len(exact),'accepted_exact_gt38':sum(r['audit']['letters']>38 for r in exact)},'exact_candidates':exact,'rendered_controls':rows,'novelty_preflight':{'status':'passed','signature':'typed-qa|tense-agreement-state|online-debt','signature_collision':False,'distinct_from':'lexical Cartesian Q/A debt and duplicated-middle lane'},'next_operator':'Add evidential modality as one further typed state, retaining four-frame bounded search.','status':'fresh exact closure found' if exact else 'no fresh exact closure; feature-compatible controls retained'}
