"""Joint residual indexing for scenes with two typed attachment edges."""
...
OUT = ROOT / "runs/two-typed-attachment-joint-residual-20260920.json"
ID = "two-typed-attachment-joint-residual-20260920"
...
def render(scene):
    a, b, c, e1, e2 = scene
    return f"{a.text}, {e1[0]} {b.text}; {e2[0]} {c.text}."


def sig(text, edges):
    tape = letters(text)
    # Both relation labels are indexed jointly with the reverse residual.
    return (tape[:1], len(tape) % 2, len(tape) % 3, edges)
...
def run(limit=6000):
    ...
    for idx, scene in enumerate(all_scenes):
        reverse_edges = (scene[3][1], scene[4][1])
        index.setdefault(sig(render(scene)[::-1], reverse_edges), []).append(idx)
    joins = checked = prunes = 0; rows = []; controls = []
    for li, left_scene in enumerate(all_scenes):
        left_text = render(left_scene)
        edge_key = (left_scene[3][1], left_scene[4][1])
        candidates = index.get(sig(left_text, edge_key), [])
        ...
        for ri in candidates:
            ...
            row["provenance"] = {"three_clause_scene": True, "two_typed_attachment_edges": True,
                "joint_edge_index": True, "agreement_state_carried": True, "valency_state_carried": True,
    ...
    result = {"experiment_id": ID,
        "method": "joint bidirectional residual index over three-clause scenes with two typed attachment edges",
        "stats": {"clause_frames": len(clauses), "three_clause_scenes": len(all_scenes),
                  "joint_signature_buckets": len(index), "signature_join_hits": joins,
        ...
        "novelty_preflight": {"status": "passed",
            "signature": "three-clause|two-joint-typed-edges|bidirectional-residual-index|agreement-valency",
            "registry_inspected": True,
            "distinct_from": "one-edge three-clause joins, two-clause residual joins, single-clause trie, and repair lanes",
        ...
        "next_construction": "Join attachment-edge signatures with a typed center residual before lexical emission; avoid broadening this Cartesian scene bank.",
