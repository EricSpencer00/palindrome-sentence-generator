...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/endpoint-seed-interior-equations-20260920.json'
...
LEFT=(('an','archivist','maps','a','harbor'),('an','artist','marks','a','garden'),('an','analyst','keeps','a','journal'))
...
def run():
 ...
 for l in LEFT:
  for r in RIGHT:
   ...
   rows.append({'left_clause':' '.join(l),'right_clause':' '.join(r),'endpoint_seed_match':ep,'live_closed':ok,'trace':tr,'surface_grammar_valid':True,
