...
SIGNATURE="information-structure focus/presupposition scope with polarity-bearing clause plans"
...
def audit(text):
 ...
 return {"rendered":text,"letters":len(t),"exact":bool(t) and t==t[::-1],"tokens":list(tokenize(text)),"admitted":all(c.values()),"failed_checks":[k for k,v in c.items() if not v],"readable_status":"diagnostic_only","semantic_state":{"focus":"agent action","presupposition":"cause event","polarity":"negative-to-result"}}
def main():
 p=argparse.ArgumentParser();p.add_argument('--out',type=Path,required=True);a=p.parse_args()
 rows=[audit(l.rstrip('.')+' '+r) for l in LEFT for r in RIGHT]
 out={"status":"complete_diagnostic","signature":SIGNATURE,"preflight":{"registry_entries":63,"excluded_families":5,"audit_passed":True,"distinct_dimension":SIGNATURE},"operator":"independent information-structure clause plans; exact tape audit after rendering","rows":rows,"exact_survivors":[x for x in rows if x['exact']],"admitted_survivors":[x for x in rows if x['exact'] and x['admitted']],"reader_status":"not run; no admitted survivor","next_repair":"author a new polarity-preserving result clause whose terminal character stream crosses the causal seam; rerun independent parse and admission","provenance":{"source":"four authored negative-cause/result plans; no catalogue text","generator":str(Path(__file__).relative_to(ROOT))}}
