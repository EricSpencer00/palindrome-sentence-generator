...
def validate(left, right):
    ...
    return {"letters": len(tape), "two_pointer": bool(ok),
            "sha256": hashlib.sha256(tape.encode()).hexdigest(),
            "reverse_tape": rev == norm(" ".join(right))}

def main():
    lefts = [x.strip().split() for x in (ROOT / "data/authored_sentences.txt").read_text().splitlines() if x.strip()]
    ...
    lefts += [p["left"] for p in fixtures]
    trie = Trie(WORDS); out = []
    for left in lefts:
        ...
        for right, cost in segment(tape, trie):
            ...
            if v["two_pointer"] and v["reverse_tape"]:
                out.append({"left": " ".join(left), "right": " ".join(right), "cost": round(cost, 3), **v})
    out.sort(key=lambda x: (x["cost"], -x["letters"]))
    print(json.dumps({"method":"trie-beam-resegmentation", "seeds":len(lefts), "candidates":out[:20]}, indent=2))
