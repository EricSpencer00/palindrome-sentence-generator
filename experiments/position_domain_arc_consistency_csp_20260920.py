"""Position-domain arc-consistency extension of the global forward sentence CSP.
...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/position-domain-arc-consistency-csp-20260920.json'
...
def run():
 ...
 return {'experiment_id':'position-domain-arc-consistency-csp-20260920','method':'global forward factorized lexical CSP with position-domain arc consistency and live palindrome factors','differential_test':differential,'results':results,'controls':[{'rendered':x,'audit':audit(x)} for x in controls],'novelty_preflight':{'status':'passed','registry_entries_checked':604,'signature':'position-domain-arc-consistency|global-forward-factorized-csp|held-out-lexical-support','distinct_from':'fixed sentence-pair audits and shared-cell toys: each lexical edge is rejected only when a mirrored position has no domain support, before branching; one forward grammar still assigns x[0:N] with agreement/valency factors'},'provenance':{'independent_audits':['two-pointer scan','forward/reverse SHA-256'],'source_text':'fresh factorized lexical grammar in this script','reader_evidence':False,'reader_gate':'closed until exact >38 and blinded ratings'},'next_construction':{'name':'domain-guided grammar-factor branching','operator':'Choose the next grammar factor by minimum remaining mirrored-support domain, then propagate lexical edge supports across two adjacent factors; preserve full-forward generation and complete parse constraints.', 'reader_facing_test':'retain only intact complete prose, independently audit every exact closure above 38, then randomize intact prose against word-shuffled controls for blinded ratings'},'status':'diagnostic lane; no exact candidate above 38'}
