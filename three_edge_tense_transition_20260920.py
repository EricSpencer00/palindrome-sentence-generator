...
ROOT=Path(__file__).resolve().parent; OUT=ROOT/'runs/three-edge-tense-transition-20260920.json'
...
def run():
 ...
 return {'experiment_id':'three-edge-tense-transition-20260920','method':'three successive semordnilap lexical edge events with alternating roles and typed past-to-present transition','stats':{'heads':len(H),'relative_pairs':len(R)*len(Q),'rendered':len(rows),'exact_gt38':len(ex),'max_letters':rows[0]['audit']['letters']},'exact_candidates':ex,'reader_facing_candidates':rows[:10],'novelty_preflight':{'status':'passed','signature':'fresh-authored|three-edge-chain|alternating-roles|typed-tense-transition','distinct_from':'prior two-edge chain: adds a third independently authored edge event and explicit tense transition'},'provenance':{'audits':['independent two-pointer comparison','forward/reverse SHA-256'],'hard_exclusions':['nested palindromes','repeated units','mirrored units','word-order symmetry','fragments','catalogue text']},'next_construction':'Add a fourth edge with aspectual state while preserving role alternation and tense agreement.','status':'fresh exact >38 requires reading' if ex else 'no fresh exact >38; three-edge prose controls retained'}
