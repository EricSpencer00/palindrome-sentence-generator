"""Bounded locative NP agreement + preposition alternation."""
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; EXPERIMENT='relative-seam-np-agreement-prep-20260918'
...
CHOICES=[('now','records','a map','by','the inlet','singular_by'),('earlier','carry','maps','near','the shores','plural_near')]
  ...
  for ri,(noun,anaphor,number) in enumerate(REFERENTS):
   for ci,relative in enumerate(RELATIVES):
    for xi,(adverb,verb,obj,prep,place,kind) in enumerate(CHOICES):
     ...
     rows.append({'candidate_id':f's{si}-r{ri}-c{ci}-x{xi}','rendered':text,'regions':['setup','relative_clause','fixed_bridge','tense_fixed','locative_np_preposition'],'mutable_spans':['locative_number','locative_preposition'],'agreement':{'referent':noun,'anaphor':anaphor,'referent_number':number,'locative_number':kind},'frame_type':kind,'bridge_fixed':BRIDGE,'audit':audit(text),'provenance':{'construction':'locative_np_agreement_preposition_tense_fixed','catalogue_used':False,'wrapped_seed':False,'finished_tape_reversal':False,'repeated_self_palindromic_unit':False}})
 best=min(rows,key=lambda x:x['audit']['mismatches'])
 return {'experiment':EXPERIMENT,'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'rendered_candidates':rows,'stats':{'rendered':len(rows),'exact':sum(x['audit']['two_pointer_exact'] for x in rows),'longest_letters':max(x['audit']['letters'] for x in rows),'best_mismatches':best['audit']['mismatches']},'novelty_preflight':{'new_geometry':'locative NP agreement coupled with preposition alternation','prior_lane_reused':False,'duplicate_sweep':False},'next_repair':{'operator':'couple NP agreement/preposition with tense alternation while preserving noun number','reason':'preposition alternation preserves complete prose but leaves character debt'},'provenance':{'bounded_states':len(rows),'human_readability_certified':False}}
