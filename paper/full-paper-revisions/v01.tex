...
\title{Generating Exact English Palindromes with POS-Guided Search}
...
\begin{document}
% Full-paper snapshot 1; critique and successor changes are in FULL-PAPER-REVISION-LEDGER.md.
\maketitle

\begin{abstract}
We present a two-ended exact palindrome decoder with trie-guided word expansion and POS-guided pruning. Under a fixed search budget it returns more accepted palindromes than terminal-only checking, and an audited 90,937-letter construction demonstrates scale. We position exact palindrome generation as a difficult constrained-NLP task.
\end{abstract}
...
This complete first draft treats a better decoder and a longer artifact as its research contribution. It has a system story, but not yet a persuasive linguistic question.

\section*{Limitations}

This version does not isolate why palindrome generation is linguistically difficult. POS guidance has not been shown to imply grammar, meaning, or human quality.
