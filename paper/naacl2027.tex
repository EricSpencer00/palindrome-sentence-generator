...
\label{sec:state}
Hoey--Norvig construction grows text from both ends while recording the unmatched remainder \citep{norvig2016palindrome}. We call this remainder the \emph{overhang}; the name does not introduce a new algorithm. Norvig's account already describes this state. A letter-level version instead advances matched letters at both ends while tracking incomplete dictionary phrases. Our work builds on these representations rather than claiming their invention.
...
\label{sec:length}
The comparison target is the published version-3 output, listed as 90,439 letters and 21,012 words \citep{norvig2016palindrome}. We cache the reference program, dictionary, and reference text. Removing the reference page's heading and author footer yields an exact 90,439-letter palindrome under our independent parser. This explicit extraction prevents page furniture from affecting either validity or counts.
...
More importantly, the historical sweep counted \emph{summed worker-distinct outputs per aggregate worker-second}, not visited states per core-second. A useful decomposition is
\[
...
\begin{center}\small
\begin{tabular}{lrrr}\toprule
Archived comparison & Items & Reported judge 1 & Reported judge 2\\\midrule
...
Using Norvig's count\_2w data \citep{norvig2009ngrams}, let $g(a,b)$ be the corpus bigram log-probability of $b$ after $a$ minus its unigram log-probability. The edge score for placing pair $j$ inside pair $i$ is
\[
...
\citet{papadopoulos2015palindromes} construct a palindrome graph from forward and backward corpus graphs, with extensions for additional constraints. Our failed local filters therefore do not rule out corpus-based construction. The finite-template diagnostic establishes an inventory-specific failure.

Lexical constraints and lookahead decoding offer methods for retaining feasible hypotheses and allocating search effort \citep{hokamp2017grid,post2018fast,lu2022astar}. They do not guarantee readable palindrome prose. Here validity, proposal supply, and readability are assessed separately.
