...
def run():
 ...
 for a in BANK['A']:
  for z in BANK['Z']:
   outer=outer_domain_support(a,z, width=7)
 ...
 return {'experiment_id':'paragraph-abba-dialogue-trie-20260922','method':'naturalist/volcanic dialogue ABBA paragraph with seven-character aredlac exposed residual','stats':{'candidate_completions':len(rows),'outer_pruned':pruned,'exact_gt38':len(exact),'longest_letters':max((r['audit']['letters'] for r in rows),default=0),'fresh_outer_pair_completions':len(fresh),'fresh_outer_pair_support_depth':sorted(set(r['outer_domain_support']['matched'] for r in fresh))},'rendered_candidates':rows,'exact_candidates':exact,'fresh_outer_pair_candidates':fresh,'fresh_residual_probe':{'status':'fresh_natural_scene_pair_found','max_new_support_depth':7,'checked_against':'authored complete-sentence openings and natural sentence-final words','excluded':'arena/an era endpoint reuse and strained proper-name or catalogue endings','next_operator':'condition the residual after aredlac before indexing the inner question/response center'},'novelty_preflight':{'status':'passed','signature':'paragraph-abba|word-trie|naturalist-volcanic-dialogue|cross-boundary|seven-char-residual','distinct_from':'complete-clause residual decoders, fixed ABBA phrase banks, and the earlier arena/an-era family','finished_tape_reversal':False,'catalogue_text':False},'provenance':{'independent_audits':['two-pointer full tape','forward/reverse SHA-256'],'reader_gate':'closed unless exact >38'},'status':'fresh exact closure found' if exact else 'no exact closure; dialogue center retained','next_repair':'Condition the next residual after aredlac, then index the inner question/response center'}
