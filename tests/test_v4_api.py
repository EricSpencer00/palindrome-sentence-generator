...
def test_v4_health_reports_evidence_gate():
    ...
    assert body["reader_study_candidate_letters"] == 54
...
def test_v4_evidence_contains_actual_rendered_candidate_and_independent_audit():
    ...
    assert candidate["exact_scope_summary"]["best_admitted_exact_letters"] == 54
    assert candidate["exact_scope_summary"]["best_human_certified_exact_letters"] is None
    ...
    study_candidate = body["reader_study_candidate"]
    assert study_candidate["letters"] == 54
    assert study_candidate["audit"]["exact"] is True
    assert all(study_candidate["mechanical_checks"].values())
    assert study_candidate["provenance"]["reader_status"] == "not_run"
...
def test_v4_method_and_best_evaluation_are_explicitly_diagnostic():
    ...
    assert method.json()["optimization"]["current_search"] == "variable-phrase-grammar-20260920"
    assert method.json()["optimization"]["search_history"][-1] == "variable-phrase-grammar-20260920"
    ...
    assert method.json()["method_runs"][55]["run_id"] == "penn-feature-slot-product-20260919"
    assert method.json()["method_runs"][55]["exact_candidates"] == 0
    assert len(method.json()["method_runs"]) == 58
    assert method.json()["method_runs"][56]["run_id"] == "variable-phrase-grammar-20260920"
    ...
    assert method.json()["agreement_passive_relative_run"]["calibration_seed_recovered"] is True
    assert method.json()["agreement_passive_relative_run"]["held_out_exact_candidates"] == 0
    assert method.json()["subject_relative_center_run"]["calibration_seed_recovered"] is True
    assert method.json()["subject_relative_center_run"]["held_out_exact_candidates"] == 0
    assert method.json()["semantic_scene_lattice_run"]["exact_candidates_over_38"] == 0
    assert method.json()["semantic_scene_lattice_run"]["longest_rendered_letters"] == 88
    assert method.json()["bidirectional_phrase_lattice_run"]["exact_clean_over_38"] == 0
    assert method.json()["bidirectional_phrase_lattice_run"]["longest_rendered_letters"] == 57
    assert method.json()["typed_relative_async_phrase_run"]["exact_clean_over_38"] == 0
    assert method.json()["typed_relative_async_phrase_run"]["longest_rendered_letters"] == 60
    assert method.json()["semantic_scene_residual_run"]["exact_candidates_over_38"] == 0
    assert method.json()["semantic_scene_residual_run"]["longest_rendered_letters"] == 82
    assert method.json()["double_relative_async_run"]["exact_clean_over_38"] == 0
    ...
    assert method.json()["three_char_emission_residual_run"]["derived_keys"] == 5
    ...
    assert method.json()["center_first_agreement_product_run"]["states"] == 64
    assert method.json()["center_first_agreement_product_run"]["exact_candidates_over_38"] == 0
    assert method.json()["center_first_valency_product_run"]["states"] == 64
    assert method.json()["center_first_valency_product_run"]["exact_candidates_over_38"] == 0
    assert method.json()["center_first_mixed_valency_adjunct_run"]["states"] == 64
    ...
    assert method.json()["reverse_morphology_residual_run"]["states"] == 256
    ...
    assert method.json()["center_two_edge_attachment_graph_run"]["sequential_expansions"] == 256
    assert method.json()["center_two_edge_attachment_graph_run"]["exact_candidates_over_38"] == 0
    assert method.json()["center_clause_matrix_dependency_run"]["clause_expansions"] == 16
