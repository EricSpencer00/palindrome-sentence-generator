...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/was-np-temporal-np1-seam-20260921.json'
NP1=('the quiet scout at dawn','a red kite after rain','our old guide last night','one blue boat at noon')
NP2=('the guide by the quay','the kite near the reef','our sailor under the pier','one boat beside the dock')
...
def run():
 rows=[]
 for n1,n2 in itertools.product(NP1,NP2):
 ...
 return {'experiment_id':'was-np-temporal-np1-seam-20260921','method':'joint NP seam expansion for a grammatical was/what/saw question','stats':{'np1_phrases':len(NP1),'np2_phrases':len(NP2),'pairs':len(rows),'online_closed':sum(r['debt_equation']['closed'] for r in rows),'boundary_extended':sum(r['debt_equation']['extended_beyond_was_saw'] for r in rows),'accepted_exact':len(exact),'accepted_exact_gt38':sum(r['audit']['letters']>38 for r in exact)},'exact_candidates':exact,'rendered_controls':rows,'novelty_preflight':{'status':'passed','signature':'was-np-what-np-saw|joint-seam|role-preserving','signature_collision':False,'distinct_from':'Q/A tense-aspect-polarity debt and SVO lexical banks'},'next_operator':'Add an instrumental adjunct to NP2 while retaining joint seam debt and role checks.','status':'fresh exact closure found' if exact else 'no fresh exact closure; retain seam mismatch controls'}
