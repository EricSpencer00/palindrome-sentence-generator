...
| live slot-equation CFG resegmentation | “The patient courier delivers sealed letter before dawn. The quiet teacher opens marked parcel beside the gate.” | 92 | direct and SHA-256 `False`; admission `False` | held-out agent/verb replacement re-solves obligations before charting; replace one object/adjunct slot pair with a fresh authored alternative |
| clause-growth semantic frame repair | “the patient courier delivered the sealed letter before dusk. the quiet clerk opened the wooden parcel by the window. the careful gardener watered the young cedar after rain. the weary sailor mended the canvas sail near harbor.” | 186 | two-pointer and SHA-256 `False`; admission `False` | incremental ordinary-order scene growth with a fourth typed frame; author a fifth semantic frame against the updated debt frontier |
...
The aggregate after this wave is 4297 normalized rows across 104 route phases,
