...
ROOT=Path(__file__).resolve().parent; OUT=ROOT/'runs/paragraph-abba-constructive-20260921.json'
...
# Distinct roles and wording: no repeated units.
A=(('subject','singular','agent','The careful archivist labels a faded map.'),('subject','singular','agent','A quiet keeper measures the old bridge.'))
B=(('event','singular','patient','The lantern guides a tired sailor.'),('event','singular','patient','A small bell warns the waiting crew.'))
...
def run():
 ...
 for a,b in itertools.product(A,B):
  ...
  au=audit(paragraph); gates={'whole_output_exact':au['pointer_exact'],'outer_support':od>=2,'inner_support':idd>=2,'independent_pointer_hash':au['pointer_exact'] and au['sha256_forward']==au['sha256_reverse'],'distinct_sentence_roles':len(set(sentences))==4,'no_reversal':True,'no_catalogue':True,'no_post_hoc_repair':True}
  rows.append({'rendered':paragraph,'roles':{'A1':a,'B1':b,'B2':sentences[2],'A2':sentences[3]},'outer_trace':ot,'inner_trace':it,'support_depth':min(od,idd),'first_unsupported':{'outer':oc,'inner':ic},'audit':au,'gates':gates,'accepted':all(gates.values()),'provenance':{'construction':'four-role ABBA paragraph with live cross-sentence residuals','independently_authored':True,'finished_tape_reversal':False,'catalogue_text':False,'rendered_before_repair':True}})
