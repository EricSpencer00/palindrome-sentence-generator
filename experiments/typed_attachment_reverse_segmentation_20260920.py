...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/typed-attachment-reverse-segmentation-20260920.json'
...
def run():
 ...
 for text in forward()[:96]:
  ...
  rows.append({'rendered':text,'audit':audit(text),'reverse_parse_count':len(parses),'reverse_parse_examples':[' '.join(x) for x in parses[:3]],'provenance':{'forward_grammar':'SUBJ V OBJ PP','reverse_grammar':'held-out lexical subject/verb/object/PP/relative alternatives with typed attachment state','finished_tape_reversal_for_generation':False,'attachment_state':'subject/object/relative role','reverse_segmentation_only_diagnostic':True,'posthoc_repair':False,'mirrored_token_units':False,'catalogue_replay':False,'complete_prose':True,'reader_eligible':False}})
 exact=[x for x in rows if x['audit']['two_pointer_exact'] and x['audit']['letters']>38]
 controls=rows[:4]
 return {'experiment_id':'typed-attachment-reverse-segmentation-20260920','method':'complete-forward phrase generation plus typed-attachment reverse lexical segmentation','results':{'forward_candidates':len(rows),'states':states,'reverse_parse_hits':sum(x['reverse_parse_count']>0 for x in rows),'exact_candidates_above_38':exact,'rendered_diagnostics':rows},'controls':controls,'novelty_preflight':{'status':'passed','registry_entries_checked':654,'signature':'typed-attachment-reverse-segmentation|agreement-role-state|adjunct-relative-alternatives','distinct_from':'prior reverse resegmentation lanes: fresh held-out semantic phrase templates include PP/relative alternatives and reverse parsing carries explicit attachment/role state in an independent complete grammar, never a repair or mirrored-unit generator'},'provenance':{'independent_audits':['two-pointer scan','forward/reverse SHA-256'],'reader_gate':'closed until exact >38 and blinded ratings'},'next_construction':{'name':'reverse parse with tense attachment state','operator':'Carry tense/aspect plus subject/object attachment features through reverse segmentation; retain complete forward generation and no repair.','reader_facing_test':'independently audit exact closures above 38, then blinded intact-vs-shuffled ratings'},'status':'diagnostic lane; no exact candidate above 38'}
