"""Full connected-tree scheduler for the explicit let/hotel imperative."""
...
spec = importlib.util.spec_from_file_location("let_hotel_engine_20260913", ENGINE_PATH)
...
BASE = ENGINE.BASE; ENGINE.FIXED_LABELS.update({"fixed_let", "fixed_our", "fixed_guests", "fixed_carry", "fixed_cross", "fixed_paint", "fixed_wash", "fixed_enter", "fixed_and", "fixed_the", "fixed_clean", "fixed_narrow", "fixed_bright", "fixed_small", "fixed_quiet", "fixed_basket", "fixed_bridge", "fixed_sign", "fixed_dish", "fixed_hotel"})
...
class LetHotelGrammar:
    ...
    def productions(self, lhs):
        if lhs.name == "D":
            rhs = [BASE.sym("T", label="fixed_let", form="let"), BASE.sym("T", label="fixed_our", form="our"), BASE.sym("T", label="fixed_guests", form="guests"), BASE.sym("T", label="fixed_carry", form="carry"), BASE.sym("NP", role="object_1", frame="basket", type="object", number="sing", depth="0")]
            ...
            rhs.extend((BASE.sym("T", label="fixed_and_5", form="and"), BASE.sym("T", label="fixed_enter", form="enter"), BASE.sym("T", label="fixed_the", form="the"), BASE.sym("T", label="fixed_quiet", form="quiet"), BASE.sym("T", label="fixed_hotel", form="hotel")))
            return (BASE.Production("D:let-our-guests-carry-basket-cross-bridge-paint-sign-wash-dish-enter-hotel", lhs, tuple(rhs)),)
        if lhs.name == "NP":
...
def semantic_witness(tree):
    ...
    if tree: visit(tree)
    return {"semantic_roles": roles, "event_frames": frames, "relative_count": 0, "agreement_ok": bool(tree and tree.children[0].terminal == "let" and tree.children[2].terminal == "guests"), "valency_ok": frames == [x[0] for x in FRAMES], "complete_tree": bool(tree and tree.symbol.name == "D")}
...
def explicit_control(grammar):
    ...
    while True:
        ...
        if symbol.name == "D": ident = "D:let-our-guests-carry-basket-cross-bridge-paint-sign-wash-dish-enter-hotel"
        elif symbol.name == "NP":
...
def boundary_preflight():
    grammar = LetHotelGrammar(); state = explicit_control(grammar); events = []; failed = None
...
def run(max_states=100000):
    grammar = LetHotelGrammar(); result = solver(grammar, max_states); control_state = explicit_control(grammar); control = audit(grammar, BASE.render(control_state), "complete_explicit_imperative_control", control_state.trace); control.update({"diagnostic_only": True, "grammar_tree_fully_expanded": True, "provenance": {"construction": "five explicit ordinary imperative action/object clauses", "shared_tree": True}, "reader_status": "grammar control only; not a palindrome candidate or readability evidence"})
    return {"status": "imperative_let_hotel_clauses_intersection", "config": {"min_letters": MIN_LETTERS, "max_letters": MAX_LETTERS, "max_depth": 0, "max_states": max_states, "single_shared_derivation_tree": True, "character_residual_during_derivation": True, "outer_boundary_crossing": "let is matched inside longer hotel and into our", "explicit_clause_count": 5, "recursive_padding": False, "closure_requires_all_leaves_consumed": True, "independent_complete_reparse": True, "corpus_generation": False}, "provenance": {"generator_sha256": sha256(Path(__file__).read_bytes()).hexdigest(), "grammar_sha256": grammar.digest(), "material": "authored ordinary imperative action clauses and typed object frames; no catalogue or known palindrome material"}, "complete_recursive_control": control, **result, "joint_boundary_preflight": boundary_preflight(), "reader_facing_next_operator": "Use the replayed deepest contradiction to alter the exact exposed clause boundary, then rerun the full scheduler.", "scope": "Zero-closure bounded run is diagnostic; no reader-facing palindrome is claimed."}
