    ...
    for bay in ROLES['bay']:
     for spray in ROLES['spray']:
      for fog in ROLES['fog']:
       ...
       rows.append({'rendered':text,'roles':{'rain':rain,'agent':sam,'bird':bird,'bay':bay,'spray':spray,'fog':fog},'pre_render_obligations':True,'two_character_obligations':obligations,'audit':audit(text),'provenance':{'authored_role_alternatives':True,'catalogue_used':False,'finished_tape_reversal':False,'word_order_only_symmetry':False,'repeated_unit':False,'human_readability_certified':False}})
