...
RUN=Path(__file__).parents[1]/'runs/live-nodes-variable-function-boundaries-20260917.json'
def test_variable_boundaries_and_audits():
 d=json.loads(RUN.read_text());assert d['candidate_count']>0 and d['pruned_branches']>0
 for r in d['candidates']:
  assert r['live_nodes'] and r['audit']['exact']==r['audit']['independent_two_pointer'] and len(r['audit']['sha256'])==64
