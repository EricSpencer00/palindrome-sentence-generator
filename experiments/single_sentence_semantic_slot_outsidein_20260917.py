...
HELD_OUT_SETTING='by the harbor'
...
def run():
 ...
 control=rows[0]
 a,v,t,_=control['slots'].values(); s=HELD_OUT_SETTING; words=[a,v,t,s]; text=' '.join(words)+'.'; ob=live_obligations(words); au=audit(text)
 rows.append({'text':text,'letters':au['letters'],'slots':{'agent':a,'verb':v,'theme':t,'setting':s},'outside_in':ob,'independent_audit':au,'provenance':'held-out same-role setting substitution conditioned on first residual family; original row retained as control','anti_shortcut':{'two_complete_halves':False,'word_mirror':False,'repeated_unit':False,'catalogue':False},'mechanically_admitted':ob['closed'] and au['exact'],'held_out_repair':True,'control_row_text':control['text']})
 exact=[r for r in rows if r['mechanically_admitted']]
 longest=sorted(rows,key=lambda x:x['letters'],reverse=True)[:8]
 payload={'experiment_id':ID,'signature':SIG,'grammar':'Scene -> Agent Verb Theme Setting','slot_banks':{'agent':AGENTS,'verb':VERBS,'theme':THEMES,'setting':SETTINGS},'evaluated':len(rows),'held_out_repair_count':1,'status':'completed_no_exact_closure' if not exact else 'completed_exact','candidates':exact,'diagnostic_witnesses':longest,'control_rows':rows[:40],'held_out_repair':rows[-1],'exact_candidates':len(exact),'reader_eligible':False,'novelty_preflight':{'status':'passed','single_sentence':True,'two_complete_halves':False,'center_inside_token':True,'seed_catalogue_mirror':False,'basis':'one semantic slot derivation is checked by settled outside-in obligations'},'repair_after_failure':{'operator':'replace only the setting slot containing the first residual family, preserving scene role','first_dead_frontier':rows[0]['outside_in']['trace'][0] if rows else None,'next':'review this held-out trace before any further substitution'},'provenance':{'generator':str(Path(__file__).relative_to(ROOT)),'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'audit':'independent pointer and SHA-256'}}
