...
from experiments.relative_seam_attachment_locative_noun_20260918 import run,letters
def test_attachment_locative_noun_is_bounded_and_audited():
 p=run(); assert p['stats']['rendered']==16; assert p['stats']['exact']==0
 assert {x['frame_type'] for x in p['rendered_candidates']}=={'subject_inlet','object_shores'}
