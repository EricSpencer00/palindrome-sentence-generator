"""Typed agreement-aware adjunct growth across alternating clause frontiers.
...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/typed-agreement-adjunct-growth-20260920.json'
...
def run():
 ...
 return {'experiment_id':'typed-agreement-adjunct-growth-20260920','method':'alternating typed clause/adjunct growth from both sentence ends with live character obligations','results':[r],'controls':[{'rendered':x,'audit':audit(x)} for x in controls],'novelty_preflight':{'status':'passed','registry_entries_checked':628,'signature':'typed-agreement-adjunct-growth|number-tense-attachment-state|live-obligation-stack','distinct_from':'fixed clause-pair products, center/bridge/relative/endpoint lanes, and scheduler variants: clause and PP units are alternated from opposing frontiers while number/tense/attachment state is carried live; no finished-tape reversal, repeated unit, or repair'},'provenance':{'independent_audits':['two-pointer scan','forward/reverse SHA-256'],'source_text':'fresh authored clause and adjunct grammar','reader_evidence':False,'reader_gate':'closed until exact >38 and blinded ratings'},'next_construction':{'name':'held-out tense agreement growth','operator':'Add held-out past-tense and plural agreement clauses with attachment-compatible PP alternatives; preflight new signature first.','reader_facing_test':'retain complete prose only, independently audit exact closures above 38, then blinded intact-vs-shuffled ratings'},'status':'diagnostic lane; no exact candidate above 38'}
