...
from experiments.relative_seam_det_tense_locative_number_20260918 import run,letters
def test_det_tense_locative_number_is_bounded_and_audited():
 p=run(); assert p['stats']['rendered']==32; assert p['stats']['exact']==0
 assert {x['frame_type'] for x in p['rendered_candidates']}=={'singular_the','plural_our'}
