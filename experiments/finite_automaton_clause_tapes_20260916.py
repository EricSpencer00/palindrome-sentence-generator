...
def build():
 ...
 return tape,states
def main():
 tape,states=build(); rendered=' '.join(CLAUSES); checks=mechanical_admission_checks(rendered)
 pairs=[(i,len(tape)-1-i,tape[i],tape[-1-i]) for i in range(len(tape))]
 row={'rendered':rendered,'letters':len(tape),'finite_states':len(states),'exact':tape==tape[::-1], 'two_pointer':all(a==b for _,_,a,b in pairs), 'pointer_hash':hashlib.sha256(repr(pairs).encode()).hexdigest(),'reverse_hash':hashlib.sha256(tape[::-1].encode()).hexdigest(),'first_mismatch':next((p for p in pairs if p[2]!=p[3]),None),'checks':checks,'admitted':bool(len(tape)>=100 and tape==tape[::-1] and all(checks.values()) and not has_repeated_nontrivial_unit(tokenize(rendered))),'novelty_preflight':{'nested_palindrome_span':False,'filler_tokens':False,'repeated_content':False,'catalogue_match':False},'provenance':{'fresh_complete_clauses':True,'source_sentences_copied':False,'finished_palindrome_reused':False}}
