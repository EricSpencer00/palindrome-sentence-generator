"""Bounded tense/number/locative-noun repair."""
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; EXPERIMENT='relative-seam-tense-number-locative-noun-20260917'
...
CHOICES=[('now','records','a map','by the inlet','present_singular'),('earlier','carry','maps','along our wide shore','past_plural')]
  ...
  for ri,(noun,anaphor,number) in enumerate(REFERENTS):
   for ci,relative in enumerate(RELATIVES):
    for xi,(adverb,verb,obj,loc,kind) in enumerate(CHOICES):
     ...
     rows.append({'candidate_id':f's{si}-r{ri}-c{ci}-x{xi}','rendered':text,'regions':['setup','relative_clause','fixed_bridge','tense_number','locative_noun'],'mutable_spans':['verb_tense','referent_number','locative_noun'],'agreement':{'referent':noun,'anaphor':anaphor,'number':number},'frame_type':kind,'bridge_fixed':BRIDGE,'audit':audit(text),'provenance':{'construction':'tense_referent_number_locative_noun','catalogue_used':False,'wrapped_seed':False,'finished_tape_reversal':False,'repeated_self_palindromic_unit':False}})
 best=min(rows,key=lambda x:x['audit']['mismatches'])
 return {'experiment':EXPERIMENT,'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'rendered_candidates':rows,'stats':{'rendered':len(rows),'exact':sum(x['audit']['two_pointer_exact'] for x in rows),'longest_letters':max(x['audit']['letters'] for x in rows),'best_mismatches':best['audit']['mismatches']},'novelty_preflight':{'new_geometry':'tense jointly coupled with referent number and locative noun','prior_lane_reused':False,'duplicate_sweep':False},'next_repair':{'operator':'couple tense and number with locative preposition while preserving noun length','reason':'tense-number coupling preserves complete prose but leaves character debt'},'provenance':{'bounded_states':len(rows),'human_readability_certified':False}}
