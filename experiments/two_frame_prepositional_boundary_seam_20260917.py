"""Prepositional-boundary seam assignment over two fixed typed frames."""
from __future__ import annotations
import hashlib,itertools,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/"runs/two-frame-prepositional-boundary-seam-20260917.json";REG=ROOT/"docs/experiment-novelty-registry.json"
ID="two-frame-prepositional-boundary-seam-20260917";SIG="two-fixed-typed-frames|prepositional-boundary-seam-assignment|same-lexical-bank|ordinary-grammar|independent-exact-audit"
...
def emit(frame,owner):
 s,v,o,r,rv,ro=frame;first=f"{s.capitalize()} {v} {FIXED['determiner']} {FIXED['adjective']} {o} {FIXED['adverb']}";second=f"{r} {rv} {FIXED['determiner']} {FIXED['adjective']} {ro} {FIXED['adverb']}"
 text=f"{first} {FIXED['preposition']} {FIXED['complement']} {FIXED['seam']} {second}." if owner=="first" else f"{first} {FIXED['complement']} {FIXED['seam']} {second} {FIXED['preposition']}.";t=norm(text);mid=len(t)//2;sp=[];cur=0
 ...
 return {"rendered":text,"choices":{"frame":frame,"preposition_owner":owner,"fixed_states":FIXED},"audit":a,"center_state":{"midpoint":mid,"crossing":cross,"closed_pairs_before_first_mismatch":pairs,"live_debt":a["first_mismatches"][0] if a["first_mismatches"] else None,"operator":"prepositional-boundary seam assignment"},"anti_shortcut_flags":{"finished_tape_reversal":False,"word_order_symmetry":False,"repeated_self_palindromic_unit":False,"catalogue_text":False,"punctuation_changes_letters":False,"fragment":False},"provenance":{"lexical_source":"same two fixed hand-authored frames","borrowed_text":False,"new_lexical_bank":False,"no_clause_or_attachment":True,"ordinary_grammar":True}}
def run():
