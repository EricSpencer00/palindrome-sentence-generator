"""Bounded auxiliary-inflection plus short complement phrase repair."""
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; EXPERIMENT='relative-seam-auxiliary-phrase-20260917'
...
PAIRS=[('can still','continuative_modal'),('will soon','future_temporal')]
 ...
 for si,setup in enumerate(SETUPS):
  for ri,(noun,anaphor,typed_verb) in enumerate(REFERENTS):
   for ci,relative in enumerate(RELATIVES):
    for xi,(phrase,kind) in enumerate(PAIRS):
     for pi,response in enumerate(RESPONSES):
      text=f'{setup}, {noun}, {relative}; {BRIDGE} {phrase}, {anaphor} {response}.'
      rows.append({'candidate_id':f's{si}-r{ri}-c{ci}-x{xi}-p{pi}','rendered':text,'regions':['setup','relative_clause','fixed_bridge','auxiliary_complement_phrase','response'],'mutable_spans':['referent_number','auxiliary_complement_phrase','anaphoric_response'],'agreement':{'referent':noun,'anaphor':anaphor,'typed_verb':typed_verb},'phrase_type':kind,'bridge_fixed':BRIDGE,'audit':audit(text),'provenance':{'construction':'inflected_auxiliary_short_complement_phrase','catalogue_used':False,'wrapped_seed':False,'finished_tape_reversal':False,'repeated_self_palindromic_unit':False}})
 best=min(rows,key=lambda x:x['audit']['mismatches'])
 return {'experiment':EXPERIMENT,'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'rendered_candidates':rows,'stats':{'rendered':len(rows),'exact':sum(x['audit']['two_pointer_exact'] for x in rows),'longest_letters':max(x['audit']['letters'] for x in rows),'best_mismatches':best['audit']['mismatches']},'novelty_preflight':{'new_geometry':'inflected auxiliary plus short complement phrase at fixed seam','prior_lane_reused':False,'duplicate_sweep':False},'next_repair':{'operator':'permit auxiliary polarity choice with a two-word complement while preserving attachment','reason':'phrase repair preserves complete prose but leaves character debt'},'provenance':{'bounded_states':len(rows),'human_readability_certified':False}}
