...
RUN=Path(__file__).parents[1]/'runs/word-boundary-position-csp-20260917.json'
def test_boundary_csp_and_audits():
 d=json.loads(RUN.read_text());assert d['candidate_count']==len(d['candidates'])>0
 for r in d['candidates']:
  assert r['boundary_variables'] and r['propagated_equalities']>=0
