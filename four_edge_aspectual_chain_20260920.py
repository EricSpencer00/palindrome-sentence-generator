...
def run():
 ...
 return {'experiment_id':'four-edge-aspectual-chain-20260920','method':'four successive semordnilap lexical edge events with alternating roles, tense agreement, and typed aspect','stats':{'heads':len(H),'relative_pairs':len(R)*len(Q),'rendered':len(rows),'exact_gt38':len(ex),'max_letters':rows[0]['audit']['letters']},'exact_candidates':ex,'reader_facing_candidates':rows[:8],'novelty_preflight':{'status':'passed','signature':'fresh-authored|four-edge-chain|alternating-roles|typed-aspect','distinct_from':'prior three-edge chain: adds a fourth independently authored edge event with perfect aspect while retaining tense/role states'},'provenance':{'audits':['independent two-pointer comparison','forward/reverse SHA-256'],'hard_exclusions':['nested palindromes','repeated units','mirrored units','word-order symmetry','fragments','catalogue text']},'next_construction':'Move aspectual states into a live lexical-edge residual automaton with boundary compatibility rather than post-render metadata.','status':'fresh exact >38 requires reading' if ex else 'no fresh exact >38; four-edge prose controls retained'}
