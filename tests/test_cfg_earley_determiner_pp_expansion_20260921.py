...
spec=importlib.util.spec_from_file_location("cfg_det_pp",ROOT/"experiments/cfg_earley_determiner_pp_expansion_20260921.py")
lane=importlib.util.module_from_spec(spec); sys.modules[spec.name]=lane; spec.loader.exec_module(lane)
def test_determiner_pp_enters_before_rendering():
 ...
 for row in result["rows"]:
  ...
  assert row["provenance"]["determiner_bearing_pp"] and row["exact_check_pointer"]["exact"] is False and row["exact_check_sha"]["exact"] is False
