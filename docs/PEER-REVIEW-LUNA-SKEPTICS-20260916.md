...
exact survivor exists. The authoritative snapshot is 4,967 rows across 283
...
non-exact. A second single-state pass adds a 144-letter verb-edge scene, a
...
and four center-first residual-grammar outputs up to 244 letters. The 12-state
semantic-relation run is retained after correcting the field interpretation:
`word_order_mirror`, `repeated_content`, and `catalogue_absent` are positive
no-shortcut checks, and all 12 states pass them. Its 91-letter clause remains
...
5,195 rows across 309 routes (79 exact rejected tapes; 0 mechanically admitted
...
current aggregate is 5,335 rows across 329 routes, with 84 exact rows and zero
mechanically admitted outputs; the registry has 428 retained artifacts, 35
exclusions, and 406 retained run artifacts.
...
Three targeted repairs follow: a finite semantic-boundary lattice (72--75
letters), a fresh manual clause-plan search (best 71 letters), and a repaired
CFG frame (124 letters). The strongest intact rendering is “The baker bakes
fresh bread before dawn; The carpenter repairs blue sails after lunch.” Its
