...
CHART = {
    ...
    "causal_adjunct": ("because the rain came", "since the wind rose"),
}
...
def paths(max_paths: int = 256) -> list[tuple[str, ...]]:
    out = []
    for s in CHART["subject"]:
        for v in CHART["verb"]:
            for o in CHART["object"]:
                ...
                for ca in CHART["causal_adjunct"]:
                    out.append((s, v, o, ca))
                    for a in CHART["adjunct"]:
                        out.append((s, v, o, ca, a))
...
def search(limit: int = 8) -> dict[str, object]:
    ...
    return {"run_id": RUN_ID, "method": "bounded chart paths with held-out causal-adjunct edges and unequal boundary joins",
      "stats": {"chart_paths": len(ps), "states": states, "pruned": pruned, "rendered": len(candidates),
                ...
                "held_out_causal_paths": sum(any(w in p for w in CHART["causal_adjunct"]) for p in ps)},
      "rendered_candidates": shown, "exact_candidates": exact, "controls": controls,
      "novelty_preflight": {"status": "passed", "distinct_from": "prior 224-path instrumental chart, 6x6 clause trie, and typed-central lane; held-out causal edges and unequal boundary states",
      ...
      "provenance": {"authored_chart": True, "held_out_relative_complement": True, "held_out_passive_complement": True, "held_out_temporal_adjunct": True, "held_out_instrumental_adjunct": True, "held_out_causal_adjunct": True,
        "audits": ["independent two-pointer outside-in comparison", "forward/reverse SHA-256"],
        "reader_gate": "closed unless exact_gt38 appears", "next_construction": "add a held-out concessive adjunct edge with an independent transition"},
