# Center-out dependency realizer (2026-09-16)

This is a distinct construction route from relation alignment. It realizes
three-clause discourse plans on each side, with a typed topic graph carried
across clauses: an artifact becomes the next clause's topic, then a place
becomes the following topic. Every topic is singular and every finite verb is
agreement-checked. Left and right plans use independent lexical pools.

The bilateral character ledger starts at the seam between the two complete
plans and expands center-out, consuming one matching character obligation at a
time. Complete proposals are rendered and independently audited for exactness;
fragments, word-order mirrors, and catalogue text are never promoted.

Novelty preflight against the live registry found no exact signature collision
(the run artifact records its 102-entry execution snapshot; the live registry
has since advanced). The nearest prior was `anaphoric-scene-chain-composition`
at Jaccard 0.130435; the new state-space dimension is typed topic continuity
plus center-out bilateral realization.

The corrected run produced three complete discourse proposals (111, 117, and
111 letters), all semantically typed but non-palindromic. It explored 89
states, made 75 lexical assignments, and matched 11 seam characters in total
before dead ends. There were zero exact closures, mechanically admitted rows,
or reader-eligible outputs.

Artifact: [`experiments/centerout_dependency_realizer_20260916.py`](../experiments/centerout_dependency_realizer_20260916.py)

Evidence: [`centerout-dependency-realizer-20260916.json`](../runs/centerout-dependency-realizer-20260916.json)
