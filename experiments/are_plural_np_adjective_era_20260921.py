...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/are-plural-np-adjective-era-20260921.json'
NPS=('the quiet scouts','the red kites','our old guides'); ADJ=('ready','calm','alert')
...
def run():
 ...
 for np,adj in itertools.product(NPS,ADJ):
  ...
  gates={'shell_grammatical':True,'plural_agreement':np.startswith('the') or np.startswith('our'),'online_shell_match':m>=2,'whole_output_exact':au['exact'],'content_disjoint':len(content)==len(set(content)),'no_self_pal_units':all(norm(w)!=norm(w)[::-1] for w in content)}
