...
RUN=Path(__file__).parents[1]/'runs/pair-specific-seam-conflict-repair-20260917.json'
def test_role_repair_and_audits():
 d=json.loads(RUN.read_text());assert d['candidate_count']==2
 for r in d['candidates']:
  assert r['repaired_role'] in {'a','v'} and r['preserved_roles']
