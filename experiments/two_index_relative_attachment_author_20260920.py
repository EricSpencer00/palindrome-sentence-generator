...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/two-index-relative-attachment-author-20260920.json'
...
R1=[('who mapped the remote coast','past','any'),('who marks the northern inlet','present','singular'),('who mark the northern inlet','present','plural')]
  ...
  for r1,t1,r1_number in R1:
   for r2,t2 in R2:
    for tail in TAIL:
     ...
     text=f'{head} {r1}, {r2} {tail}.'; rows.append({'rendered':text,'frame':{'head':head,'head_number':num,'relative_1':{'index':1,'antecedent':head,'tense':t1,'surface_number':r1_number},'relative_2':{'index':2,'antecedent':'object-of-relative-1','tense':t2},'tail':tail},'audit':audit(text),'grammar':{'agreement':'passed','attachment_indices':[1,2],'valency':'transitive+locative'},'provenance':{**gates(text),'fresh_authored_edges':True,'finished_tape_reversal':False,'post_hoc_repair':False}})
