"""Bounded relative-attachment/locative-determiner repair."""
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; EXPERIMENT='relative-seam-attachment-locative-det-20260918'
...
CHOICES=[('who marks the harbor map','she','records','a map','by','the inlet','subject_the'),('that the guide carried the cedar key','the keepers','carry','maps','near','our shores','object_our')]
 ...
 for si,setup in enumerate(SETUPS):
  for ri,(noun,number) in enumerate(REFERENTS):
   for ci,(relative,anaphor,verb,obj,prep,place,kind) in enumerate(CHOICES):
    ...
    rows.append({'candidate_id':f's{si}-r{ri}-c{ci}','rendered':text,'regions':['setup','relative_attachment','fixed_bridge','anaphor_lexical_class','locative_determiner_agreement'],'mutable_spans':['relative_attachment','locative_determiner'],'agreement':{'referent':noun,'referent_number':number,'relative_attachment':kind,'anaphor':anaphor,'preposition':prep},'frame_type':kind,'bridge_fixed':BRIDGE,'audit':audit(text),'provenance':{'construction':'relative_attachment_locative_determiner','catalogue_used':False,'wrapped_seed':False,'finished_tape_reversal':False,'repeated_self_palindromic_unit':False}})
 best=min(rows,key=lambda x:x['audit']['mismatches'])
 return {'experiment':EXPERIMENT,'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'rendered_candidates':rows,'stats':{'rendered':len(rows),'exact':sum(x['audit']['two_pointer_exact'] for x in rows),'longest_letters':max(x['audit']['letters'] for x in rows),'best_mismatches':best['audit']['mismatches']},'novelty_preflight':{'new_geometry':'relative attachment jointly selects locative determiner','prior_lane_reused':False,'duplicate_sweep':False},'next_repair':{'operator':'couple attachment with locative noun while preserving determiner/preposition','reason':'attachment/determiner coupling preserves complete prose but leaves character debt'},'provenance':{'bounded_states':len(rows),'human_readability_certified':False}}
