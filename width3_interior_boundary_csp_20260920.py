...
def run():
 ...
 for h,v,o in left:
  for rh,rv,ro in right:
   ...
   # Third interior equation: the subject-slot seam is checked before render.
   if n(h)[-1]!=n(rh)[0]:continue
   ib+=1;text=f'{h} {v} {o}; {rh} {rv} {ro}.'
   overlap=content_overlap(text.split('; ')[0],text.split('; ')[1])
   rows.append({'rendered':text,'left_clause':text.split('; ')[0],'right_clause':text.split('; ')[1],'audit':audit(text),'equations':{'endpoint_width':3,'left_prefix':lt[:3],'reverse_right_suffix':rt[-3:][::-1],'interior_verb_boundary':{'left_slot_prefix':n(v)[0],'reverse_right_slot_suffix':n(rv)[-1],'matched':True},'interior_object_boundary':{'left_slot_prefix':n(o)[0],'right_slot_prefix':n(ro)[0],'matched':True},'interior_subject_boundary':{'left_slot_suffix':n(h)[-1],'right_slot_prefix':n(rh)[0],'matched':True}},'provenance':{'left':'fresh forward clause bank','right':'fresh disjoint forward clause bank','finished_tape_reversal':False,'post_hoc_repair':False,'catalogue_borrowing':False,'mirrored_units':False,'repeated_units':bool(overlap),'repeated_content_words':overlap,'fragment':False}})
