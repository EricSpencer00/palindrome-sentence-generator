"""Frame-conditioned clause topology over independent full-clause factors.
...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/frame-conditioned-clause-csp-20260920.json'
...
def run():
 ...
 return {'experiment_id':'frame-conditioned-clause-csp-20260920','method':'factorized independent full-clause grammar coupled by live word-boundary character obligations','results':[r],'controls':[{'rendered':x,'audit':audit(x)} for x in controls],'novelty_preflight':{'status':'passed','registry_entries_checked':648,'signature':'frame-conditioned-clause|tense-voice-alternation-state|word-boundary-obligation','distinct_from':'cadence adjunct growth, fixed clause pairs, endpoint envelopes, and center/bridge lanes: both full clauses are composed independently from subject/verb/object/locative factors while boundary characters are coupled online; no reversal, repeated units, repair, or catalogue text'},'provenance':{'independent_audits':['two-pointer scan','forward/reverse SHA-256'],'source_text':'fresh authored factorized clause grammar','reader_evidence':False,'reader_gate':'closed until exact >38 and blinded ratings'},'next_construction':{'name':'aspect-conditioned clause topology','operator':'Add held-out aspect/voice alternations with agreement state; preflight a new signature first.','reader_facing_test':'retain complete vivid prose, independently audit exact closures above 38, then blinded intact-vs-shuffled ratings'},'status':'diagnostic lane; no exact candidate above 38'}
