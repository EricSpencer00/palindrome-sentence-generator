"""Two-character beam only on tied object-setting frontier branches."""
from __future__ import annotations
import hashlib,itertools,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/"runs/scene-lattice-tied-two-char-beam-20260917.json";REG=ROOT/"docs/experiment-novelty-registry.json"
ID="scene-lattice-tied-two-char-beam-20260917";SIG="independent-scene-lattice|tied-two-character-reverse-prefix-beam|object-setting-frontier|compact-single-sentence|independent-exact-audit"
...
def run():
 ...
 # Only tied branches at each object are considered; no one-character sweep is rerun.
 selected=[]
 for obj in OBJECTS:
 ...
 return {"experiment_id":ID,"signature":SIG,"status":"completed_exact" if exact else "completed_no_exact_closure","method":"tied two-character reverse-prefix beam at object-setting frontier","novelty_preflight":pre,"pre_filter_count":len(pool),"candidate_count":len(rows),"exact_count":len(exact),"reader_eligible":False,"rendered_candidates":rows,"stats":{"beam_depth":2,"selected":len(rows),"max_matched_characters":max(r["beam"]["matched_characters"] for r in rows),"longest_letters":max(r["audit"]["letters"] for r in rows),"midpoint_inside_token":sum(r["center_state"]["crossing"] is not None for r in rows)},"failure_and_repair":{"failure":"no exact closure" if not exact else "exact closure found","next_repair":"change the setting bank by one independently authored phrase whose first two letters satisfy a typed role, then rerun only this two-character beam"},"provenance":{"generator_sha256":hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),"independent_audits":["two-pointer scan","forward/reverse SHA-256","pre-render tied two-character beam"],"shortcuts_excluded":True}}
