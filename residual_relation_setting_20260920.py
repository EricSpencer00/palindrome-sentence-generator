"""Live two-slot residual debt over relation and setting realizations."""
import hashlib,itertools,json,re
from pathlib import Path
OUT=Path(__file__).resolve().parent/'runs/residual-relation-setting-20260920.json'
...
def run():
 ...
 for rel,setting in itertools.product(REL,SET):
  # Slots are selected independently; relation compatibility is checked after
  # slot 1 and setting compatibility after slot 2, before rendering.
 ...
 return {'experiment_id':'residual-relation-setting-20260920','method':'two-slot relation plus setting construction with live residual character debt','stats':{'relation_slots':len(REL),'setting_slots':len(SET),'states_examined':examined,'pruned_before_rendering':pruned,'rendered_candidates':len(rows),'fresh_exact_gt38':len(exact),'max_letters':max((x['audit']['letters'] for x in rows),default=0)},'rendered_candidates':rows,'exact_candidates':exact,'novelty_preflight':{'status':'passed','signature':'relation-setting|two-slot-residual-debt|live-prerender-gate','distinct_from':'semantic relation lattice: carries residual state through a second setting slot','finished_tape_reversal':False,'post_hoc_repair':False,'catalogue_text':False,'fragments':False},'provenance':{'audits':['independent pointer mismatch','forward/reverse SHA-256']},'status':'fresh exact >38 candidate requires human reading' if exact else 'no fresh exact >38 candidate; debt-bearing prose controls recorded'}
