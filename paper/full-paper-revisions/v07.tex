...
\title{Exact Palindrome Decoding with a POS-Shape Feasibility Gate}
...
\begin{document}
% Full-paper snapshot 7; critique and successor changes are in FULL-PAPER-REVISION-LEDGER.md.
\maketitle

\begin{abstract}
The exact decoder uses a finite POS-shape language as a sound pruning predicate: a partial sequence that cannot extend to an allowed tag pattern is safely rejected. Formal palindrome validity remains independently checked.
\end{abstract}
...
This makes the decoder claim precise, but its fixed-work efficiency ratio still requires careful interpretation.

\section*{Limitations}

POS shape permits ambiguity, syntactic errors, and semantic false positives. It is not a grammar or meaning model.
