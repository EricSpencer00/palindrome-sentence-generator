...
CLAUSE_TAILS = ["and the bell was quiet", "while the tide rose", "and the crew saw flora"]
...
def run():
    rows, outer_prunes, seam_prunes, retained_outer = [], 0, 0, 0
    ...
    for (det, start_class, required_end), (subject, number), (verb, end_class), obj, att, tail in itertools.product(
        ...
        # Equality is the outer-character condition, enforced before interior slots.
        if start_class != required_end or start_class != tape(det)[0] or required_end != tape(tail)[-1]:
    ...
    return {"experiment_id": "outer-character-conditioned-sentence-20260921",
            ...
            "stats": {"outer_assignments_pruned": outer_prunes, "retained_outer_assignments": retained_outer, "rendered_controls": len(rows),
