"""Full connected-tree scheduler for the explicit can/cognac question.

The lexicon-backed final noun ``cognac`` is longer than opening ``can``; its
reversed stream crosses ``can`` and the opening ``go`` of ``golfers``. Length
comes from four explicit ordinary action/object clauses, not recursion.
"""
...
spec = importlib.util.spec_from_file_location("can_cognac_engine_20260913", ENGINE_PATH)
...
BASE = ENGINE.BASE; ENGINE.FIXED_LABELS.update({"fixed_can", "fixed_golfers", "fixed_carry", "fixed_cross", "fixed_pour", "fixed_rinse", "fixed_serve", "fixed_and", "fixed_the", "fixed_clean", "fixed_narrow", "fixed_dark", "fixed_warm", "fixed_strong", "fixed_basket", "fixed_bridge", "fixed_syrup", "fixed_glass", "fixed_cognac"})
from llm_palindrome.admission import mechanical_admission_checks

MIN_LETTERS, MAX_LETTERS = 100, 180
FRAMES = (("basket", "clean"), ("bridge", "narrow"), ("syrup", "dark"), ("glass", "warm"))


class CanCognacGrammar:
    ...
    def productions(self, lhs):
        if lhs.name == "D":
            rhs = [BASE.sym("T", label="fixed_can", form="can"), BASE.sym("T", label="fixed_golfers", form="golfers"), BASE.sym("T", label="fixed_carry", form="carry"), BASE.sym("NP", role="object_1", frame="basket", type="object", number="sing", depth="0")]
            for i, (verb, frame) in enumerate((("cross", "bridge"), ("pour", "syrup"), ("rinse", "glass")), 2): rhs.extend((BASE.sym("T", label=f"fixed_and_{i}", form="and"), BASE.sym("T", label=f"fixed_{verb}", form=verb), BASE.sym("NP", role=f"object_{i}", frame=frame, type="object", number="sing", depth="0")))
            rhs.extend((BASE.sym("T", label="fixed_and_5", form="and"), BASE.sym("T", label="fixed_serve", form="serve"), BASE.sym("T", label="fixed_the", form="the"), BASE.sym("T", label="fixed_strong", form="strong"), BASE.sym("T", label="fixed_cognac", form="cognac")))
            return (BASE.Production("D:can-golfers-carry-basket-cross-bridge-pour-syrup-rinse-glass-serve-cognac", lhs, tuple(rhs)),)
        if lhs.name == "NP":
...
def semantic_witness(tree):
    ...
    return {"semantic_roles": roles, "event_frames": frames, "relative_count": 0, "agreement_ok": bool(tree and tree.children[0].terminal == "can" and tree.children[1].terminal == "golfers"), "valency_ok": frames == expected, "complete_tree": bool(tree and tree.symbol.name == "D")}
...
def explicit_control(grammar):
    ...
    while True:
        ...
        if symbol.name == "D": ident = "D:can-golfers-carry-basket-cross-bridge-pour-syrup-rinse-glass-serve-cognac"
        elif symbol.name == "NP":
...
def boundary_preflight():
    grammar = CanCognacGrammar(); state = explicit_control(grammar); events = []; failed = None
...
def run(max_states=100000):
    grammar = CanCognacGrammar(); result = solver(grammar, max_states); control_state = explicit_control(grammar); control = audit(grammar, BASE.render(control_state), "complete_explicit_question_control", control_state.trace); control.update({"diagnostic_only": True, "grammar_tree_fully_expanded": True, "provenance": {"construction": "four explicit ordinary question action/object clauses", "shared_tree": True}, "reader_status": "grammar control only; not a palindrome candidate or readability evidence"})
    return {"status": "question_can_cognac_clauses_intersection", "config": {"min_letters": MIN_LETTERS, "max_letters": MAX_LETTERS, "max_depth": 0, "max_states": max_states, "single_shared_derivation_tree": True, "character_residual_during_derivation": True, "outer_boundary_crossing": "can is matched inside longer cognac and into golfers", "explicit_clause_count": 5, "recursive_padding": False, "closure_requires_all_leaves_consumed": True, "independent_complete_reparse": True, "corpus_generation": False}, "provenance": {"generator_sha256": sha256(Path(__file__).read_bytes()).hexdigest(), "grammar_sha256": grammar.digest(), "material": "authored ordinary question action clauses and typed basket/bridge/syrup/glass frames; no catalogue or known palindrome material"}, "complete_recursive_control": control, **result, "joint_boundary_preflight": boundary_preflight(), "reader_facing_next_operator": "Use the replayed deepest contradiction to alter the exact exposed clause boundary, then rerun the full scheduler.", "scope": "Zero-closure bounded run is diagnostic; no reader-facing palindrome is claimed."}
