"""Fresh typed bilateral event-product with two live role obligations.
...
def audit(text):
    ...
    while i < j:
        ...
        i += 1; j -= 1
    return {"exact": not mismatches, "letters": len(x),
...
def main():
    ...
    result={"run_id":"dual-obligation-event-product-20260920","method":"typed bilateral event-product with two live agent/theme boundary obligations and held-out centre lexicon","novelty_preflight":{"signature":"fresh-authored|typed-bilateral-event-product|dual-live-obligations|heldout-centre","prior_signatures_checked":["recursive-typed-event-graph|free-center|carried-character-obligations","semantic-lattice|live-equation|scene-first"],"duplicate_sweep":False},"inventory":{"left_events":len(LEFT),"right_events":len(RIGHT),"heldout_centres":len(CENTRES)},"stats":{"candidate_states":len(LEFT)*len(RIGHT)*len(CENTRES),"admitted":len(rows),"obligation_prunes":prunes,"exact_over_38":sum(r["audit"]["exact"] and r["audit"]["letters"]>38 for r in rows),"max_letters":max([r["audit"]["letters"] for r in rows+controls])},"candidates":rows[:20],"complete_prose_controls":controls,"independent_audit":{"algorithm":"two-pointer over normalized letters","sha256_forward":True,"sha256_reverse":True,"all_rows_audited":True},"next_repair":"Replace boundary-class equality with typed subcategorical obligations that include agreement and a centre word selected before event realization; do not enlarge this lexical inventory."}
