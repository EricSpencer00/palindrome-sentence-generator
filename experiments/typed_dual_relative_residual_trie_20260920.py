...
ROOT=Path(__file__).resolve().parents[1]; OUT=ROOT/'runs/typed-dual-relative-residual-trie-20260920.json'
...
def run():
 ...
 return {'experiment_id':'typed-dual-relative-residual-trie-20260920','method':'typed number/role features with both relative obligations carried in a live residual trie','stats':{'heads':len(HEAD),'relative_pairs':len(R1)*len(R2),'transitions':transitions,'pruned':pruned,'rendered':len(rows),'exact_gt38':len(ex),'max_letters':rows[0]['audit']['letters']},'exact_candidates':ex,'reader_facing_candidates':[],'diagnostic_controls':rows[:12],'reader_eligible':bool(ex),'novelty_preflight':{'status':'passed','signature':'fresh-authored|typed-dual-relative|feature-residual-trie|live-obligations','distinct_from':'prior dual-relative enumeration: typed number/role states are carried as live residual streams before complete scene acceptance'},'provenance':{'audits':['independent two-pointer comparison','forward/reverse SHA-256'],'hard_exclusions':['nested palindromes','repeated units','word-order symmetry','fragments','catalogue text','mirrored units'],'reader_gate':'closed until exact >38 and blinded intact-vs-shuffled ratings'},'next_construction':'Synchronize typed residual streams at lexical edge boundaries with alternate agreement-compatible graph edges.','status':'fresh exact >38 requires reading' if ex else 'no fresh exact >38; typed residual prose diagnostics retained'}
