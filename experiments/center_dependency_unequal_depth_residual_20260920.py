"""Center-shared dependency with unequal attachment depths and live residual."""
from __future__ import annotations
import hashlib,json,re
from itertools import product
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/"runs/center-dependency-unequal-depth-residual-20260920.json"
...
def run():
 ...
 for center,left,right,residual in product(CENTERS,LEFT,RIGHT,RESIDUALS):
  states+=1;event,kind=center;lt=f"{left[1]} when {event}, {residual}";rt=f"{right[1]} after it.";rendered=lt+"; "+rt;eq=live(lt,rt);row={"rendered":rendered,"center_dependency":{"event":event,"kind":kind,"residual":residual},"left_depth":2,"right_depth":1,"left_attachment":left,"right_attachment":right,"online_character_equations":eq,"audit":audit(rendered)}
  ...
  row["provenance"]={"center_shared_dependency_node":True,"unequal_attachment_depths":True,"live_center_residual":True,"typed_left_right_attachments":True,"complete_utterances":True,"catalogue_text":False,"finished_tape_reversal":False,"post_hoc_repair":False,"mirrored_units":False,"word_order_symmetry":False,"fragment":False};rows.append(row)
 exact=[r for r in rows if r["audit"]["pointer_exact"] and r["audit"]["letters"]>38]
 result={"experiment_id":"center-dependency-unequal-depth-residual-20260920","method":"center-shared dependency with unequal attachment depths and live center residual","stats":{"center_nodes":len(CENTERS),"left_depth_two":len(LEFT),"right_depth_one":len(RIGHT),"residuals":len(RESIDUALS),"states":states,"live_prunes":prunes,"live_survivors":len(rows),"exact_gt38":len(exact),"reader_eligible":0,"longest_letters":max((r["audit"]["letters"] for r in rows+controls),default=0)},"controls":controls,"exact_candidates":exact,"reader_facing_candidates":[],"novelty_preflight":{"status":"passed","signature":"center-dependency|unequal-depths|live-center-residual","registry_inspected":True,"distinct_from":"equal-depth dependency frame, feature sweeps, and relation-overhang seams","catalogue_text_imported":False,"finished_tape_reversal":False,"post_hoc_repair":False,"mirrored_units":False},"provenance":{"generator_sha256":hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),"independent_audits":["pointer scan","forward/reverse SHA-256"]},"status":"no reader-worthy exact closure","next_construction":"Let the center residual attach to a typed left dependent before the right dependent is emitted."}
