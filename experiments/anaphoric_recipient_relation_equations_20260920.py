...
def banks(state):
 ...
 rec2=chunks("recipient","child","anaphoric-recipient","him",number="singular",animacy="animate",anaphoric=True)+chunks("recipient","friend","anaphoric-recipient","her",number="singular",animacy="animate",anaphoric=True)+chunks("recipient","poet","anaphoric-recipient","them",number="plural",animacy="animate",anaphoric=True)+chunks("recipient","guards","anaphoric-recipient","them",number="plural",animacy="animate",anaphoric=True)
 return s1,v,t,p,rec,c,s2,v,t,p,rec2
def vok(s,v):return (s.number=="singular")==v.text.endswith("s")
def fok(s,v,t,p,r,c,state):return vok(s,v) and v.valency=="dative" and t.valency=="patient" and t.animacy=="inanimate" and p.relation==state and p.text==("for" if state=="benefit" else "to") and r.valency in {"animate-recipient","anaphoric-recipient"} and r.animacy=="animate" and c.relation==state and c.text==("although" if state=="benefit" else "while")
def controls():
 ts=["Mara gives the letter for the child although she sends a book for him.","Noah brings the seal for a friend although he offers books for her.","The guards show a book for the poet although they give the letter for them.","Mara sends the letters for the child although she brings a book for him.","Mara gives a book to the friend while she sends the letters to her.","Noah brings the letter to the poet while he offers books to them.","The guards send the seal to the child while they give a book to him.","Mara offers a letter to a friend while she shows the book to her.","Noah gives books to the poet while he sends the seal to them.","The guards bring the letter to the child while they show a book to him.","Mara sends the letters to a friend while she gives a book to her.","Noah offers a book to the poet while he gives letters to them.","The guards send the letter to the child while they bring books to him.","Mara shows the seal to a friend while she gives a book to her.","Noah offers books to the poet while he sends the letter to them.","The guards bring a book to the child while they give the seal to him.","Mara gives the book for a friend although she sends the seal for her.","Noah brings the letter to the poet while he offers the book to them.","The guards show seals to the child while they give a book to him.","Mara sends books for the poet although she brings the letter for them."]
 ...
 def one(state):
  ...
  def walk(lo,hi,left,right,li,ri,eq):
   ...
   if lo>hi:
    ...
    if rs[0].ref!=ls[0].ref or not rs[0].anaphoric or not rs[4].anaphoric or rs[4].ref not in {ls[4].ref,"guards"} or not fok(ls[0],ls[1],ls[2],ls[3],ls[4],ls[5],state) or not fok(rs[0],rs[1],rs[2],rs[3],rs[4],ls[5],state):feature_pruned+=1;return
