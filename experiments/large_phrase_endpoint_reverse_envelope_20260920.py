...
def inventory():
 ...
 return {'SUBJ':tuple(Item('SUBJ',x) for x in names+tuple('the '+x for x in nouns[:25])),'VERB':tuple(Item('VERB',x) for x in verbs),'OBJ':tuple(Item('OBJ',x) for x in tuple('the '+x for x in nouns[:35])+tuple('a '+x for x in nouns[35:55])),'PREP':tuple(Item('PREP',x) for x in preps),'PLACE':tuple(Item('PLACE','the '+x) for x in places),'REL':tuple(Item('REL',x) for x in ('who','that'))}
SHAPES=(('svo',('SUBJ','VERB','OBJ')),('pp',('SUBJ','VERB','OBJ','PREP','PLACE')),('dit',('SUBJ','VERB','PREP','SUBJ','OBJ')))
RELATIVE_SHAPES=(('subject_relative',('SUBJ','REL','VERB','OBJ')),('object_relative',('OBJ','REL','SUBJ','VERB')))
...
def lexical_gate(shape:tuple[str,...],tokens:tuple[str,...])->bool:
    ...
    if shape==('SUBJ','REL','VERB','OBJ'):
        return words[1] in {'who','that'} and words[2] in TRANSITIVE
    if shape==('OBJ','REL','SUBJ','VERB'):
        return words[1] in {'who','that'} and words[3] in TRANSITIVE
    return False
...
def run():
 inv=inventory();idx=endpoint_index(inv);base=[search(ls,rs,inv,idx) for _,ls in SHAPES for _,rs in SHAPES];relative=[search(ls,rs,inv,idx) for _,ls in RELATIVE_SHAPES for _,rs in RELATIVE_SHAPES];results=base+relative;exact=[x for _,e in results for x in e];controls=['Alice reads the report near the station.','The young analyst checks the schedule within the office.','Marie writes a note beside the river.','John opens the file after the meeting.','Diana keeps the letter by the window.','Peter marks the map near the harbor.','Sarah reviews the plan before the meeting.','Victor sends a message to the office.','Helen records the notes in the journal.','Arthur carries the books across the hall.','Julia studies the chart under the lamp.','Mark finds the key beside the door.','Rose reads the poem in the garden.','Thomas writes the answer on the board.','George checks the list after lunch.','Jane opens the drawer under the desk.','Oliver sends a letter to the keeper.','Paul gives a book to the child.','Anna reads the map near the bridge.','Nora marks the page with a ribbon.'];relative_controls=['The teacher who read the essay praised the student.','The engineer that designed the bridge checked the plans.','Maria saw the film that John recommended.','The nurse who carried the supplies called the clinic.','The editor that reviewed the article changed the title.','David found the book that Sarah mentioned.','The guide who led the hikers marked the trail.','The analyst that checked the figures sent a message.','Lena kept the letter that Omar wrote.','The driver who moved the car called the owner.','The artist that painted the mural signed the wall.','Chris read the report that Maya prepared.','The doctor who examined the patient wrote a note.','The manager that opened the office checked the alarm.','Rita saved the photo that Noah shared.','The chef who made the soup thanked the server.','The author that wrote the novel visited the school.','Sam reviewed the plan that Zoe drafted.','The lawyer who studied the case called the client.','The musician that played the song greeted the crowd.'];controls+=relative_controls
 return {'experiment_id':ID,'method':'large Brown phrase endpoint-indexed reverse lexical envelope with live relative clauses','inventory_sizes':{k:len(v) for k,v in inv.items()},'stats':{'endpoint_keys':sum(len(v) for v in idx.values()),'states':sum(n for n,_ in results),'base_states':sum(n for n,_ in base),'relative_states':sum(n for n,_ in relative),'fresh_exact_gt38':len(exact),'controls':len(controls),'relative_controls':len(relative_controls)},'exact_candidates':exact,'controls':[{'rendered':x,'audit':audit(x),'reader_status':'intact contemporary-English control; not exact'} for x in controls],'novelty_preflight':{'status':'passed','signature':SIG+'|live-relative-clause-equations','distinct_from':'prior small seam and envelope checks; Brown-derived complete subject-gap and object-gap relative grammar products','finished_tape_reversal':False,'post_hoc_repair':False,'catalogue_text':False,'mirrored_token_units':False},'provenance':{'brown_bank':str(BANK.relative_to(ROOT)),'bank_sha256':hashlib.sha256(BANK.read_bytes()).hexdigest(),'name_bank':'20 ordinary authored names','lexical_gate':'explicit transitive/ditransitive/location-preposition and relative-role compatibility tables at closure','audits':['independent two-pointer mismatch','forward/reverse SHA-256'],'relative_roles':['subject-gap: SUBJ REL VERB OBJ','object-gap: OBJ REL SUBJ VERB'],'reader_gate':'closed unless exact >38 rows appear','next_reader_test':'blinded complete sentence versus shuffled controls'},'status':'fresh exact >38 candidate requires human reading' if exact else 'no fresh exact >38 candidate','next_construction':'extend relative-clause inventory only after an exact closure appears'}
