...
It branches directly from the named 568 incumbent at matched parent sentence
cuts 64/504, adds 72 letters, and records the 36-character live residual,
grammar owners, and deterministic replay.
...
The separate **672-letter** `discourse-linked-reverse-chain-672` in
`runs/incumbent-672-discourse-linked-reverse-chain-20260922.json` is an older
...
## 686-letter repeated-shell repair (2026-09-23)

The 640 child exposed a second 27-letter repeated shell at mirrored supports
`[100,127)` / `[513,540)`. A deterministic grammar chart replaced each support
with a 50-letter connected event cycle. The left cycle is “Leon sees Sara.
Sara sees Aidan. Aidan sees Nora. Nora sees Leon.”; the reversed character
obligation resegmented as “Noel sees Aron. Aron sees Nadia. Nadia sees Aras.
Aras sees Noel.” The 50-character residual closed with zero contradictions.
The exact child is 686 letters, +46 over its 640 parent, SHA-256
`cb47534ec3f66a5a3624e7573e15cde7c1cdc53c7ff9cebdc788412b940f982c`. The
...
- **Endpoint-conditioned compositional decoder, 2026-09-20:** paired fresh
  lexical endpoints before expanding independent interiors, so endpoint
  choices—not a post-hoc repair—controlled the outer equation. It rendered 72
  intact candidates up to 79 letters, including “A patient cartographer
  sketches distant hills; answers a folded letter beside a small veranda.”
  The matched `a` endpoint passed, but the next character failed (`p` versus
  `d`); there were 0 fresh exact candidates above 38. The next construction is
  an agreement-typed endpoint schema with held-out interiors. Artifact:
  `runs/endpoint-conditioned-compositional-decoder-20260920.json`.

- **Center-first scene grammar, 2026-09-20:** selected a semantic center
  before lexical outward growth and expanded paired scene arms under live
  obligations. Three centers produced nine bounded transitions and three
  complete prose controls at 58--66 letters; the longest was “In autumn Nell
  entered; the apples ripened; while the keeper counted the baskets.” Its
  first outer comparison failed (`i` versus `s`), with 0 exact candidates
  above 38. The next operator is a two-clause event bridge with an
  obligation-indexed semantic phrase bank. Artifact:
  `runs/center-first-scene-grammar-20260920.json`.

- **Character-LM grammar-constrained lane, 2026-09-20:** used a five-gram
  character model only to order emissions inside independently authored typed
  SVO clauses while a live cross-boundary residual remained hard. Sixteen
  clause pairs were tested and all 16 pruned before closure; there were 0 live
  closures and 0 exact candidates above 38. An intact 76-letter control was
  “A careful sailor studies the northern current; the northern current tests
...
- **Word-boundary grammar automaton, 2026-09-20:** selected ordinary lexical
  edges jointly with complete agent/event/theme frames while consuming
  character equations across word boundaries. Semordnilap words were ordinary
  alternatives, not mirrored units. Four fresh frames incurred 2,048 live
  boundary prunes and retained 20 grammatical near-misses, the longest 51
  letters (`The patient gardener records the patient diaper before dusk.`);
  0 exact candidates above 38 survived. Pointer/SHA audits were recorded for
  every near-miss. The next topology adds an optional relative-clause slot
  while carrying its event role through the same boundary equation. Artifact:
  `runs/word-boundary-grammar-automaton-20260920.json`.

- **Packed single-sentence grammar, 2026-09-20:** moved from frame products
  to one acyclic forward grammar trie. Paired grammar states consumed matching
...
- **Object-relative boundary topology, 2026-09-20:** added an indexed object
  relative alongside the subject-relative option, carrying finite-event and
  3sg agreement state through the complete semantic frame. Four frames incurred
  18,432 live boundary prunes and retained 20 grammatical near-misses to 89
  letters, including `The patient gardener who records the note records the
  patient diaper that the gardener guards before dusk.`; 0 exact candidates
  above 38 survived. The next construction coordinates object relatives with
  distinct attachment indices. Artifact:
  `runs/word-boundary-object-relative-automaton-20260920.json`.

- **Coordinated object-relative topology, 2026-09-20:** carried two finite
  relative events with distinct attachment indices and 3sg agreement inside
  the object noun phrase. Four frames incurred 36,864 live boundary prunes and
  retained 20 complete near-misses to 112 letters, including `The patient
  gardener who records the note records the patient diaper that the scribe
  records and that the gardener guards before dusk.`; 0 exact candidates above
  38 survived. The next construction adds an explicit attachment graph and
  tense state. Artifact:
  `runs/word-boundary-coordinated-object-relatives-20260920.json`.

- **Modal passive embedding, 2026-09-20:** added modal scope before the
  passive beneficiary/agent/theme frame and finite complement. Across target
  lengths 44/56/68/80 it visited 1,454 states and made 772 live factor prunes,
  with 0 complete parses and 0 exact candidates above 38. Two modal passive
  controls (94 and 103 letters) passed independent pointer/SHA audits. The next
  topology is an aspectual modal passive frame. Artifact:
  `runs/modal-passive-embedding-csp-20260920.json`.

- **Aspectual modal-passive topology, 2026-09-20:** added perfect aspect
  before the modal passive beneficiary/agent/theme frame and finite complement.
  Across target lengths 44/56/68/80 it visited 1,390 states and made 532 live
  factor prunes, with 0 complete parses and 0 exact candidates above 38. Two
  aspectual controls (100 and 109 letters) passed independent pointer/SHA
  audits. The next topology is progressive modal passive embedding. Artifact:
...
- **Habitual modal-passive topology, 2026-09-20:** introduced a typed
  `usually-being` habitual state before the modal passive beneficiary/agent/
  theme frame. Across target lengths 44/56/68/80 it visited 1,192 states and
  made 420 live factor prunes, with 0 complete parses and 0 exact candidates
  above 38. The next construction is iterative modal passive. Artifact:
  `runs/habitual-modal-passive-csp-20260920.json`.

- **Two-index relative attachment author, 2026-09-20:** carried two separately
  indexed relative attachments with explicit antecedent and tense features,
  filtering subject-number agreement before rendering. It retained 16 scenes
  from six relative-pair alternatives, with a 93-letter diagnostic and 0 exact
...
- **Conditional connector graph, 2026-09-20:** added a typed conditional
  relation to the concessive attachment graph. Four frames incurred 1,474,560
...
- **Frequentative modal-passive topology, 2026-09-20:** introduced a typed
  `often-being` frequentative state before the modal passive
  beneficiary/agent/theme frame. Across target lengths 44/56/68/80 it visited
  1,278 states and made 460 live factor prunes, with 0 complete parses and 0
  exact candidates above 38. The next construction is distributive modal
  passive. Artifact: `runs/frequentative-modal-passive-csp-20260920.json`.

- **Width-two relative residual buffer, 2026-09-20:** carried two-character
  residual buffers across two indexed relative boundaries and checked surface
  attachment semantics. All 81 transitions were pruned at the strict width-two
  gate; complete controls reached 94 letters, with 0 exact candidates above 38.
  The next construction increases the buffer to width three with typed roles.
  Artifact: `runs/multichar-relative-residual-buffer-20260920.json`.

- **Since/while duration connector graph, 2026-09-20:** added typed durative
  `since`/`while` relations to the temporal-until attachment graph. Four
  frames incurred 2,359,296 live boundary prunes and retained 20 ordinary-prose
  near-miss diagnostics to 186 letters; 0 exact candidates above 38 survived.
  The next construction adds a causal-subordinator connector edge. Artifact:
  `runs/word-boundary-since-while-graph-20260920.json`.

- **Causal-subordinator connector graph, 2026-09-20:** added a typed `as`-
  clause relation to the since/while attachment graph. Four frames incurred
  2,654,208 live boundary prunes and retained 20 ordinary-prose near-miss
  diagnostics to 186 letters; 0 exact candidates above 38 survived. The next
  construction adds a concessive-subordinator edge. Artifact:
  `runs/word-boundary-causal-subordinator-20260920.json`.

- **Distributive modal-passive topology, 2026-09-20:** introduced a typed
  `each-being` distributive state before the modal passive
  beneficiary/agent/theme frame. Across target lengths 44/56/68/80 it visited
  1,336 states and made 496 live factor prunes, with 0 complete parses and 0
  exact candidates above 38. The next construction is quantified modal
  passive. Artifact: `runs/distributive-modal-passive-csp-20260920.json`.

- **Width-three relative-role buffer, 2026-09-20:** carried width-three
  residuals with typed agent/theme roles and tense/number states across two
  indexed attachments. All 81 transitions were pruned at the strict gate;
...
- **Generic modal-passive baseline, 2026-09-20:** added a fresh typed
  `may-being` modal state before the beneficiary/agent/theme frame. Across
  target lengths 44/56/68/80 it visited 1,402 states and made 532 live factor
  prunes, with 0 complete parses and 0 exact candidates above 38. The next
  construction is deontic modal passive. Artifact:
  `runs/generic-modal-passive-baseline-csp-20260920.json`.

- **Role/attachment-indexed cross-boundary trie, 2026-09-20:** indexed residual
  keys by semantic role and attachment index across two relative boundaries.
  It retained 81 prose states, 162 keys, and 81 key prunes, with a 92-letter
  diagnostic and 0 exact candidates above 38. The next construction joins keys
  through a multi-step trie with partial compatibility. Artifact:
  `runs/role-indexed-crossboundary-trie-20260920.json`.

- **Conditional-subordinator connector graph, 2026-09-20:** added a typed
  `unless`-clause relation to the concessive-subordinator graph. Four frames
  incurred 3,244,032 live boundary prunes and retained 20 ordinary-prose
...
- **Constructive semordnilap scene grammar, 2026-09-20:** embedded five
  ordinary lexical edges independently into scene roles, producing 135
  complete diagnostics to 71 letters and 0 exact candidates above 38. The
  next construction expands the edge bank with valency-typed finite clauses.
  Artifact: `runs/constructive-semordnilap-scene-20260920.json`.

- **Held-out semordnilap valency search, 2026-09-20:** jointly selected five
  held-out pair edges under three transitive valency/attachment states. It
  retained 40 complete controls to 77 letters, with 0 exact candidates above
  38. The next construction adds a ditransitive recipient frame. Artifact:
  `runs/heldout-semordnilap-valency-search-20260920.json`.

- **Synchronous typed clause growth, 2026-09-20:** alternated clause and
  adjunct growth from opposing frontiers through depth four, retaining four
  intact controls to 96 letters after six live seam prunes and 0 exact
  candidates above 38. The next construction carries agreement and
  attachment state across the alternating growth. Artifact:
  `runs/synchronous-clause-growth-20260920.json`.

- **Typed agreement-aware adjunct growth, 2026-09-20:** carried subject
  number, tense, and PP attachment through the same depth-four alternating
  growth. It retained four intact controls to 96 letters after six seam
  prunes, with 0 exact candidates above 38. The next construction adds
  held-out tense/agreement growth. Artifact:
  `runs/typed-agreement-adjunct-growth-20260920.json`.

- **Two-clause semordnilap valency grammar, 2026-09-20:** selected two
  independently authored finite clauses with explicit transitive/intransitive
  states and four ordinary lexical edges. It produced 108 complete scenes to
  82 letters, with 0 exact candidates above 38. The next construction adds
  tense agreement and clause complements. Artifact:
  `runs/semordnilap-two-clause-valency-20260920.json`.

- **Held-out ditransitive semordnilap valency, 2026-09-20:** added a recipient
  role and a second lexical edge to five held-out pair edges. It tested 25
  dual-edge states, made 1,350 live prunes, and retained 40 controls to 135
...
- **Author-first semantic lattice, 2026-09-20:** selected a complete scene
  frame before lexical realization and consumed a live two-sided character
  equation during selection. It explored 27 semantic states, retained prose
  diagnostics up to 53 letters, and found 0 exact candidates above 38. The
  next construction carries a two-character residual through number,
  subject, verb, object, and setting states. Artifact:
  `runs/semantic-lattice-live-equation-author-20260920.json`.

- **Shared-scene online clause matching, 2026-09-20:** grew independent
  subject/verb/object clauses from a fresh semantic-role graph while matching
  characters online. After correcting an initial duplicated-clause control,
  the rerun checked 243 online prunes and retained 20 non-repeated controls up
  to 53 letters, with 0 exact candidates above 38. The next construction adds
  a second independently typed event. Artifact:
  `runs/shared-scene-online-clause-match-20260920.json`.

- **Full-clause compositional obligation CSP, 2026-09-20:** factorized both
  ordinary clauses into independent subject, verb, object, and locative slots
  while carrying live word-boundary obligations. The bounded run visited 5,000
  states, pruned 4,499, and reached no complete two-sided parse; its longest
  intact control was 108 letters and exact count above 38 was 0. The next
...
- **Valency attachment topology, 2026-09-20:** added held-out benefactive and
  locative attachment frames with agreement state to the independent
  full-clause factorization. The bounded run visited 5,000 states, pruned
  4,499, and reached no complete two-sided parse; its longest intact control
  was 108 letters and exact count above 38 was 0. The next construction adds
  instrument and source attachment frames. Artifact:
  `runs/valency-attachment-topology-csp-20260920.json`.

- **Asynchronous width-two semantic slots, 2026-09-20:** carried width-two
  buffers asynchronously across number-agreeing subject, verb, object, and
  typed adjunct slots. It explored 48 scene states and 144 boundary transitions,
  retaining diagnostics up to 58 letters with 0 exact candidates above 38. The
  next construction permits unequal slot lengths with residual carryover.
  Artifact: `runs/async-width2-semantic-slots-20260920.json`.

- **Two-event order and tense matching, 2026-09-20:** added two explicit tense
  orders to the independently typed two-event scene graph. It checked 486
  online prunes and retained 20 complete controls up to 65 letters, with 0
...
- **Aspect-conditioned clause topology, 2026-09-20:** added held-out
  aspect/voice alternations with agreement state to the independent full-clause
  factorization. The bounded run visited 5,000 states, pruned 4,499, and
  reached no complete two-sided parse; its longest intact control was 108
  letters and exact count above 38 was 0. The next construction adds
  polarity/aspect alternations. Artifact:
  `runs/aspect-conditioned-clause-csp-20260920.json`.

- **Live valency character residual, 2026-09-20:** checked a live character
  residual while selecting transitive-theme and intransitive-location
  complements. It evaluated 36 alternatives, pruned 36, and retained controls
  up to 64 letters, with 0 exact candidates above 38. The next construction
  carries variable-length residuals with relation-specific adjuncts. Artifact:
  `runs/valency-live-character-residual-20260920.json`.

- **Quotation/reporting scope, 2026-09-20:** added quotation/reporting states
  and hard negative-polarity agreement to the source/negation graph. It checked
  31,104 online prunes and retained 20 complete controls up to 112 letters,
  with 0 exact candidates above 38. The next construction adds quotation
...
- **Concessive-result graph edge, 2026-09-21:** added the distinct
  `event.kept.result -> concession` attachment after the resultative frontier
  survived its first support conflict. Eight trees produced 56 incrementally
  checked graph edges and 12 intact controls (maximum 135 letters), with zero
  exact closures at or above 39 letters. The next operator is a conditional-
...
- **Temporal-result graph edge, 2026-09-21:** added the distinct
  `event.found.result -> time` attachment after the conditional frontier
  survived its first support conflict. Ten trees produced 90 incrementally
  checked graph edges and 12 intact controls (maximum 135 letters), with zero
  exact closures at or above 39 letters. The next operator is a causal-result
  edge only if this temporal frontier likewise survives. Artifact:
  `runs/graph-unification-attachment-20260921.json`.

- **Causal-result graph edge, 2026-09-21:** added the distinct
  `event.kept.result -> cause` attachment after the temporal frontier survived
  its first support conflict. The incremental run now covers 11 trees and 110
  graph edges with 161 online support checks, retaining 12 intact controls to
  135 letters and zero exact closures at or above 39 letters. The next
...
- **Inferential-result graph edge, 2026-09-21:** added the distinct
  `event.marked.result -> inference` attachment after the evidential frontier
  survived its first support conflict. Thirteen trees produced 156
  incrementally checked graph edges and 12 intact controls (maximum 136
  letters), with zero exact closures at or above 39 letters. The next
  operator is an epistemic-result edge only if this inferential frontier
...
- **Six-event bridge-cycle graph, 2026-09-21:** added interacting focus
  bridges `e2.ward -> e4.gate` and `e4.gate -> e6.report`, plus the cycle basis
  `e1 -> e3 -> e5 -> e1`, under the global endpoint CSP. One 135-letter intact
  control produced four online checks and zero exact closures above 38. The
  next topology is a seven-event graph with three focus bridges and a cycle-
...
- **Plural progressive Earley frame, 2026-09-21:** added pre-render
  ...
  (148--152 letters) produced 202 concrete domain conflicts and zero exact
...
- **Held-out `are ready to V` Earley frame, 2026-09-21:** added the new
  Aux+Adj+Infinitive VP topology before rendering, preserving plural
  agreement, locative valency, and live mirrored domains. Three intact
  controls were 140, 144, and 140 letters; 192 concrete domain conflicts were
  retained and zero exact closures occurred. The next operator is a held-out
  `are willing to V` frame. Artifact:
  `runs/cfg-earley-are-ready-frame-20260921.json`.

- **Held-out `are willing to V` Earley frame, 2026-09-21:** added the
  distinct Aux+Adj+Infinitive topology before rendering, preserving plural
  agreement, locative valency, and live mirrored domains. Three intact
  controls were 144, 148, and 144 letters; 199 concrete domain conflicts were
  retained and zero exact closures occurred. The next operator is a held-out
