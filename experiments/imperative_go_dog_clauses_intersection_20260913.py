"""Full connected-tree scheduler for an explicit imperative action list.

The ordinary imperative ends in ``the hungry dog``.  Its final ``dog`` is a
longer lexicon-backed endpoint whose reverse begins the opening ``go do``;
the rest of the length comes from five explicit action/object clauses.
"""
...
spec = importlib.util.spec_from_file_location("go_dog_engine_20260913", ENGINE_PATH)
...
BASE = ENGINE.BASE; ENGINE.FIXED_LABELS.update({"fixed_go", "fixed_do", "fixed_carry", "fixed_close", "fixed_wash", "fixed_paint", "fixed_and", "fixed_feed", "fixed_the", "fixed_work", "fixed_basket", "fixed_gate", "fixed_dish", "fixed_sign", "fixed_dog", "fixed_hungry", "fixed_sturdy", "fixed_wooden"})
from llm_palindrome.admission import mechanical_admission_checks

MIN_LETTERS, MAX_LETTERS = 100, 180
FRAMES = (("work", "careful"), ("basket", "clean"), ("gate", "sturdy"), ("dish", "small"), ("sign", "plain"))
...
class GoDogGrammar:
    ...
    def productions(self, lhs):
        if lhs.name == "D":
            rhs = [BASE.sym("T", label="fixed_go", form="go"), BASE.sym("T", label="fixed_do", form="do")]
            for i, (noun, adj) in enumerate(FRAMES, 1):
                if i > 1: rhs.append(BASE.sym("T", label=f"fixed_and_{i}", form="and"))
                verb = ("carry", "close", "wash", "paint")[i - 2] if i > 1 else None
                if verb: rhs.append(BASE.sym("T", label=f"fixed_{verb}", form=verb))
                rhs.append(BASE.sym("NP", role=f"object_{i}", frame=noun, type="object", number="sing", depth="0"))
            rhs.extend((BASE.sym("T", label="fixed_and_6", form="and"), BASE.sym("T", label="fixed_feed", form="feed"), BASE.sym("T", label="fixed_the", form="the"), BASE.sym("T", label="fixed_hungry", form="hungry"), BASE.sym("T", label="fixed_dog", form="dog")))
            return (BASE.Production("D:go-do-five-actions-and-feed-hungry-dog", lhs, tuple(rhs)),)
        if lhs.name == "NP":
...
def semantic_witness(tree):
    ...
    if tree: visit(tree)
    if frames != [x[0] for x in FRAMES]: valency = False
    root_ok = bool(tree and tree.children[0].terminal == "go" and tree.children[1].terminal == "do" and tree.children[-1].terminal == "dog")
...
def explicit_control(grammar):
    ...
    while True:
        ...
        if symbol.name == "D": ident = "D:go-do-five-actions-and-feed-hungry-dog"
        elif symbol.name == "NP":
...
def boundary_preflight():
    grammar = GoDogGrammar(); state = explicit_control(grammar); events = []; failed = None
...
def run(max_states=100000):
    grammar = GoDogGrammar(); result = solver(grammar, max_states); control_state = explicit_control(grammar); control = audit(grammar, BASE.render(control_state), "complete_explicit_imperative_control", control_state.trace); control.update({"diagnostic_only": True, "grammar_tree_fully_expanded": True, "provenance": {"construction": "five explicit imperative action/object clauses", "shared_tree": True}, "reader_status": "grammar control only; not a palindrome candidate or readability evidence"})
    return {"status": "imperative_go_dog_clauses_intersection", "config": {"min_letters": MIN_LETTERS, "max_letters": MAX_LETTERS, "max_depth": 0, "max_states": max_states, "single_shared_derivation_tree": True, "character_residual_during_derivation": True, "outer_boundary_crossing": "go is matched through longer dog and into do", "explicit_clause_count": 5, "recursive_padding": False, "closure_requires_all_leaves_consumed": True, "independent_complete_reparse": True, "corpus_generation": False}, "provenance": {"generator_sha256": sha256(Path(__file__).read_bytes()).hexdigest(), "grammar_sha256": grammar.digest(), "material": "authored ordinary imperative action clauses and typed object frames; no catalogue or known palindrome material"}, "complete_recursive_control": control, **result, "joint_boundary_preflight": boundary_preflight(), "reader_facing_next_operator": "Use the replayed deepest contradiction to alter the exact exposed clause boundary, then rerun the full scheduler.", "scope": "Zero-closure bounded run is diagnostic; no reader-facing palindrome is claimed."}
