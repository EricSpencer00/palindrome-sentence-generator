...
def main():
    ...
    while frontier and expanded < 512:
        li, ri, lp, rp, left, right, path = frontier.pop(0); expanded += 1
        if li == len(SLOTS) and ri == len(SLOTS):
            if len(left) == len(right) and left == right[::-1]:
                terminals.append((left, right, path))
            continue
        if len(samples) < 32:
        ...
        for side, idx, prefix, tape in (('L', li, lp, left), ('R', ri, rp, right)):
            ...
            for c in next_chars:
                ...
                # Only compare a newly exposed opposing position; unequal tape
                # lengths are expected while the two lexical tries interleave.
                if side == 'L':
                    k = len(nleft) - 1 - len(nright)
                    bad = k >= 0 and bool(nright) and nleft[k] != nright[-1]
                else:
                    k = len(nright) - 1 - len(nleft)
                    bad = k >= 0 and bool(nleft) and nright[k] != nleft[-1]
                if bad:
                    pruned += 1; continue
                ...
                frontier.append((nl, nr, nlp, nrp, nleft, nright,
                                 path + ((side, name, c),)))
    rendered = []
    for left, right, path in terminals[:4]:
        text = left + right[::-1]
        tape = letters(text)
        rendered.append({'text': text, 'length': len(tape), 'audit': audit(tape),
                         'provenance': path, 'novelty_signature': 'functionword-live-trie-20260917',
    ...
    payload = {'experiment': 'semantic-valency-trie-nfa-functionwords-20260917',
               ...
               'terminal_paths': [{'left': a, 'right': b, 'path': p} for a, b, p in terminals[:4]],
