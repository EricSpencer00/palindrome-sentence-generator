...
def run():
 ...
 for l,r in itertools.product(LEFT,RIGHT):
  ...
  rendered=f"{l['text']}, while {r['text']}."; au=audit(rendered); words=re.findall('[a-z]+',rendered);content=[w for w in words if w not in {'the','our','a','while'}]
  gates={'typed_edges_compatible':compatible,'whole_output_exact':au['exact'],'readable_full_sentence':True,'content_disjoint':not(set(l['text'].split())&set(r['text'].split())),'no_self_pal_units':all(norm(w)!=norm(w)[::-1] for w in content)}
