"""Natural grammar-guidance frame with a no-repeated-content topology.

The method starts from a complete ordinary sentence rather than a center noun:
grammar guides writers through an exercise while editors revise technical prose.
The internal ``gram|mar`` pivot is derived only after that frame is fixed.  A
single expanded tree owns every surface leaf, and its frontier scheduler emits
one character at a time with independent replay and central admission.
"""
...
LEXICON = (
    Lexeme("clear", "adj_center", "grammar"), Lexeme("proper", "adj_center", "grammar"),
    Lexeme("grammar", "noun_center", "grammar"), Lexeme("guides", "verb_intro", "guidance"),
    Lexeme("patient", "adj_person", "writer"), Lexeme("receptive", "adj_person", "writer"),
    Lexeme("writers", "noun_person", "writer"), Lexeme("through", "prep", "path"),
    Lexeme("a", "det", "indefinite"), Lexeme("difficult", "adj_event", "exercise"),
    Lexeme("exercise", "noun_event", "exercise"), Lexeme("while", "conj", "contrast"),
    Lexeme("careful", "adj_person", "editor"), Lexeme("editors", "noun_person", "editor"),
    Lexeme("revise", "verb_record", "revision"), Lexeme("complex", "adj_artifact", "prose"),
    Lexeme("technical", "adj_artifact", "prose"), Lexeme("prose", "noun_artifact", "prose"),
...
class Grammar:
    def productions(self, s: Symbol) -> tuple[tuple[Symbol, ...], ...]:
        if s.name == "S":
            return ((T("adj_center", "center_adj"), T("noun_center", "center_word"), T("verb_intro", "predicate"), T("adj_person", "object_adj"), T("noun_person", "object_noun"), Symbol("PP", "path", "path_pp"), T("conj", "subordinator"), T("adj_person", "second_adj"), T("noun_person", "second_noun"), T("verb_record", "second_predicate"), T("adj_artifact", "object_adj1"), T("adj_artifact", "object_adj2"), T("noun_artifact", "object_noun2"), T("adv", "today")),)
        if s.name == "PP": return ((T("prep", s.role + "_prep"), T("det", s.role + "_det"), T("adj_event", s.role + "_adj"), T("noun_event", s.role + "_noun")),)
        return ()
...
FRAME = {"center": "grammar", "pivot": "gram|mar", "predicate": "guides", "center_adjectives": {"clear", "proper"}, "object_adjectives": {"patient", "receptive"}, "object_noun": "writers", "path": ("through", "a", "difficult", "exercise"), "subordinator": "while", "second_subject": ("careful", "editors"), "second_predicate": "revise", "second_object": ("complex", "technical", "prose"), "today": "today"}


def semantic_choice(slot: Slot, item: Lexeme) -> bool:
    allowed = {
        "center_adj": FRAME["center_adjectives"], "center_word": {FRAME["center"]}, "predicate": {FRAME["predicate"]},
        "object_adj": FRAME["object_adjectives"], "object_noun": {FRAME["object_noun"]},
        "path_prep": {FRAME["path"][0]}, "path_det": {FRAME["path"][1]}, "path_adj": {FRAME["path"][2]}, "path_noun": {FRAME["path"][3]},
        "subordinator": {FRAME["subordinator"]}, "second_adj": {FRAME["second_subject"][0]}, "second_noun": {FRAME["second_subject"][1]},
        "second_predicate": {FRAME["second_predicate"]}, "object_adj1": {FRAME["second_object"][0]}, "object_adj2": {FRAME["second_object"][1]}, "object_noun2": {FRAME["second_object"][2]}, "today": {FRAME["today"]},
    }
    return item.word in allowed.get(slot.symbol.role, set())


def render(words: list[str]) -> str:
    return " ".join(words[:9]).capitalize() + " " + " ".join(words[9:]) + "."
...
def search(grammar: Grammar, leaf_slots: tuple[Slot, ...], *, state_limit: int, closure_limit: int, stats: Counter):
    ...
    def visit(left_next, right_next, left_stream, right_stream, assigned, residual, owner, last_side, ledger):
        ...
        for side in sides:
            ...
            event["residual_after"] = residual2; new_stream = (index, chars, position + 1)
    ...
    center_item = next(item for item in BY_KIND["noun_center"] if item.word == center_word); words[center] = center_word; assigned = {leaf_slots[center].symbol.role: center_item}; stats["lexical_assignments_considered"] += 1; visit(center - 1, center + 1, (center, normalize_letters(center_word[:pivot])[::-1], 0), (center, normalize_letters(center_word[pivot:]), 0), assigned, "", "", "left", []); words[center] = ""
...
def run(*, state_limit: int = 100_000, closure_limit: int = 100) -> dict[str, object]:
    ...
    frames = []
    for center_adj in ("clear", "proper"):
        for writer_adj in ("patient", "receptive"):
            frames.append({"rendered": f"{center_adj.capitalize()} grammar guides {writer_adj} writers through a difficult exercise while careful editors revise complex technical prose today.", "center": FRAME["center"], "pivot": FRAME["pivot"], "predicate": FRAME["predicate"], "semantic_status": "complete ordinary teaching/editing event fixed before center search"})
