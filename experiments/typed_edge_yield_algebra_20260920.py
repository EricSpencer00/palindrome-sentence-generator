...
def run() -> dict:
    ...
    return {
        ...
        "next_construction": {
            "method": "discontinuous typed-edge expansion",
            "operator": "split one semantic edge into head and argument yields with a live attachment interface; select the next fragment only when its first character satisfies the opposing residual",
            "reason": "all complete four-edge paths fail at the outer character, so another complete-edge product would repeat the same dead frontier",
            "constraints": ["ordinary-order rendering", "agreement and attachment types carried across the split", "no finished-tape reversal", "no post-hoc repair", "no repeated or self-palindromic content unit"],
        },
