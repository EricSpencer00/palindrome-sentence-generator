"""Bridge-conjunction complement selection with an attached relative clause."""
...
OUT = ROOT / "runs" / "bridge-complement-relative-orbit-20260920.json"
EXPERIMENT_ID = "bridge-complement-relative-orbit-20260920"
...
def fs(role: str, clause: str, valency: str, *texts: str,
       complement: str | None = None, number: str | None = None,
       agreement: str | None = None) -> tuple[Frame, ...]:
    return tuple(Frame(role, text, clause, valency, complement, number, agreement) for text in texts)


def build_lattice() -> tuple[tuple[Frame, ...], ...]:
    # Complete rendering: main S V O, bridge, finite complement S [relative]
    # V O.  The bridge selects its complement type before expansion.
    return (
        fs("subject", "main", "agent", "the steward", "a raven", "the poet", number="singular"),
        fs("event", "main", "transitive", "keeps the vow", "marks the page", "kept the vow", agreement="singular"),
        fs("object", "main", "patient", "a silver cup", "the quiet bell", "one true word"),
        fs("bridge", "bridge", "subordinator", "while", "though", "when", complement="finite"),
        fs("object", "subordinate", "patient", "the winter seal", "a hidden key", "one pale crown"),
        fs("event", "subordinate", "transitive", "guard the gate", "name the heir", "guarded the gate", agreement="plural"),
        fs("relative", "subordinate-relative", "subject-modifier", "who sings", "that waits", "who remembers", agreement="plural"),
...
def run(*, state_limit: int = 250_000) -> dict[str, object]:
        ...
        if lo > hi:
            ...
            if checked["exact"]:
                candidates.append({"rendered": rendered, "audit": checked,
                    "provenance": {"construction": "bridge complement selection with attached relative",
                        "roles": [x.role for x in ordered], "clauses": [x.clause for x in ordered],
                        "valencies": [x.valency for x in ordered],
                        "complements": [x.complement for x in ordered],
        ...
        for lframe in lattice[lo]:
            for rframe in lattice[hi]:
                ...
                if rframe.role in {"event", "relative"} and rframe.agreement != next_state.get("sub_number"):
                    feature_pruned += 1
                    continue
                if lframe.role == "bridge" and lframe.complement != "finite":
                    feature_pruned += 1
                    continue
    ...
    result = {"experiment": EXPERIMENT_ID,
              "method": "bridge finite-complement selection with subordinate relative",
              ...
              "provenance": {"complete_main_clause": True, "complete_finite_complement": True,
                  "bridge_complement_selected_before_expansion": True,
                  "attached_relative_clause": True, "agreement_state_before_emission": True,
                  "held_out_subordinate_lexicalizations": True,
                  "finished_tape_reversal": False, "post_hoc_repair": False,
                  "catalogue_text": False, "aligned_token_mirror": False,
                  "next_construction": "condition bridge complement selection on tense and mood before the seam"}}
