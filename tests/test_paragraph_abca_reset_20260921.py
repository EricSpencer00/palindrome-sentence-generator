...
def test_fresh_abca_topology():
 d=m.run(); assert d['stats']['candidates']==1; r=d['actual_paragraph_candidates'][0]
...
def test_live_exact_and_novelty_audits():
 d=m.run(); assert len({r['audit']['sha256_forward'] for r in d['rendered_outputs']})==1; a=d['rendered_outputs'][0]['audit']
