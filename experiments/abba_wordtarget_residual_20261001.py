...
def residual(left: str, right: str) -> dict:
 ...
 return {"supported_depth":d, "left_residual":l[d:d+18],
  ...
  "word_target":target.group(1) if target else "", "next_left":l[d] if d<len(l) else None,
...
def run():
 ...
 best=max(rows,key=lambda x:x["word_targeting"]["residual"]["supported_depth"])
 return {"experiment_id":"abba-wordtarget-residual-20261001",
  ...
  "stats":{"frames":len(FRAMES),"b2_tails":len(B2_TAILS),"a2_variants":len(A2S),
   "joint_candidates":len(rows),"exact_gt38":len(exact),
   "max_supported_depth":best["word_targeting"]["residual"]["supported_depth"]},
  "exact_candidates":exact,"best_frontier":best,"rendered_candidates":rows[:10],
  ...
  "conclusion":"Jointly preselected ordinary B2 openings preserve ordinary prose but no exact closure above 38 letters; the seam still dies before a complete target word can be discharged, and its residuals are retained for the next operator.",
