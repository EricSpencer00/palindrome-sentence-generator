...
ROOT=Path(__file__).resolve().parents[1];ID='function-word-auxiliary-single-repair-20260916';SIGNATURE='function-word-auxiliary-repair|past-participle-fixed|scene-fixed|single-state|fresh'
TEXT='The careful archivist will carry the ledger, and a patient porter had stored it beside the northern shelves.'
def audit(s):
...
def main():
 ...
 row={'repair_state':'single second-clause auxiliary','parent_state':'function-word-boundary-single-repair-20260916','change':{'old':'has stored','new':'had stored'},'past_participle_fixed':'stored','scene_fixed':True,'agreement_preserved':True,'semantic_consistency':True,'audit':audit(TEXT),'next_repair':{'operator':'change only the first-clause modal while preserving infinitive and both event roles','reason':'auxiliary repair remains grammatical but leaves nonzero character residual'}}
 out={'experiment_id':ID,'signature':SIGNATURE,'status':'completed_no_exact_closure','reader_eligible':False,'method':'single auxiliary substitution with past participle, agreement, and scene held fixed','candidates':[row],'stats':{'rendered':1,'exact':0},'next_repair':row['next_repair'],'novelty_preflight':{'registry_entries_read':len(allr),'exact_signature_collision':False,'catalogue_text_imported':False,'fixed_tape_used':False,'sweep':False,'prior_state_replayed':False},'provenance':{'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'lexical_source':'held-out auxiliary choice','parent_state':'function-word-boundary-single-repair-20260916','repair':'has -> had; past participle stored and dependency roles unchanged','audits':['independent two-pointer','forward/reverse SHA-256','agreement replay','anti-shortcut']}}
