"""Natural-event-first internal-centre search over the connected tree scheduler.

Unlike the retired class variants, this experiment starts from complete discourse
frames: a researcher plans a brief meeting during a study, then documents the
report after the meeting.  Only after that semantic frame is fixed is ``meeting``
examined for word-internal pivots.  The inherited scheduler is hash-locked and
all bindings are restored after the run.
"""
...
def isolated_bindings():
    ...
    by_kind["noun_center"] += (base.Lexeme("meeting", "noun_center", "meeting"),)
    by_kind["adj_center"] += (base.Lexeme("brief", "adj_center", "meeting"),)
    by_kind["verb_intro"] += (base.Lexeme("planned", "verb_intro", "planning", "person", "meeting"),)
    by_kind["noun_event"] += (
        base.Lexeme("meeting", "noun_event", "event"),
        base.Lexeme("review", "noun_event", "event"),
    )
    base.BY_KIND = by_kind
    base.FRAME = {"meeting": {"verb": "planned", "adjectives": {"brief"}, "relations": {"during"}}}
    # The doubled e is discovered from the semantically selected center, rather
    # than copied from a prepared palindrome or a multiword seed.
    center = "meeting"
    pivots = tuple(i for i in range(1, len(center)) if center[i - 1] == center[i])
    if not pivots:
        raise RuntimeError("natural center has no internal pivot")
    base.PIVOTS = {center: pivots}
...
def run(*, state_limit: int = 100_000, closure_limit: int = 100) -> dict[str, object]:
    ...
    try:
        ...
        frames = [
            {
                "first_sentence": "The careful researcher planned a brief meeting during the annual study.",
                "second_sentence": "The careful researcher documented the annual report after the public meeting today.",
                "center": "meeting", "verb": "planned", "adjective": "brief",
                "relation": "during", "event_noun": "study",
                "semantic_status": "complete ordinary event/discourse pair fixed before center search",
            },
            {
                "first_sentence": "The patient analyst planned a brief meeting during the public review.",
                "second_sentence": "The patient analyst reviewed the public record after the annual meeting today.",
                "center": "meeting", "verb": "planned", "adjective": "brief",
                "relation": "during", "event_noun": "review",
                "semantic_status": "complete ordinary event/discourse pair fixed before center search",
            },
        ]
        ...
        for frame in frames:
            ...
            controls.append(base.audit(grammar, text, "complete_natural_event_pair_control",
        ...
        return {
            "status": "natural_event_pair_internal_center_typed_tree_search",
            "config": {
            ...
            "provenance": {
                ...
                "material": "task-authored complete natural meeting event pairs; no catalogue text",
            },
            "reader_facing_next_operator": "Derive the successor from this complete-frame scheduler ledger; do not patch a relation or revive the retired class family.",
