...
ROOT=Path(__file__).resolve().parent; OUT=ROOT/'runs/shared-scene-event-order-tense-20260920.json'
ID='shared-scene-event-order-tense-20260920'; SIG='shared-scene-graph|event-order|tense-compatibility|online-clause-match'
...
def run():
 ...
 return {'experiment_id':ID,'method':'shared scene graph grows two typed SVO events with explicit order and tense compatibility during online matching','stats':{'scene_frames':len(SCENES),'subject_edges':len(SUBJ),'verb_edges':len(VERBS),'object_edges':len(OBJS),'second_event_edges':len(RIGHT_SUBJ)*len(RIGHT_VERBS)*len(RIGHT_OBJS),'tense_orders':len(TENSES),'online_prunes':prunes,'rendered_controls':len(rows),'fresh_exact_gt38':len(exact),'max_letters':max((r['audit']['letters'] for r in rows),default=0)},'rendered_candidates':rows,'exact_candidates':exact,'novelty_preflight':{'status':'passed','signature':SIG,'distinct_from':'two-event lane without explicit event-order/tense state'},'next_topology':'add cross-event attachment scope and temporal connector state','status':'fresh exact >38 candidate requires human reading' if exact else 'no exact >38 closure; complete scene controls retained'}
