...
RUN=Path(__file__).parents[1]/'runs/grammar-boundary-transition-csp-20260917.json'
def test_bridge_csp_and_audits():
 d=json.loads(RUN.read_text());assert d['candidate_count']==len(d['candidates'])>0
 assert len(d['layers'])==9
 for r in d['candidates']:
  assert r['bidirectional_equalities']>=0 and r['audit']['exact']==r['audit']['independent_two_pointer'] and len(r['audit']['sha256'])==64
