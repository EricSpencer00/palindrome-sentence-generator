...
from experiments.centerout_grammar_boundary_repair2_20260916 import run, EXPERIMENT_ID

ROOT=Path(__file__).resolve().parents[1]

def test_outer_letter_directed_repair():
    p=run(); r=p["candidate"]
    assert p["stats"] == {"candidates":1,"exact":0,"mechanically_admitted":0}
    assert r["repair"]["new_final_letter"] == "t"
    ...
    assert r["rendered"].endswith("guest.")
