...
def test_clause_growth_has_real_prose_and_independent_rejection():
    ...
    assert x["stats"] == {"candidates": 4, "exact": 0, "admitted": 0}
    assert [len(c["frames"]) for c in x["candidates"]] == [1, 2, 3, 2]
    ...
    assert x["candidates"][-1]["provenance"]["repair_from"] == "growth-2-clause"
