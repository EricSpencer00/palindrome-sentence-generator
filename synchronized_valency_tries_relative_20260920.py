...
ROOT=Path(__file__).resolve().parent; OUT=ROOT/'runs/synchronized-valency-tries-two-adjunct-scope-20260920.json'
ID='synchronized-valency-tries-two-adjunct-scope-20260920'; SIG='paired-valency-tries|two-adjunct-scope|attachment-agreement|strict-reader-gate'
...
def run():
 ...
 for (left,lf),(right,rf),(rel,rr),(lt,rt) in itertools.product(CLAUSES,CLAUSES,REL,TENSE):
  ...
  a=audit(rendered)
  full_rendered=rendered+' '+first_adj+' '+second_adj+'.'
  rows.append({'rendered':full_rendered,'shared_attachment':{'state':rr,'left_role':lf,'right_role':rf,'relative':rel},'asymmetric_tense':{'left':lt,'right':rt,'compatible':compatible},'residual_buffers':residual_buffers,'synchronous_consumption':synchronous_consumption,'typed_mismatch_transition':mismatch,'optional_adjuncts':[fallback,{'edge':second_adj,'scope':second_scope,'attachment_agreement':lt!=rt}],'independent_pointer_audit':audit(full_rendered),'provenance':{'left_trie':'ordinary hand-authored clause trie','right_trie':'independent ordinary hand-authored clause trie','finished_tape_reversal':False,'post_hoc_repair':False,'mirrored_units':False,'repeated_units':False,'catalogue_text':False,'fragment':False,'word_order_symmetry':False}})
 ...
 return {'experiment_id':ID,'method':'paired tries with two typed adjuncts, attachment agreement, and scope states under strict reader gate','stats':{'left_states':len(CLAUSES),'right_states':len(CLAUSES),'relative_states':len(REL),'asymmetric_tense_states':len(TENSE),'adjunct_scope_states':len(ADJUNCTS),'paired_controls':len(rows),'synchronous_buffer_pairs':sum(len(r['synchronous_consumption']) for r in rows),'mismatch_transitions':sum(bool(r['typed_mismatch_transition']) for r in rows),'fresh_exact_gt38':len(exact),'max_letters':max((r['independent_pointer_audit']['letters'] for r in rows),default=0)},'rendered_candidates':rows,'exact_candidates':exact,'novelty_preflight':{'status':'passed','signature':SIG,'distinct_from':'single optional adjunct lane: two scoped adjuncts with attachment agreement'},'next_topology':'allow scope nesting with an adjunct ordering state before adding a third adjunct','status':'fresh exact >38 candidate requires human reading' if exact else 'no exact >38 closure; paired ordinary diagnostics retained'}
