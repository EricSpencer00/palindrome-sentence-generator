"""Subject-verb boundary seam state over two fixed ordinary-English frames."""
from __future__ import annotations
import hashlib,itertools,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/"runs/two-frame-subject-verb-boundary-seam-20260917.json";REG=ROOT/"docs/experiment-novelty-registry.json"
ID="two-frame-subject-verb-boundary-seam-20260917";SIG="two-fixed-typed-frames|subject-verb-boundary-state|same-ordinary-rendering|live-center-obligation|independent-exact-audit"
...
def emit(frame,seam):
 ...
 return {"rendered":text,"choices":{"frame":frame,"subject_verb_seam":seam,"fixed_states":FIXED},"audit":a,"center_state":{"midpoint":mid,"crossing":cross,"closed_pairs_before_first_mismatch":pairs,"live_debt":a["first_mismatches"][0] if a["first_mismatches"] else None,"subject_verb_boundary":{"token_sequence":[s,v],"seam_index":"before_verb" if seam=="before_verb" else "after_subject","rewrite_admitted":False}},"anti_shortcut_flags":{"finished_tape_reversal":False,"word_order_symmetry":False,"repeated_self_palindromic_unit":False,"catalogue_text":False,"punctuation_changes_letters":False,"fragment":False},"provenance":{"lexical_source":"same two fixed hand-authored frames","borrowed_text":False,"new_lexical_bank":False,"no_clause_or_attachment":True,"malformed_reordering_rejected":True}}
def run():
 pre=preflight();rows=[emit(f,s) for f,s in itertools.product(FRAMES,("before_verb","after_subject"))];rows.sort(key=lambda x:(x["audit"]["exact"],x["audit"]["letters"]),reverse=True);exact=[r for r in rows if r["audit"]["exact"]]
 return {"experiment_id":ID,"signature":SIG,"status":"completed_exact" if exact else "completed_no_exact_closure","method":"subject-verb boundary state with ordinary rendering and rejected rewrites","novelty_preflight":pre,"candidate_count":len(rows),"exact_count":len(exact),"reader_eligible":False,"rendered_candidates":rows,"stats":{"variants":len(rows),"longest_letters":max(r["audit"]["letters"] for r in rows),"midpoint_inside_token":sum(r["center_state"]["crossing"] is not None for r in rows)},"failure_and_repair":{"failure":"no exact closure" if not exact else "exact closure found","next_repair":"retire seam-only variants and test a new constructive center grammar with independent lexical obligations"},"provenance":{"generator_sha256":hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),"independent_audits":["two-pointer scan","forward/reverse SHA-256"],"shortcuts_excluded":True}}
