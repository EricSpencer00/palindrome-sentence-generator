...
HELDOUT_RIGHT_LOCATIVE_DETERMINERS = ("the", "a", "our")
CENTERS = (
...
def run() -> dict:
    ...
    # Keep determiner and head noun from the repaired object NP, replacing
    # only its adjective.  Use the two explicitly determiner-bearing source
    # objects; ``fresh route notes`` has no overt determiner and belongs to a
    # separate noun-boundary repair.
    adjective_repair_rows = []
    for rank, (left, center, punct, right, replacement_subject, replacement_object,
               replacement_place, replacement_verb, object_adj) in enumerate(
        itertools.islice(itertools.product(LEFT, CENTERS, PUNCTUATION, RIGHT,
                                            HELDOUT_RIGHT_AGREEMENT_SUBJECTS, HELDOUT_RIGHT_OBJECTS[:2],
                                            HELDOUT_RIGHT_LOCATIVES, HELDOUT_RIGHT_VERBS,
                                            HELDOUT_RIGHT_OBJECT_ADJECTIVES), 12), 1):
        object_words = replacement_object.split()
        determiner = object_words[0]
        head_noun = object_words[-1]
        repaired = (replacement_subject[0], replacement_subject[1], replacement_verb,
                    f"{determiner} {object_adj} {head_noun}", replacement_place)
        adjective_repair_rows.append(audit(left, center, punct, repaired, rank + 84,
                                           repair_stage="held_out_right_object_adjective",
                                           held_out_slot="right_object_adjective"))
    # Preserve the entire repaired object frame except its head noun.
    noun_repair_rows = []
    for rank, (left, center, punct, right, replacement_subject, replacement_object,
               replacement_place, replacement_verb, object_adj, object_noun) in enumerate(
        itertools.islice(itertools.product(LEFT, CENTERS, PUNCTUATION, RIGHT,
                                            HELDOUT_RIGHT_AGREEMENT_SUBJECTS, HELDOUT_RIGHT_OBJECTS[:2],
                                            HELDOUT_RIGHT_LOCATIVES, HELDOUT_RIGHT_VERBS,
                                            HELDOUT_RIGHT_OBJECT_ADJECTIVES, HELDOUT_RIGHT_OBJECT_NOUNS), 12), 1):
        object_words = replacement_object.split()
        determiner = object_words[0]
        repaired = (replacement_subject[0], replacement_subject[1], replacement_verb,
                    f"{determiner} {object_adj} {object_noun}", replacement_place)
        noun_repair_rows.append(audit(left, center, punct, repaired, rank + 96,
                                      repair_stage="held_out_right_object_head_noun",
                                      held_out_slot="right_object_head_noun"))
    # Preserve the locative frame (preposition, determiner, modifier) and
    # replace only its head noun.
    locative_noun_repair_rows = []
    for rank, (left, center, punct, right, replacement_subject, replacement_object,
               replacement_place, replacement_verb, object_adj, object_noun,
               place_noun) in enumerate(
        itertools.islice(itertools.product(LEFT, CENTERS, PUNCTUATION, RIGHT,
                                            HELDOUT_RIGHT_AGREEMENT_SUBJECTS, HELDOUT_RIGHT_OBJECTS[:2],
                                            HELDOUT_RIGHT_LOCATIVES, HELDOUT_RIGHT_VERBS,
                                            HELDOUT_RIGHT_OBJECT_ADJECTIVES, HELDOUT_RIGHT_OBJECT_NOUNS,
                                            HELDOUT_RIGHT_LOCATIVE_HEAD_NOUNS), 12), 1):
        place_words = replacement_place.split()
        repaired_place = " ".join(place_words[:-1] + [place_noun])
        object_words = replacement_object.split()
        repaired = (replacement_subject[0], replacement_subject[1], replacement_verb,
    ...
    locative_det_repair_rows = []
    for rank, (left, center, punct, right, replacement_subject, replacement_object,
               replacement_place, replacement_verb, object_adj, object_noun,
               place_noun, place_modifier, place_det) in enumerate(
        itertools.islice(itertools.product(LEFT, CENTERS, PUNCTUATION, RIGHT,
                                            HELDOUT_RIGHT_AGREEMENT_SUBJECTS, HELDOUT_RIGHT_OBJECTS[:2],
                                            HELDOUT_RIGHT_LOCATIVES, HELDOUT_RIGHT_VERBS,
                                            HELDOUT_RIGHT_OBJECT_ADJECTIVES, HELDOUT_RIGHT_OBJECT_NOUNS,
                                            HELDOUT_RIGHT_LOCATIVE_HEAD_NOUNS, HELDOUT_RIGHT_LOCATIVE_MODIFIERS,
                                            HELDOUT_RIGHT_LOCATIVE_DETERMINERS), 12), 1):
        place_words = replacement_place.split()
        repaired_place = " ".join(place_words[:1] + [place_det, place_modifier, place_noun])
        object_words = replacement_object.split()
        repaired = (replacement_subject[0], replacement_subject[1], replacement_verb,
                    f"{object_words[0]} {object_adj} {object_noun}", repaired_place)
        locative_det_repair_rows.append(audit(left, center, punct, repaired, rank + 132,
                                              repair_stage="held_out_right_locative_determiner",
                                              held_out_slot="right_locative_determiner"))
    ...
    return {"experiment": EXPERIMENT, "signature": SIGNATURE,
            ...
            "novelty_preflight": pre, "states_considered": 144, "candidate_count": len(rows),
            "exact_count": len(exact), "rendered_candidates": rows, "exact_survivors": exact,
            "repair_summary": {"method": "sequential_subject_object_locative_verb_agreement_determiner_adjective_noun_modifier_then_locative_determiner_holdout",
                               "preserved_center_event": True, "preserved_punctuation": True,
                               "held_out_slots": ["right_subject_np", "right_object_np", "right_locative", "right_verb_inflection", "right_determiner_agreement_subject", "right_object_determiner", "right_object_adjective", "right_object_head_noun", "right_locative_head_noun", "right_locative_modifier", "right_locative_determiner"],
                               ...
                               "locative_determiner_repair_count": len(locative_det_repair_rows),
                               ...
                               "new_locative_determiners": list(HELDOUT_RIGHT_LOCATIVE_DETERMINERS)},
            ...
            "next_repair": "Use the locative-determiner residual to hold out the right clause's locative preposition while preserving event, punctuation, subject, object, modifier, determiner, head noun, and verb.",
