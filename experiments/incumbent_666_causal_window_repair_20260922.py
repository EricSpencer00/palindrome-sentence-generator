"""Repair two mirrored rough shells in the diversified 666 child.

The bounded edit replaces normalized windows [140,197) and [469,526), whose
raw shells are [183,259) and [644,724), with complete finite clauses carrying
an explicit 57-letter reverse obligation.
"""
...
PARENT = ROOT / "runs" / "incumbent-666-central-diversification-20260922.json"
OUT = ROOT / "runs" / "incumbent-666-causal-window-repair-20260922.json"
PARENT_ID = "central-diversification-nadia-spots-666"
PARENT_SHA256 = "6d96e68c2f9cffec70f2c688a89ed5ad2dcdf0a80097f1d711c28ec4fdad298a"
CHILD_SHA256 = "268012039a4f838fdd86b15f37e4bd6d554208f6dd7fd4ceb8fb01c26776b015"
WINDOW_LEFT = (140, 197)
WINDOW_RIGHT = (469, 526)
RAW_LEFT = (183, 259)
RAW_RIGHT = (644, 724)
OLD_LEFT = " Aidan delivers maps. Mara stops rats. A tub? He maps Nora. Deliver no evil."
OLD_RIGHT = " Live on, reviled. Aron, spam. Eh, but a star spots Aram. Spam's reviled, Nadia."
NEW_LEFT = "Nadia saw Noel live. Mara stops Nadia. Nora sees Aram. Sara saw Noel live."
NEW_RIGHT = "Evil Leon was Aras. Mara sees Aron. Aidan spots Aram. Evil Leon was Aidan."
...
def normalized_indices(text: str) -> list[int]:
    return [index for index, char in enumerate(text) if "a" <= char.lower() <= "z"]
...
def build_payload() -> dict[str, object]:
    ...
    indices = normalized_indices(parent_rendered)
    ...
    assert len(left_emission) == WINDOW_LEFT[1] - WINDOW_LEFT[0] == 57
    assert len(right_obligation) == WINDOW_RIGHT[1] - WINDOW_RIGHT[0] == 57
    ...
    row = {
        ...
        "live_seam": {
            ...
            "new_left": NEW_LEFT,
            "new_right": NEW_RIGHT,
            "initial_owner": "left_causal_window",
        ...
        "readability_delta": {
            "targeted_aron_spam_fragment_removed": True,
            "targeted_spams_reviled_nadia_fragment_removed": True,
        ...
        "provenance": (
            "reviewer-specified bounded windows loaded from the exact diversified "
            "666 child; complete finite clauses consume the live reverse-character "
            "obligation before independent admission"
        ),
    }

    return {
        "experiment_id": "incumbent-666-causal-window-repair-20260922",
        "method": "bounded mirrored causal-window repair with live residual ownership",
        "working_incumbent": {
            "artifact": "runs/incumbent-560-outer-causal-scene-20261002.json",
            "id": "outer-causal-scene-568-working-incumbent",
            "letters": 568,
        ...
        "stats": {
            ...
            "left_window_letters": 57,
            "right_window_letters": 57,
            "committed_character_contradictions": 0,
            "backtracks": 0,
        },
        ...
        "next_operator": (
            ...
            "change this causal window only on a committed contradiction"
        ),
    }
