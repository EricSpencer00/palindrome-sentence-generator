"""Bounded object-number/polarity repair with a short locative complement."""
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; EXPERIMENT='relative-seam-object-number-locative-20260917'
...
CHOICES=[('can still','a map','by the inlet','singular_locative'),('cannot yet','maps','near the docks','plural_locative')]
 ...
 for si,setup in enumerate(SETUPS):
  for ri,(noun,anaphor,typed_verb) in enumerate(REFERENTS):
   for ci,relative in enumerate(RELATIVES):
    for xi,(polarity,obj,loc,kind) in enumerate(CHOICES):
     text=f'{setup}, {noun}, {relative}; {BRIDGE} {polarity}, {anaphor} record {obj} {loc}.'
     rows.append({'candidate_id':f's{si}-r{ri}-c{ci}-x{xi}','rendered':text,'regions':['setup','relative_clause','fixed_bridge','polarity_object','locative'],'mutable_spans':['referent_number','object_number_polarity','locative'],'agreement':{'referent':noun,'anaphor':anaphor,'typed_verb':typed_verb},'choice_type':kind,'bridge_fixed':BRIDGE,'audit':audit(text),'provenance':{'construction':'object_number_polarity_short_locative','catalogue_used':False,'wrapped_seed':False,'finished_tape_reversal':False,'repeated_self_palindromic_unit':False}})
 best=min(rows,key=lambda x:x['audit']['mismatches'])
 return {'experiment':EXPERIMENT,'generator_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'rendered_candidates':rows,'stats':{'rendered':len(rows),'exact':sum(x['audit']['two_pointer_exact'] for x in rows),'longest_letters':max(x['audit']['letters'] for x in rows),'best_mismatches':best['audit']['mismatches']},'novelty_preflight':{'new_geometry':'object number and polarity jointly coupled to short locative','prior_lane_reused':False,'duplicate_sweep':False},'next_repair':{'operator':'jointly vary locative preposition with object number while preserving polarity','reason':'locative coupling preserves complete prose but leaves character debt'},'provenance':{'bounded_states':len(rows),'human_readability_certified':False}}
