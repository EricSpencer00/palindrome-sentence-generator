"""Hashed, isolated staff-centre successor.

The prior attack/career artifacts are not overwritten.  This variant enforces
the exact base-source hash before importing its verified scheduler, then binds
one fresh ordinary center/frame: ``staff`` at ``staf|f`` in ``managed a skilled
staff at the public institute``.  No prepared multiword center is used.
"""
...
def isolated_bindings() -> tuple[object, object, object]:
    ...
    by_kind["noun_center"] = by_kind["noun_center"] + (base.Lexeme("staff", "noun_center", "staff"),)
    by_kind["adj_center"] = by_kind["adj_center"] + (base.Lexeme("skilled", "adj_center", "staff"),)
    by_kind["verb_intro"] = by_kind["verb_intro"] + (base.Lexeme("managed", "verb_intro", "management", "person", "staff"),)
    by_kind["noun_event"] = by_kind["noun_event"] + (base.Lexeme("institute", "noun_event", "institution"),)
    base.BY_KIND = by_kind
    base.FRAME = {"staff": {"verb": "managed", "adjectives": {"skilled"}, "relations": {"at"}}}
    base.PIVOTS = {"staff": (4,)}
...
def run(*, state_limit: int = 100_000, closure_limit: int = 100) -> dict[str, object]:
    ...
    try:
        ...
        controls = [
            base.audit(grammar, "The careful researcher managed a skilled staff at the public institute. The careful researcher documented the annual report after the public study today.", "shared_referent_two_sentence_control", tuple(re.findall(r"[a-z]+", "the careful researcher managed a skilled staff at the public institute the careful researcher documented the annual report after the public study today"))),
            base.audit(grammar, "The patient analyst managed a skilled staff at the public clinic. The patient analyst reviewed the public record after the western study today.", "shared_referent_two_sentence_control", tuple(re.findall(r"[a-z]+", "the patient analyst managed a skilled staff at the public clinic the patient analyst reviewed the public record after the western study today"))),
        ]
        ...
        result = {"status": "staff_center_hashed_variant_typed_tree_search", "config": {"state_limit": state_limit, "closure_limit": closure_limit, "one_connected_tree": True, "grammar_owns_every_leaf": True, "word_internal_center_inventory": True, "prepared_multiword_center": False, "joint_boundary_enumeration_before_search": True, "ordinary_staff_frame": True, "prior_sources_hash_enforced": True, "one_character_emission_states": True, "replayed_ledger": True, "independent_complete_reparse": True, "reject_every_self_palindromic_contiguous_multiword_span": True, "corpus_or_catalogue_generation": False}, "grammar_leaf_count": len(leaf_slots), "centre_inventory": inventory, "boundary_inventory": boundaries, "best_boundary_trace": boundaries[0] if boundaries else None, "deepest_full_scheduler_replay": deepest, "stats": dict(stats), "exact_closures": exact, "admitted_closures": admitted, "complete_grammar_controls": controls, "provenance": {"generator_sha256": sha256(Path(__file__).read_bytes()).hexdigest(), "base_generator": str(Path(base.__file__).resolve()), "base_generator_sha256": BASE_SOURCE_SHA256, "grammar_sha256": grammar.digest(), "material": "task-authored ordinary staff center and shared-referent discourse grammar; no catalogue text"}, "reader_facing_next_operator": "Derive any successor from this variant's persisted deepest scheduler ledger; do not mutate the hashed base or prepare a center phrase.", "scope": "This is a bounded construction run; exactness and feature parsing do not certify human readability."}
        return result
    finally:
