...
OUT = ROOT / "runs/word-boundary-since-while-graph-20260920.json"
ID = "word-boundary-since-while-graph-20260920"
SIG = "word-boundary-aware|since-while-duration|connector-edge|live-equations"
...
RELATION_EDGES = (("", "temporal"), (" because the record matters", "causal"), (" so the garden opens", "resultative"), (" although the path narrows", "concessive"), (" if the seal remains", "conditional"), (" until the lantern dims", "temporal-until"), (" since the bell rang", "temporal-since"), (" while the harbor waits", "temporal-while"))
...
def run():
    ...
    rows=[]; near=[]; prunes=0
    for frame in FRAMES:
        for d, a, n, v, od, oa, on, tail, (rel, rel_event, agreement), (orel, orevent, oagreement), (corel, corevent, coagreement, attach), (tense, finite_forms), (third, third_event, third_agreement), (fourth, fourth_event, fourth_agreement), (connector, relation_kind) in itertools.product(dets, adjs, nouns, verbs, dets, adjs, nouns, tails, RELATIVES, OBJECT_RELATIVES, COORD_OBJECT_RELATIVES, TENSE_STATES, THIRD_EVENTS, FOURTH_EVENTS, RELATION_EDGES):
            ...
            relation_tail = Edge(tail.text + connector, "PP", relation_kind)
            rendered = render(subject, v, obj, relation_tail)
    ...
    return {"experiment_id":ID,"method":"bounded word-boundary grammar automaton with four finite event relations, explicit attachment graph, and typed since/while duration connector edges",
      ...
      "novelty_preflight":{"status":"passed","signature":SIG,"distinct_from":"temporal-until connector graph: adds typed since/while durative relations without increasing the event count"},
      "next_topology":{"if_no_closure":"add a causal-subordinator connector edge with explicit attachment and tense state","reason":"current since/while duration connector frame bank has no exact closure above the reader threshold"},
