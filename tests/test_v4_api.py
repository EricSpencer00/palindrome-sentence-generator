...
def test_v4_method_and_best_evaluation_are_explicitly_diagnostic():
    ...
    assert method.json()["optimization"]["current_search"] == "residual-seam-scene-lattice-20260919"
    assert method.json()["optimization"]["search_history"][-1] == "residual-seam-scene-lattice-20260919"
    ...
    assert len(method.json()["method_runs"]) == 4
