...
def run():
 ...
 return {'experiment_id':'dual-relative-graph-antecedents-20260920','method':'fresh dual-relative scene grammar with explicit antecedent indices and graph lexical options','stats':{'heads':len(H),'relative_pairs':len(R)*len(Q),'edges':len(E),'rendered':len(rows),'exact_gt38':len(ex),'max_letters':rows[0]['audit']['letters']},'exact_candidates':ex,'reader_facing_candidates':rows[:12],'novelty_preflight':{'status':'passed','signature':'fresh-authored|dual-relative|explicit-antecedents|graph-lexical-options','distinct_from':'prior graph valency shifts: two independently indexed relative attachments bind separate antecedents before lexical-edge insertion'},'provenance':{'audits':['independent two-pointer comparison','forward/reverse SHA-256'],'hard_exclusions':['nested palindromes','repeated units','word-order symmetry','fragments','catalogue text','mirrored units']},'next_construction':'Replace literal antecedent labels with typed agreement features and carry both relative obligations into a live residual trie.','status':'fresh exact >38 requires reading' if ex else 'no fresh exact >38; dual-relative prose controls retained'}
