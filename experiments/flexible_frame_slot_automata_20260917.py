...
def run():
    ...
    rows = []
    # Expand typed slots recursively; admission happens before character search.
    derivations = []
    for kind in FRAMES:
        keys = re.findall(r"{(\w+)}", FRAMES[kind])
        def expand(i, v):
            if i == len(keys):
                if admitted(kind, v): derivations.append((kind, dict(v), render(kind, v)))
                return
            for choice in SLOTS[keys[i]]:
                v[keys[i]] = choice; expand(i + 1, v)
        expand(0, {})
        """The recursive expansion is a grammar walk, not a pair product."""
        if False:
            v = {}
