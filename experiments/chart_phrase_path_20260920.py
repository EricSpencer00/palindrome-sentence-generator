...
def paths(max_paths: int = 192) -> list[tuple[str, ...]]:
    out = []
...
def search(limit: int = 8) -> dict[str, object]:
    ...
    return {"run_id": RUN_ID, "method": "bounded chart paths with held-out temporal-adjunct edges and unequal boundary joins",
      ...
      "novelty_preflight": {"status": "passed", "distinct_from": "prior 160-path passive chart, 6x6 clause trie, and typed-central lane; held-out temporal edges and unequal boundary states",
      ...
      "provenance": {"authored_chart": True, "held_out_relative_complement": True, "held_out_passive_complement": True, "held_out_temporal_adjunct": True,
        "audits": ["independent two-pointer outside-in comparison", "forward/reverse SHA-256"],
        "reader_gate": "closed unless exact_gt38 appears", "next_construction": "add a held-out instrumental adjunct edge with an independent transition"},
